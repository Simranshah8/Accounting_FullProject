...
app.controller('Ledger', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.LoadData = function () {
		...
		//		$scope.PaymentTypeColl = res.data.Data;			 
		//	}
		//}, function (reason) {
		...
		$http({
			...
			$scope.loadingstatus = "stop";
			if (res.data.IsSuccess && res.data.Data) {
				$scope.DebtorGroupList = res.data.Data;
			}
		}, function (reason) {
			Swal.fire('Failed' + reason);
		});
		...
		$timeout(function () {
			...
			$http({
				...
				if (res.data.IsSuccess && res.data.Data) {
					...
					$scope.DebtorType_Qry = mx(res.data.Data);
				}
			}, function (reason) {
				Swal.fire('Failed' + reason);
			});
		...
		$http({
			...
			hidePleaseWait();
			if (res.data.IsSuccess && res.data.Data) {
				var uColl = res.data.Data;
				angular.forEach(uColl, function (uc) {
					if (uc.UserType == 2)
						$scope.UserNameColl.push(uc);
				});
			} else
	...
	$scope.IsDealer = function (id) {
		var val = false;

		if ($scope.DebtorType_Qry && id>0) {
			var find = $scope.DebtorType_Qry.firstOrDefault(p1 => p1.DebtorTypeId == id);
			if (find) {
				return true;
            }
        }
	...
	$scope.GenerateCode = function () {
		...
		var para = {
			...
			panVatNo: ($scope.newLedger.StatutoryDetail ? $scope.newLedger.StatutoryDetail.PanVatNo : ''),
			mobileNo: ($scope.newLedger.CompanyContactNo ? $scope.newLedger.CompanyContactNo : '')
		};
		$http({
			method: 'POST',
	...
	$scope.IsValidLedger = function () {
		...
		//if (isEmpty($scope.newLedger.LedgerGroupId) == 0) {
		//	Swal.fire('Please ! Select Group Name');
		//	return false;
		//}
	...
	$scope.CallSaveUpdateLedger = function () {
		...
		if (!$scope.newLedger.LedgerGroupId || $scope.newLedger.LedgerGroupId == 0)
			$scope.newLedger.LedgerGroupId = 12;
		...
		$http({
			method: 'POST',
			url: base_url + "Account/Creation/SaveCustomer",
		...
		if ($scope.MulData == null) {
			...
			$http({
				...
				if (res1.data.IsSuccess && res1.data.PropertiesColl) {
					...
					$scope.MulData.Title = 'Create Multiple Customer';
		...
		if (((!$scope.newLedger.Name || $scope.newLedger.Name.length == 0) || lastPanVat != panVatNo) && panVatNo.toString().length > 2)
		{
			...
			$http({
				...
				if (data && data.panDetails && data.panDetails.length > 0) {
					...
					if (data.panRegistrationDetail && data.panRegistrationDetail.length > 0) {
						showPanDetailsList(data.panRegistrationDetail);
					}
				} else {
	...
	function showPanDetailsList(list) {
		...
		list.forEach((item, index) => {
            ...
            <tr>
                ...
                <td style="padding:6px; border:1px solid #ddd;">${item.pan}</td>
                <td style="padding:6px; border:1px solid #ddd;">${item.acctType}</td>
                <td style="padding:6px; border:1px solid #ddd;">${item.registrationDate}</td>
                <td style="padding:6px; border:1px solid #ddd;">${item.filing_Period}</td>
                <td style="padding:6px; border:1px solid #ddd;">${item.accountStatus}</td>
            </tr>
        `;
		});

		Swal.fire({
			title: 'PAN Registration Details',
			width: 900,
			html: `
                ...
                <table style="width:100%; border-collapse:collapse; text-align:left;">
                    <thead>
                        <tr style="background:#f2f2f2;">
                            ...
                            <th style="padding:8px; border:1px solid #ddd;">PAN</th>
                            <th style="padding:8px; border:1px solid #ddd;">Account Type</th>
                            <th style="padding:8px; border:1px solid #ddd;">Registration Date</th>
                            <th style="padding:8px; border:1px solid #ddd;">Filing Period</th>
