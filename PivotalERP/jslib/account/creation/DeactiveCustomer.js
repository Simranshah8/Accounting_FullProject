app.controller('deactiveCustomerController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.GetPendingLocation = function () {
		...
		var para = {
			procName: 'U_GetPendingCustomerForDeActive',
			qry: '',
			asParentChild: false,
			tblNames: '',
			colRelations: '',
	...
	$scope.PostModal = function (obj) {
		...
		Swal.fire({
			title: 'Do you want to deactive the selected customer of (' + obj.Name + ')   ? ',
			...
			if (result.isConfirmed) {
				...
				$http({
					method: 'POST',
					url: base_url + "Account/Creation/ApprovedDactiveCust",
					dataType: "json",
	...
	$scope.ApprovedAll = function () {
		...
		Swal.fire({
			title: 'Do you want to deactive all pending customer ? ',
			showCancelButton: true,
			confirmButtonText: 'Post',
		}).then((result) => {
			/* Read more about isConfirmed, isDenied below */
			if (result.isConfirmed) {
				...
				$http({
					method: 'POST',
					url: base_url + "Account/Creation/ApprovedAllDactiveCust",
					dataType: "json",
