app.controller('LedgerTargetController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	$scope.Title = 'Target ';
	...
	$scope.LoadData = function () {
		...
		$scope.TargetForColl = [{ id: 1, text: 'Distributors' }, { id: 2, text: 'Wholesaler' }];
		...
		$scope.CustomerList_D = [];
		$scope.CustomerList_W = [];

		$http({
			method: 'POST',
			url: base_url + "Account/Creation/GetLedgerForTarget",
			dataType: "json"
		}).then(function (res) {
			if (res.data.IsSuccess && res.data.Data) {
				var CustomerList = res.data.Data;
				if (CustomerList) {
					CustomerList.forEach(function (cs) {
						if (cs.DebtorType == 'Distributor') {
							$scope.CustomerList_D.push(cs);
						} else {
							$scope.CustomerList_W.push(cs);
                        }
					});
                }
			}
	...
	$scope.GetMonthlyTarget = function () {
		...
		var para = {
			...
			LedgerId: null,
			TargetFor:$scope.newMonthly.TargetFor
		};
		$http({
			method: 'POST',
			url: base_url + "Account/Creation/GetLedgerTarget",
			dataType: "json",
	...
	$scope.SaveSalesmanTarget = function () {
		...
		$http({
			method: 'POST',
			url: base_url + "Account/Creation/SaveLedgerTarget",
	...
	$scope.GetYearlyTarget = function () {
		...
		var para = {
			...
			LedgerId: 0,
			TargetFor: $scope.newYearly.TargetFor,
		};
		$http({
			method: 'POST',
			url: base_url + "Account/Creation/GetLedgerTarget",
			...
			if (res.data.IsSuccess && res.data.Data) {
				...
				var groupQry = dtColl.groupBy(p1 => p1.LedgerId);
	...
	$scope.GetSalesmanTarget = function ()
	{
		...
		var para = {
			...
			LedgerId: $scope.newSalesmanWise.LedgerId,
			TargetFor: $scope.newSalesmanWise.TargetFor,
		};
		$http({
			method: 'POST',
			url: base_url + "Account/Creation/GetLedgerTarget",
