...
app.controller("LedgerVoucherDetails", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    LoadData();
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'LedgerVoucherDetails.csv',
            sheetName: 'LedgerVoucherDetails'
        };
    ...
    function LoadData() {
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.LedgerVoucherDetails.DateFrom_TMP = new Date(comDet.StartDate);
                }
        ...
        $scope.columnDefs = [
            {
                headerName: "Date(A.D.)", width: 250, field: "VoucherDate", cellRenderer: 'agGroupCellRenderer',
            ...
            {
                headerName: "Date(B.S.)", width: 250, field: "VoucherDateBS", cellRenderer: 'agGroupCellRenderer',
            ...
            {
                headerName: "Particular's", width: 250,
            ...
            { headerName: "CostCenter", width: 250, field: "CostCenter", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            { headerName: "ChequeNo", width: 250, field: "ChequeNo", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'center' } },
            { headerName: "ChequeRemarks", width: 250, field: "ChequeRemarks", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            { headerName: "Amount", width: 250, field: "TranDrAmount", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'right' } },
            { headerName: "VoucherName", width: 250, field: "VoucherName", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            { headerName: "VoucherType", width: 250, field: "VoucherType", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            { headerName: "Voucher No.", width: 250, field: "VoucherNo", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'center' } },
            { headerName: "Ref.No.", width: 250, field: "RefNo", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'center' } },
            { headerName: "Debit", width: 250, field: "DrAmount",cellStyle: { 'text-align': 'right' } },
            { headerName: "Credit", width: 250, field: "CrAmount",cellStyle: { 'text-align': 'right' } },
            { headerName: "CurrentClosing", width: 250, field: "CurrentClosing",cellStyle: { 'text-align': 'right' } },
            { headerName: "CostClass", width: 250, field: "CostClass",cellStyle: { 'text-align': 'left' } },
            { headerName: "User", width: 250, field: "UserName",cellStyle: { 'text-align': 'left' } },
            { headerName: "IsLocked", width: 250, field: "IsLocked",cellStyle: { 'text-align': 'left' } },
            { headerName: "Branch", width: 250, field: "Branch",cellStyle: { 'text-align': 'left' } },
            { headerName: "LogDateTime", width: 250, field: "LogDateTime",cellStyle: { 'text-align': 'center' } },
            { headerName: "LedgerNarration", width: 250, field: "LedgerNarration",cellStyle: { 'text-align': 'left' } },
            { headerName: "VoucherAge", width: 250, field: "VoucherAge",cellStyle: { 'text-align': 'center' } },
...
$timeout(function () {
    ...
    $scope.ClearData = function () {
        ...
        $scope.LedgerVoucherDetails.ODr = 0;
        $scope.LedgerVoucherDetails.OCr = 0;
        $scope.LedgerVoucherDetails.TDr = 0;
        $scope.LedgerVoucherDetails.TCr = 0;
        $scope.LedgerVoucherDetails.CDr = 0;
        $scope.LedgerVoucherDetails.CCr = 0;
    ...
    $scope.GetLedgerVoucherDetails = function () {

        $scope.ClearData();

        if (!$scope.LedgerVoucherDetails.LedgerId)
            return;

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.LedgerVoucherDetails.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.LedgerVoucherDetails.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.LedgerVoucherDetails.DateToDet)
            dateTo = new Date(($filter('date')($scope.LedgerVoucherDetails.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            ledgerId: $scope.LedgerVoucherDetails.LedgerId,
            DrAmount: 0,
            CrAmount: 0
        };
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetLedgerVoucherDetails",
            ...
            $scope.LedgerVoucherDetails.ODr = (openingAmt > 0 ? openingAmt : 0);
            $scope.LedgerVoucherDetails.OCr = (openingAmt < 0 ? Math.abs(openingAmt) : 0);
            $scope.LedgerVoucherDetails.TDr = drAmt;
            $scope.LedgerVoucherDetails.TCr = crAmt;
            $scope.LedgerVoucherDetails.CDr = (closingAmt > 0 ? closingAmt : 0);
            $scope.LedgerVoucherDetails.CCr = (closingAmt < 0 ? Math.abs(closingAmt) : 0);
                                ...
                                return new Promise((resolve) => {
                                    if (value >= 0) {
                                        ...
                                        if (rptTranId > 0) {
                                            ...
                                            $http({
                                                method: 'POST',
                                                url: base_url + "Account/Reporting/PrintLedgerVoucherDetails",
                                                ...
                                                if (res.data.IsSuccess && res.data.Data) {
                                                    ...
                                                    var rptPara = {
                                                        ...
                                                        Period: $scope.LedgerVoucherDetails.DateFromDet.dateBS + " TO " + $scope.LedgerVoucherDetails.DateToDet.dateBS,
                                                        ODr: $scope.LedgerVoucherDetails.ODr,
                                                        OCr: $scope.LedgerVoucherDetails.OCr,
                                                        TDr: $scope.LedgerVoucherDetails.TDr,
                                                        TCr: $scope.LedgerVoucherDetails.TCr,
                                                        CDr: $scope.LedgerVoucherDetails.CDr,
                                                        CCr: $scope.LedgerVoucherDetails.CCr
                                                    };
    ...
    $scope.GetDataForPrint = function () {
        ...
        RptParamentersColl.push({
            ...
            {
                Name: 'Ledger',
                Value: $scope.LedgerVoucherDetails.LedgerDetails.Name
            },
            {
                Name: 'Address',
                Value: ($scope.LedgerVoucherDetails.LedgerDetails.Address ? $scope.LedgerVoucherDetails.LedgerDetails.Address : '')
            },
            {
                Name: 'MobileNo',
                Value: ($scope.LedgerVoucherDetails.LedgerDetails.MobileNo1 ? $scope.LedgerVoucherDetails.LedgerDetails.MobileNo1 : '')
            },
            {
                Name: 'PanVatNo',
                Value: ($scope.LedgerVoucherDetails.LedgerDetails.PanVat ? $scope.LedgerVoucherDetails.LedgerDetails.PanVat : '')
            },
            {
                Name: 'TelNo',
                Value: ($scope.LedgerVoucherDetails.LedgerDetails.MobileNo2 ? $scope.LedgerVoucherDetails.LedgerDetails.MobileNo2 : '')
            },
            {
                Name: 'EmailId',
                Value: ($scope.LedgerVoucherDetails.LedgerDetails.EmailId ? $scope.LedgerVoucherDetails.LedgerDetails.EmailId : '')
            },
            {
                Name: 'ODr',
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "LedgerVoucherDetails.xlsx");
            }
