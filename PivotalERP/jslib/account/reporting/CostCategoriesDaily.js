...
app.controller("CostCategoriesDailyController", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
 var PrintPreviewAs = 1;
  const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'CostCategoriesDaily.csv',


            sheetName: 'CostCategoriesDaily'
        };
    ...
    function LoadData() {
        ...
        $scope.columnDefs = [
            { headerName: "Particulars", width: 300, field: "MonthName", dataType: 'Text',cellStyle:{'text-align':'left'} },
...
$timeout(function () {
    ...
    $scope.GetCostCategoriesDaily = function () {
        ...
        if ($scope.CostCategoriesDaily.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.CostCategoriesDaily.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.CostCategoriesDaily.DateToDet)
            dateTo = new Date(($filter('date')($scope.CostCategoriesDaily.DateToDet.dateAD, 'yyyy-MM-dd')));

        $scope.loadingstatus = 'running';
        showPleaseWait();

        var beData = {
            CostCategoriesId: $scope.CostCategoriesDaily.CostCategoriesId,
            DateFrom: dateFrom,
            DateTo: dateTo,
        };
        ...
        $http({
            method: "POST",
            url: base_url + "Account/Reporting/GetAllCostCategoriesDailySummary",
...
$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "CostCategoriesDaily.xlsx");
            }
