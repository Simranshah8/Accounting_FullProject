...
app.controller("BillsReceivableSummary", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
 ...
 var PrintPreviewAs = 1;
  const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'BillsReceivableSummary.csv',
            sheetName: 'BillsReceivableSummary'
        };
    ...
    $scope.GenConfig = {};
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.BillsReceivableSummary.DateFrom_TMP = new Date(comDet.StartDate);
                }
            }, function (errormessage) {
                alert('Unable to Delete data. pls try again.' + errormessage.responseText);
            });
        });
        $scope.BillsReceivableSummary = {
        ...
        var columnDefs = [
            ...
            { headerName: "RefNO", width: 120, field: "RefNO",cellStyle:{'text-align':'center'} },

            { headerName: "VoucherNo", width: 180, field: "VoucherId",cellStyle:{'text-align':'center'} },
            ...
            { headerName: "GroupName", width: 180, field: "GroupName",cellStyle:{'text-align':'left'} },
            { headerName: "SalesLedger", width: 180, field: "SalesLedgerName",cellStyle:{'text-align':'left'} },
            { headerName: "Salesman", width: 180, field: "Salesman",cellStyle:{'text-align':'left'} },
            { headerName: "Area", width: 180, field: "Area",cellStyle:{'text-align':'left'} },
            { headerName: "DrAmt", width: 180, field: "TotalDebit",cellStyle:{'text-align':'right'} },
            { headerName: "CrAmt", width: 180, field: "TotalCredit",cellStyle:{'text-align':'right'} },
            { headerName: "CurrentClosing", width: 180, field: "CurrentClosing",cellStyle:{'text-align':'right'} },
            { headerName: "PendingAmount", width: 180, field: "PendingAmount",cellStyle:{'text-align':'right'} },
            { headerName: "TotalDays", width: 180, field: "TotalDays",cellStyle:{'text-align':'center'} },
...
onCellContextMenu: onCellContextMenu, // Handle right-click event
    ...
    $scope.GetBillsReceivableSummary = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.BillsReceivableSummary.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.BillsReceivableSummary.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.BillsReceivableSummary.DateToDet)
            dateTo = new Date(($filter('date')($scope.BillsReceivableSummary.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetBillsReceivableSummary",
            ...
            if (res.data.IsSuccess && res.data.Data) {
                ...
                angular.forEach($scope.DataColl, function (dc) {
                    if (dc.TotalDebit == 1 || dc.TotalDebit == 'DR')
                        Opening += dc.TotalDebit;
                    else
                        Opening -= dc.TotalDebit;
                });
                ...
                $scope.dataForBottomGrid[0].TotalDebit = Opening;
                $scope.dataForBottomGrid[0].TotalCredit = drcr;
                $scope.gridOptionsBottom.api.setRowData($scope.dataForBottomGrid);
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "BillsReceivableSummary.xlsx");
            }
