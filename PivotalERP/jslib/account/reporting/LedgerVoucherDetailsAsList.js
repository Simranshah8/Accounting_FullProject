...
app.controller("LedgerVoucherDetailsAsList", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    LoadData();
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'LedgerVoucherDetailsAsList.csv',
            sheetName: 'LedgerVoucherDetailsAsList'
        };
    ...
    function LoadData() {
        $scope.LedgerVoucherDetailsAsList = {
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.LedgerVoucherDetailsAsList.DateFrom_TMP = new Date(comDet.StartDate);
                }
        ...
        $scope.columnDefs = [
            ...
            { headerName: "VoucherType", width: 250, field: "VoucherName", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            ...
            { headerName: "Debit", width: 250, field: "DrAmt",cellStyle: { 'text-align': 'right' } },
            { headerName: "Credit", width: 250, field: "CrAmt",cellStyle: { 'text-align': 'right' } },
...
$timeout(function () {
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    ...
                    if (res.data.IsSuccess) {
                        $scope.GetLedgerVoucherDetailsAsList();
                    } else {
    ...
    $scope.ClearData = function () {
        ...
        $scope.LedgerVoucherDetailsAsList.ODr = 0;
        $scope.LedgerVoucherDetailsAsList.OCr = 0;
        $scope.LedgerVoucherDetailsAsList.TDr = 0;
        $scope.LedgerVoucherDetailsAsList.TCr = 0;
        $scope.LedgerVoucherDetailsAsList.CDr = 0;
        $scope.LedgerVoucherDetailsAsList.CCr = 0;
    ...
    $scope.GetLedgerVoucherDetailsAsList = function () {

        $scope.ClearData();

        if (!$scope.LedgerVoucherDetailsAsList.LedgerId)
            return;

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.LedgerVoucherDetailsAsList.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.LedgerVoucherDetailsAsList.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.LedgerVoucherDetailsAsList.DateToDet)
            dateTo = new Date(($filter('date')($scope.LedgerVoucherDetailsAsList.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            ledgerId: $scope.LedgerVoucherDetailsAsList.LedgerId,
            DrAmt: 0,
            CrAmt: 0
        };
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetLedgerVoucherDetailsAsList",
            ...
            $scope.LedgerVoucherDetailsAsList.ODr = (openingAmt > 0 ? openingAmt : 0);
            $scope.LedgerVoucherDetailsAsList.OCr = (openingAmt < 0 ? Math.abs(openingAmt) : 0);
            $scope.LedgerVoucherDetailsAsList.TDr = drAmt;
            $scope.LedgerVoucherDetailsAsList.TCr = crAmt;
            $scope.LedgerVoucherDetailsAsList.CDr = (closingAmt > 0 ? closingAmt : 0);
            $scope.LedgerVoucherDetailsAsList.CCr = (closingAmt < 0 ? Math.abs(closingAmt) : 0);
                                ...
                                return new Promise((resolve) => {
                                    if (value >= 0) {
                                        ...
                                        if (rptTranId > 0) {
                                            ...
                                            $http({
                                                method: 'POST',
                                                url: base_url + "Account/Reporting/PrintLedgerVoucherDetailsAsList",
                                                ...
                                                if (res.data.IsSuccess && res.data.Data) {
                                                    ...
                                                    var rptPara = {
                                                        ...
                                                        Period: $scope.LedgerVoucherDetailsAsList.DateFromDet.dateBS + " TO " + $scope.LedgerVoucherDetailsAsList.DateToDet.dateBS,
                                                        ODr: $scope.LedgerVoucherDetailsAsList.ODr,
                                                        OCr: $scope.LedgerVoucherDetailsAsList.OCr,
                                                        TDr: $scope.LedgerVoucherDetailsAsList.TDr,
                                                        TCr: $scope.LedgerVoucherDetailsAsList.TCr,
                                                        CDr: $scope.LedgerVoucherDetailsAsList.CDr,
                                                        CCr: $scope.LedgerVoucherDetailsAsList.CCr
                                                    };
    ...
    $scope.GetDataForPrint = function () {
        ...
        RptParamentersColl.push({
            ...
            {
                Name: 'Ledger',
                Value: $scope.LedgerVoucherDetailsAsList.LedgerDetails.Name
            },
            {
                Name: 'Address',
                Value: ($scope.LedgerVoucherDetailsAsList.LedgerDetails.Address ? $scope.LedgerVoucherDetailsAsList.LedgerDetails.Address : '')
            },
            {
                Name: 'MobileNo',
                Value: ($scope.LedgerVoucherDetailsAsList.LedgerDetails.MobileNo1 ? $scope.LedgerVoucherDetailsAsList.LedgerDetails.MobileNo1 : '')
            },
            {
                Name: 'PanVatNo',
                Value: ($scope.LedgerVoucherDetailsAsList.LedgerDetails.PanVat ? $scope.LedgerVoucherDetailsAsList.LedgerDetails.PanVat : '')
            },
            {
                Name: 'TelNo',
                Value: ($scope.LedgerVoucherDetailsAsList.LedgerDetails.MobileNo2 ? $scope.LedgerVoucherDetailsAsList.LedgerDetails.MobileNo2 : '')
            },
            {
                Name: 'EmailId',
                Value: ($scope.LedgerVoucherDetailsAsList.LedgerDetails.EmailId ? $scope.LedgerVoucherDetailsAsList.LedgerDetails.EmailId : '')
            },
            {
                Name: 'ODr',
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "LedgerVoucherDetailsAsList.xlsx");
            }
