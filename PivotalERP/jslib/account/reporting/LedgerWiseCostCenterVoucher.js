...
app.controller("LedgerWiseCostCenterVoucher", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    LoadData();
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'LedgerWiseCostCenterVoucher.csv',
            sheetName: 'LedgerWiseCostCenterVoucher'
        };
    ...
    function LoadData() {
        $scope.LedgerWiseCostCenterVoucher = {
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.LedgerWiseCostCenterVoucher.DateFrom_TMP = new Date(comDet.StartDate);
                }
        ...
        $scope.columnDefs = [
            ...
            { headerName: "VoucherNo", width: 250, field: "VoucherNo", filter: 'agTextColumnFilter', },
            {
            ...
            { headerName: "Particulars", width: 250, field: "Particulars", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            { headerName: "Code", width: 250, field: "CostCenterCode", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            { headerName: "RefNo", width: 250, field: "RefNo", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'center' } },
            { headerName: "VoucherType", width: 250, field: "VoucherType", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            { headerName: "DrAmount", width: 250, field: "DrAmunt", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'right' } },
            { headerName: "CrAmount", width: 250, field: "CrAmount", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'right' } },
            { headerName: "Narration", width: 250, field: "Narration", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            { headerName: "LedgerNarration", width: 250, field: "LedgerNarration", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            { headerName: "CostCenterNarration", width: 250, field: "CostCenterNarration", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'left' } },
            ...
            { headerName: "CurrentClosing", width: 250, field: "CurrentClosingBalance", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'right' } },
            { headerName: "Days", width: 250, field: "Days", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'center' } },
            { headerName: "Interest", width: 250, field: "InterestAmt", filter: 'agTextColumnFilter',cellStyle: { 'text-align': 'right' } },
...
$timeout(function () {
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    ...
                    if (res.data.IsSuccess) {
                        $scope.GetCostCenterBreakupLedgerWise();
                    } else {
    ...
    $scope.ClearData = function () {
        ...
        $scope.LedgerWiseCostCenterVoucher.ODr = 0;
        $scope.LedgerWiseCostCenterVoucher.OCr = 0;
        $scope.LedgerWiseCostCenterVoucher.TDr = 0;
        $scope.LedgerWiseCostCenterVoucher.TCr = 0;
        $scope.LedgerWiseCostCenterVoucher.CDr = 0;
        $scope.LedgerWiseCostCenterVoucher.CCr = 0;
    ...
    $scope.GetLedgerWiseCostCenterVoucher= function () {

        $scope.ClearData();

        if (!$scope.LedgerWiseCostCenterVoucher.AgentId)
            return;

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.LedgerWiseCostCenterVoucher.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.LedgerWiseCostCenterVoucher.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.LedgerWiseCostCenterVoucher.DateToDet)
            dateTo = new Date(($filter('date')($scope.LedgerWiseCostCenterVoucher.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            LedgerId: $scope.LedgerWiseCostCenterVoucher.LedgerId,
            BranchIdColl: $scope.LedgerWiseCostCenterVoucher.BranchIdColl
        };
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetLedgerWiseCostCenterVoucher",
            ...
            $scope.LedgerWiseCostCenterVoucher.ODr = (openingAmt > 0 ? openingAmt : 0);
            $scope.LedgerWiseCostCenterVoucher.OCr = (openingAmt < 0 ? Math.abs(openingAmt) : 0);
            $scope.LedgerWiseCostCenterVoucher.TDr = drAmt;
            $scope.LedgerWiseCostCenterVoucher.TCr = crAmt;
            $scope.LedgerWiseCostCenterVoucher.CDr = (closingAmt > 0 ? closingAmt : 0);
            $scope.LedgerWiseCostCenterVoucher.CCr = (closingAmt < 0 ? Math.abs(closingAmt) : 0);
                                ...
                                return new Promise((resolve) => {
                                    if (value >= 0) {
                                        ...
                                        if (rptTranId > 0) {
                                            ...
                                            $http({
                                                method: 'POST',
                                                url: base_url + "Account/Reporting/PrintLedgerWiseCostCenterVoucher",
                                                ...
                                                if (res.data.IsSuccess && res.data.Data) {
                                                    ...
                                                    var rptPara = {
                                                        ...
                                                        Period: $scope.LedgerWiseCostCenterVoucher.DateFromDet.dateBS + " TO " + $scope.LedgerWiseCostCenterVoucher.DateToDet.dateBS,
                                                        ODr: $scope.LedgerWiseCostCenterVoucher.ODr,
                                                        OCr: $scope.LedgerWiseCostCenterVoucher.OCr,
                                                        TDr: $scope.LedgerWiseCostCenterVoucher.TDr,
                                                        TCr: $scope.LedgerWiseCostCenterVoucher.TCr,
                                                        CDr: $scope.LedgerWiseCostCenterVoucher.CDr,
                                                        CCr: $scope.LedgerWiseCostCenterVoucher.CCr
                                                    };
    ...
    $scope.GetDataForPrint = function () {
        ...
        RptParamentersColl.push({
            ...
            {
                Name: 'Ledger',
                Value: $scope.LedgerWiseCostCenterVoucher.LedgerDetails.Name
            },
            {
                Name: 'Address',
                Value: ($scope.LedgerWiseCostCenterVoucher.LedgerDetails.Address ? $scope.LedgerWiseCostCenterVoucher.LedgerDetails.Address : '')
            },
            {
                Name: 'MobileNo',
                Value: ($scope.LedgerWiseCostCenterVoucher.LedgerDetails.MobileNo1 ? $scope.LedgerWiseCostCenterVoucher.LedgerDetails.MobileNo1 : '')
            },
            {
                Name: 'PanVatNo',
                Value: ($scope.LedgerWiseCostCenterVoucher.LedgerDetails.PanVat ? $scope.LedgerWiseCostCenterVoucher.LedgerDetails.PanVat : '')
            },
            {
                Name: 'TelNo',
                Value: ($scope.LedgerWiseCostCenterVoucher.LedgerDetails.MobileNo2 ? $scope.LedgerWiseCostCenterVoucher.LedgerDetails.MobileNo2 : '')
            },
            {
                Name: 'EmailId',
                Value: ($scope.LedgerWiseCostCenterVoucher.LedgerDetails.EmailId ? $scope.LedgerWiseCostCenterVoucher.LedgerDetails.EmailId : '')
            },
            {
                Name: 'ODr',
...
$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "LedgerWiseCostCenterVoucher.xlsx");
            }
