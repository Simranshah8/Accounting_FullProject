...
app.controller("SalesManMonthly", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'SalesManMonthly.csv',
            sheetName: 'SalesManMonthly'
        };
 ...
 $scope.GenConfig = {};
        ...
        $http({
            method: 'GET',
            url: base_url + "Account/Reporting/GetAllAgentList",
            dataType: "json"
        }).then(function (res) {
            if (res.data.IsSuccess && res.data.Data) {
        ...
        $scope.VoucherSearchOptions = [
            { text: 'Particulars', value: 'MonthName', dataType: 'text' },
        ...
        $scope.columnDefs = [
            { headerName: "Particulars", width: 350, field: "LedgerName", dataType: 'Text', cellStyle: { 'text-align': 'left' } },
...
$timeout(function () {
    ...
    $scope.GetSalesManMonthly = function () {
        $scope.ClearData();
        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.SalesManMonthly.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.SalesManMonthly.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.SalesManMonthly.DateToDet)
            dateTo = new Date(($filter('date')($scope.SalesManMonthly.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            ...
            AgentId: $scope.SalesManMonthly.AgentId,
        ...
        $http({
            method: "POST",
            url: base_url + "Account/Reporting/GetAllSalesManMonthly",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "SalesManMonthly.xlsx");
            }
