...
app.controller("InterestCalculationLedgerWiseAfterDays", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'InterestCalculationLedgerWiseAfterDays.csv',
            sheetName: 'InterestCalculationLedgerWiseAfterDays'
        };
 ...
 $scope.GenConfig = {};
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.InterestCalculationLedgerWiseAfterDays.DateFrom_TMP = new Date(comDet.StartDate);
                }
        ...
        $scope.InterestCalculationLedgerWiseAfterDays = {
            LedgerGroupId: 0,
        ...
        var columnDefs = [
            ...
            { headerName: "Date", width: 250, field: "VoucherDate",cellStyle:{'text-align':'center'} },
            { headerName: "DebitAmount", width: 250, field: "DrAmount",cellStyle:{'text-align':'righjt'} },
            { headerName: "CreditAmount", width: 250, field: "CrAmount",cellStyle:{'text-align':'righjt'} },
            { headerName: "TotalDays", width: 250, field: "TotalDays",cellStyle:{'text-align':'center'} },
            { headerName: "Rate", width: 250, field: "Rate",cellStyle:{'text-align':'right'} },
            { headerName: "Interest", width: 250, field: "Interest",cellStyle:{'text-align':'right'} },
            { headerName: "BalanceAmount", width: 250, field: "BalanceAmount",cellStyle:{'text-align':'right'} },
...
$timeout(function () {
    ...
    $scope.GetInterestCalculationLedgerWiseAfterDays = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.InterestCalculationLedgerWiseAfterDays.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.InterestCalculationLedgerWiseAfterDays.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.InterestCalculationLedgerWiseAfterDays.DateToDet)
            dateTo = new Date(($filter('date')($scope.InterestCalculationLedgerWiseAfterDays.DateToDet.dateAD, 'yyyy-MM-dd')));

        $scope.DataColl = []; //declare an empty array
        $scope.gridOptions.api.setRowData($scope.DataColl);

        var beData = {
            LedgerGroupId: $scope.InterestCalculationLedgerWiseAfterDays.LedgerGroupId,
            DateFrom: dateFrom,
            DateTo: dateTo,
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetInterestCalculationLedgerWiseAfterDays",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "InterestCalculationLedgerWiseAfterDays.xlsx");
            }
