...
app.controller("PendingDayBook", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'PendingDayBook.csv',
            sheetName: 'PendingDayBook'
        };
 ...
 $scope.GenConfig = {};
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.PendingDayBook.DateFrom_TMP = new Date(comDet.StartDate);
                }
            }, function (errormessage) {
                alert('Unable to Delete data. pls try again.' + errormessage.responseText);
            });
        });
        $scope.PendingDayBook = {
        ...
        var columnDefs = [
            ...
            {
                headerName: "Date(B.S.)", width: 250, field: "VoucherDateStrNP", cellRenderer: 'agGroupCellRenderer',
            ...
            { headerName: "Particulars", width: 180, field: "PartyLedger", cellStyle: { 'text-align': 'left' } },
            { headerName: "VoucherType", width: 180, field: "VoucherType", cellStyle: { 'text-align': 'left' } },
            { headerName: "VoucherNo", width: 180, field: "AutoVoucherNo", cellStyle: { 'text-align': 'right' } },
            { headerName: "RefNo", width: 180, field: "RefNumber", cellStyle: { 'text-align': 'right' } },
            { headerName: "CrAmount", width: 180, field: "CrAmount", cellStyle: { 'text-align': 'right' } },
            { headerName: "DrAmount", width: 180, field: "DrAmount", cellStyle: { 'text-align': 'right' } },
            { headerName: "CostClass", width: 180, field: "CostClassName", cellStyle: { 'text-align': 'left' } },
            { headerName: "User", width: 180, field: "User", cellStyle: { 'text-align': 'left' } },
            { headerName: "DeliveryNoteVoucherNo", width: 250, field: "DeliveryNoteVoucherNo", cellStyle: { 'text-align': 'right' } },
            { headerName: "ReceiptNoteVoucherNo", width: 250, field: "ReceiptNoteVoucherNo", cellStyle: { 'text-align': 'right' } },
            { headerName: "ReturnInvoiceVoucherNo", width: 250, field: "ReturnInvoiceVoucherNo", cellStyle: { 'text-align': 'right' } },
            { headerName: "OrderVoucherNo", width: 250, field: "OrderVoucherNo", cellStyle: { 'text-align': 'right' } },
            { headerName: "InvoiceVoucherNo", width: 250, field: "InvoiceVoucherNo", cellStyle: { 'text-align': 'right' } },
            { headerName: "QuotationVoucherNo", width: 250, field: "QuotationVoucherNo", cellStyle: { 'text-align': 'right' } },
...
$timeout(function () {
    ...
    $scope.GetPendingDayBook = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.PendingDayBook.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.PendingDayBook.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.PendingDayBook.DateToDet)
            dateTo = new Date(($filter('date')($scope.PendingDayBook.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            ...
            DateFrom: dateFrom,
            DateTo: dateTo,
            VoucherType: $scope.PendingDayBook.VoucherType,
            isPending: $scope.PendingDayBook.isPending,
        };
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetAllPendingDayBook",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "PendingDayBook.xlsx");
            }
