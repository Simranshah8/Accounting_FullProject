...
app.controller("StatisticVoucher", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'StatisticVoucher.csv',
            sheetName: 'StatisticVoucher'
        };
 ...
 $scope.GenConfig = {};
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.StatisticVoucher.DateFrom_TMP = new Date(comDet.StartDate);
                }
            }, function (errormessage) {
                alert('Unable to Delete data. pls try again.' + errormessage.responseText);
            });
        });
        $scope.StatisticVoucher = {
...
$timeout(function () {
    ...
    $scope.GetStatisticVoucher = function () {

        $scope.ClearData();
        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.StatisticVoucher.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.StatisticVoucher.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.StatisticVoucher.DateToDet)
            dateTo = new Date(($filter('date')($scope.StatisticVoucher.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            VoucherType: $scope.StatisticVoucher.VoucherId,
            isPost: $scope.StatisticVoucher.IsPost,
            branchId: $scope.StatisticVoucher.BranchId
        };
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetAllStatisticVoucher",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "StatisticVoucher.xlsx");
            }
