...
app.controller("CostCategoriesDetailsController", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
 var PrintPreviewAs = 1;
  const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'CostCategoriesDetails.csv',


            sheetName: 'CostCategoriesDetails'
        };
  ...
  $scope.GenConfig = {};
        ...
        $scope.VoucherSearchOptions = [
            ...
            { text: 'CostCategories Name', value: 'CostCategoriesName', dataType: 'Text' },
            { text: 'Ledger Name', value: 'LedgerName', dataType: 'Text' },
        ...
        { text: 'Closing Dr', value: 'Closing', dataType: 'Number' },
        { text: 'Closing Cr', value: 'Closing', dataType: 'Number' },
        ...
        var columnDefs = [
            { headerName: "CostCenter Name", width: 250, field: "CostCenterName",cellStyle:{'text-align':'left'} },
            { headerName: "CostCategories Name", width: 250, field: "CostCategoriesName",cellStyle:{'text-align':'left'} },
            { headerName: "Ledger Name", width: 250, field: "LedgerName",cellStyle:{'text-align':'left'} },
            { headerName: "Opening Dr", width: 250, field: "OpeningDr" ,cellStyle:{'text-align':'right'}},
            { headerName: "Opening Cr", width: 250, field: "OpeningCr",cellStyle:{'text-align':'right'} },
            { headerName: "Transaction Dr", width: 250, field: "DrAmt",cellStyle:{'text-align':'right'} },
            { headerName: "Transaction Cr", width: 250, field: "CrAmt",cellStyle:{'text-align':'right'} },
            { headerName: "Closing Balance", width: 250, field: "Closing",cellStyle:{'text-align':'right'} },
            {
        ...
        $scope.gridOptions = {
            ...
            onFilterChanged: function () {
                ...
                $scope.gridOptions.api.forEachNodeAfterFilterAndSort(function (node) {
                    if (node.data.DrAmt == 1 || node.data.DrAmt == 'DR')
                        Opening += node.data.DrAmt;
                    else
                        Opening -= node.data.DrAmt;
                });
                ...
                $scope.dataForBottomGrid[0].DrAmt = Opening;
                $scope.dataForBottomGrid[0].CrAmt = drcr;
...
$timeout(function () {
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    ...
                    if (res.data.IsSuccess) {
                        $scope.GetCostCategoriesDetails();
                    } else {
                        Swal.fire(res.data.ResponseMSG);
                    }
    ...
    $scope.GetCostCategoriesDetails = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.CostCategoriesDetails.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.CostCategoriesDetails.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.CostCategoriesDetails.DateToDet)
            dateTo = new Date(($filter('date')($scope.CostCategoriesDetails.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        $http({
            method: "POST",
            url: base_url + "Account/Reporting/GetAllCostCategoriesDetails",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "CostCategoriesDetails.xlsx");
            }
