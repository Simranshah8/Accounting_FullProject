...
app.controller("BillsReceivable", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
 ...
 var PrintPreviewAs = 1;
  const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'BillsReceivable.csv',
            sheetName: 'BillsReceivable'
        };
...
$timeout(function () {
    ...
    $scope.GetBillsReceivable = function () {

        $scope.ClearData();
        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.BillsReceivable.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.BillsReceivable.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.BillsReceivable.DateToDet)
            dateTo = new Date(($filter('date')($scope.BillsReceivable.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            IsOnlyPending: $scope.BillsReceivable.IsOnlyPending
        };
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetBillsReceivable",
...
$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "BillsReceivable.xlsx");
            }
