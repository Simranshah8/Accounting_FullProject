...
app.controller("CashAndBankBookLedgerWise", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
 ...
 var PrintPreviewAs = 1;
  const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'CashAndBankBookLedgerWise.csv',
            sheetName: 'CashAndBankBookLedgerWise'
        };
    ...
    $scope.GenConfig = {};
        ...
        $scope.VoucherSearchOptions = [
            ...
            { text: 'LedgerGroup', value: 'LedgerGroupName', dataType: 'text' },
        ...
        $scope.CashAndBankBookLedgerWise = {
        ...
        var columnDefs = [
            { headerName: "LedgerName", width: 250, field: "LedgerName" },
            { headerName: "LedgerGroup", width: 250, field: "LedgerGroupName" },
            { headerName: "Opening Dr", width: 250, field: "OpeningDr" },
            { headerName: "Opening Cr", width: 250, field: "OpeningCr" },
            { headerName: "Transaction Dr", width: 250, field: "TransactionDr" },
            { headerName: "Transaction Cr", width: 250, field: "TransactionCr" },
            { headerName: "Closing Dr", width: 250, field: "ClosingDr" },
            { headerName: "Closing Cr", width: 250, field: "ClosingCr" },
            {
        ...
        $scope.gridOptions = {
            ...
            onFilterChanged: function () {
                ...
                $scope.gridOptions.api.forEachNodeAfterFilterAndSort(function (node) {
                    if (node.data.OpeningDr == 1 || node.data.OpeningDr == 'DR')
                        Opening += node.data.OpeningDr;
                    else
                        Opening -= node.data.OpeningDr;
                });
                ...
                $scope.dataForBottomGrid[0].OpeningDr = Opening;
                $scope.dataForBottomGrid[0].ClosingDr = drcr;
...
$timeout(function () {
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    ...
                    if (res.data.IsSuccess) {
                        $scope.GetCashAndBankBookLedgerWise();
                    } else {
                        Swal.fire(res.data.ResponseMSG);
                    }
    ...
    $scope.GetCashAndBankBookLedgerWise = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.CashAndBankBookLedgerWise.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.CashAndBankBookLedgerWise.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.CashAndBankBookLedgerWise.DateToDet)
            dateTo = new Date(($filter('date')($scope.CashAndBankBookLedgerWise.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            VoucherType: $scope.CashAndBankBookLedgerWise.VoucherId,
            isPost: $scope.CashAndBankBookLedgerWise.IsPost,
            branchId: $scope.CashAndBankBookLedgerWise.BranchId
        };
        ...
        $http({
            method: "POST",
            url: base_url + "Account/Reporting/GetAllCashAndBankBookLedgerWise",
            ...
            if (res.data.IsSuccess && res.data.Data) {
                ...
                angular.forEach($scope.DataColl, function (dc) {
                    if (dc.OpeningDr == 1 || dc.OpeningDr == 'DR')
                        Opening += dc.OpeningDr;
                    else
                        Opening -= dc.OpeningDr;
                });
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "CashAndBankBookLedgerWise.xlsx");
            }
