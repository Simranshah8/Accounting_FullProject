...
app.controller("CostCategoriesSummary", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
 var PrintPreviewAs = 1;
  const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'CostCategoriesSummary.csv',


            sheetName: 'CostCategoriesSummary'
        };
    ...
    function LoadData() {
        ...
        { text: 'TranDr', value: 'TranDr', dataType: 'Number' },
        { text: 'TranCr', value: 'TranCr', dataType: 'Number' },
        ...
        $scope.CostCategoriesSummary = {
            CostCategoriesId: null,
        ...
        $scope.columnDefs = [
            { headerName: "Particulars", width: 300, field: "CostCenterName", dataType: 'Text',cellStyle:{'text-align':'left'} },
            { headerName: "Opening Amount", width: 200, field: "Opening", dataType: 'Number',cellStyle:{'text-align':'right'} },
            { headerName: "Transaction Dr", width: 200, field: "TranDr", dataType: 'Number', dataType: 'Number',cellStyle:{'text-align':'right'} },
            { headerName: "Transaction Cr", width: 200, field: "TranCr", dataType: 'Number',cellStyle:{'text-align':'right'} },
            { headerName: "Closing Balance", width: 200, field: "Closing", dataType: 'Number',cellStyle:{'text-align':'right'} },
            //{
            //    headerName: "Action", width: 200, cellRenderer:
            //        function (params) {
        ...
        $scope.gridOptions = {
            ...
            onFilterChanged: function () {
                ...
                $scope.gridOptions.api.forEachNodeAfterFilterAndSort(function (node) {
                    if (node.data.TranDr == 1 || node.data.TranDr == 'DR')
                        Opening += node.data.TranDr;
                    else
                        Opening -= node.data.TranDr;
                });
                ...
                $scope.dataForBottomGrid[0].TranDr = Opening;
                $scope.dataForBottomGrid[0].TranCr = drcr;
...
$timeout(function () {
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    ...
                    if (res.data.IsSuccess) {
                        $scope.GetCostCategoriesSummary();
                    } else {
                        Swal.fire(res.data.ResponseMSG);
                    }
    ...
    $scope.GetCostCategoriesSummary = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.CostCategoriesSummary.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.CostCategoriesSummary.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.CostCategoriesSummary.DateToDet)
            dateTo = new Date(($filter('date')($scope.CostCategoriesSummary.DateToDet.dateAD, 'yyyy-MM-dd')));

        $scope.DataColl = []; //declare an empty array
        $scope.gridOptions.api.setRowData($scope.DataColl);

        var beData = {
            CostCategoriesId: $scope.CostCategoriesSummary.CostCategoriesId,
            DateFrom: dateFrom,
            DateTo: dateTo,
        };
        ...
        $http({
            method: "POST",
            url: base_url + "Account/Reporting/GetCostCategoriesSummary",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "CostCategoriesSummary.xlsx");
            }
