...
app.controller("DayBookSummaryOnly", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'DayBookSummaryOnly.csv',
            sheetName: 'DayBookSummaryOnly'
        };
 ...
 $scope.GenConfig = {};
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.DayBookSummaryOnly.DateFrom_TMP = new Date(comDet.StartDate);
                }
            }, function (errormessage) {
                alert('Unable to Delete data. pls try again.' + errormessage.responseText);
            });
        });
        $scope.DayBookSummaryOnly = {
        ...
        var columnDefs = [
            ...
            {
                headerName: "Date(B.S.)", width: 140, field: "VoucherDateStrNP", cellRenderer: 'agGroupCellRenderer',
            ...
            { headerName: "Particulars", width: 180, field: "PartyLedger",cellStyle:{'text-align':'right'} },
            { headerName: "VoucherType", width: 180, field: "VoucherType",cellStyle:{'text-align':'left'} },
            { headerName: "VoucherNo", width: 180, field: "AutoVoucherNo",cellStyle:{'text-align':'center'} },
            { headerName: "RefNo", width: 180, field: "RefNumber",cellStyle:{'text-align':'center'} },
            { headerName: "Amount", width: 180, field: "DrAmount",cellStyle:{'text-align':'right'} },
            
            { headerName: "CostClass", width: 180, field: "CostClassName",cellStyle:{'text-align':'left'} },
            { headerName: "User", width: 180, field: "Username",cellStyle:{'text-align':'left'} },
        ...
        $scope.gridOptions = {
            ...
            onFilterChanged: function () {
                ...
                $scope.dataForBottomGrid[0].Amount = drcr;
    ...
    $scope.GetDayBookSummaryOnly = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.DayBookSummaryOnly.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.DayBookSummaryOnly.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.DayBookSummaryOnly.DateToDet)
            dateTo = new Date(($filter('date')($scope.DayBookSummaryOnly.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            ...
            DateFrom: dateFrom,
            DateTo: dateTo,
            VoucherType: $scope.DayBookSummaryOnly.VoucherType,
            isPending: $scope.DayBookSummaryOnly.isPending,
        };
        ...
        $http({
            method: 'POST',
            url: base_url + "Account/Reporting/GetDayBookSummary",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "DayBookSummaryOnly.xlsx");
            }
