...
app.controller("StatisticVoucherDaily", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'StatisticVoucherDaily.csv',
            sheetName: 'StatisticVoucherDaily'
        };
 ...
 $scope.GenConfig = {};
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.StatisticVoucherDaily.DateFrom_TMP = new Date(comDet.StartDate);
                }
            }, function (errormessage) {
                alert('Unable to Delete data. pls try again.' + errormessage.responseText);
            });
        });
        $scope.StatisticVoucherDaily = {
            VoucherId: 0,
            VoucherType: 0,
...
$timeout(function () {
    ...
    $scope.GetStatisticVoucherDaily = function () {

        $scope.ClearData();
        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.StatisticVoucherDaily.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.StatisticVoucherDaily.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.StatisticVoucherDaily.DateToDet)
            dateTo = new Date(($filter('date')($scope.StatisticVoucherDaily.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            VoucherId: 0,
            VoucherType: $scope.StatisticVoucherDaily.VoucherId,
            DateFrom: dateFrom,
            DateTo: dateTo,
            IsNepaliDate: $scope.StatisticVoucherDaily.IsNepaliDate
        };
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetAllStatisticVoucherDaily",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "StatisticVoucherDaily.xlsx");
            }
