...
app.controller("LedgerVoucherAsList", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'LedgerVoucherAsList.csv',
            sheetName: 'LedgerVoucherAsList'
        };
 ...
 $scope.GenConfig = {};
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.LedgerVoucherAsList.DateFrom_TMP = new Date(comDet.StartDate);
                }
        ...
        var columnDefs = [
            ...
            {
                headerName: "Date(B.S.)", width: 140, field: "VoucherDateStr", cellRenderer: 'agGroupCellRenderer',
            ...
            { headerName: "Particulars", width: 150, field: "Particulars" ,cellStyle: { 'text-align': 'left' }},
            { headerName: "AlternateParticulars", width: 200, field: "AlternateParticulars",cellStyle: { 'text-align': 'left' } },
            { headerName: "VoucherType", width: 150, field: "VoucherType",cellStyle: { 'text-align': 'left' } },
            { headerName: "VoucherNo", width: 150, field: "AutoVoucherNo",cellStyle: { 'text-align': 'center' } },
            { headerName: "RefNo", width: 150, field: "RefNo",cellStyle: { 'text-align': 'center' } },
            { headerName: "Opening", width: 180, field: "IsOpening",cellStyle: { 'text-align': 'right' } },
            { headerName: "Closing", width: 180, field: "CurrentClosing",cellStyle: { 'text-align': 'right' } },
            { headerName: "CrAmt", width: 180, field: "CreditAmt",cellStyle: { 'text-align': 'right' } },
            { headerName: "DrAmt", width: 180, field: "DebitAmt" ,cellStyle: { 'text-align': 'right' }},
            {
        ...
        $scope.gridOptions = {
            ...
            onFilterChanged: function () {
                ...
                $scope.gridOptions.api.forEachNodeAfterFilterAndSort(function (node) {
                    if (node.data.DebitAmt == 1 || node.data.DebitAmt == 'DR')
                        Opening += node.data.DebitAmt;
                    else
                        Opening -= node.data.DebitAmt;
                });
                ...
                $scope.dataForBottomGrid[0].DebitAmt = Opening;
                $scope.dataForBottomGrid[0].CreditAmt = drcr;
...
$timeout(function () {
    ...
    $scope.GetLedgerVoucherAsList = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.LedgerVoucherAsList.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.LedgerVoucherAsList.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.LedgerVoucherAsList.DateToDet)
            dateTo = new Date(($filter('date')($scope.LedgerVoucherAsList.DateToDet.dateAD, 'yyyy-MM-dd')));

        $scope.DataColl = []; //declare an empty array
        $scope.gridOptions.api.setRowData($scope.DataColl);

        var beData = {
            LedgerId: $scope.LedgerVoucherAsList.LedgerId,
            DateFrom: dateFrom,
            DateTo: dateTo,
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetLedgerVoucherAsList",
            ...
            if (res.data.IsSuccess && res.data.Data) {
                ...
                angular.forEach($scope.DataColl, function (dc) {
                    if (dc.DebitAmt == 1 || dc.DebitAmt == 'DR')
                        Opening += dc.DebitAmt;
                    else
                        Opening -= dc.DebitAmt;
                });
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "LedgerVoucherAsList.xlsx");
            }
