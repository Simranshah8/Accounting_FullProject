...
app.controller("SalesmanWiseLedgerVoucher", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    LoadData();
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'SalesmanWiseLedgerVoucher.csv',
            sheetName: 'SalesmanWiseLedgerVoucher'
        };
    ...
    function LoadData() {
        $scope.SalesmanWiseLedgerVoucher = {
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.SalesmanWiseLedgerVoucher.DateFrom_TMP = new Date(comDet.StartDate);
                }
        ...
        $scope.columnDefs = [
            ...
            {
                headerName: "Date(B.S.)", width: 250, field: "NVoucherDate", cellRenderer: 'agGroupCellRenderer',
            ...
            { headerName: "Voucher No.", width: 250, field: "VoucherNo", filter: 'agTextColumnFilter', cellStyle: { 'text-align': 'right' } },
            { headerName: "RefNo", width: 250, field: "RefNo", filter: 'agTextColumnFilter', cellStyle: { 'text-align': 'right' } },

            { headerName: "DebitAmt", width: 250, field: "DebitAmt", filter: 'agTextColumnFilter', cellStyle: { 'text-align': 'right' } },

            { headerName: "CreditAmt", width: 250, field: "CreditAmt", filter: 'agTextColumnFilter', cellStyle: { 'text-align': 'right' } },
...
$timeout(function () {
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    ...
                    if (res.data.IsSuccess) {
                        $scope.GetSalesmanWiseLedgerVoucher();
                    } else {
    ...
    $scope.ClearData = function () {
        ...
        $scope.SalesmanWiseLedgerVoucher.ODr = 0;
        $scope.SalesmanWiseLedgerVoucher.OCr = 0;
        $scope.SalesmanWiseLedgerVoucher.TDr = 0;
        $scope.SalesmanWiseLedgerVoucher.TCr = 0;
        $scope.SalesmanWiseLedgerVoucher.CDr = 0;
        $scope.SalesmanWiseLedgerVoucher.CCr = 0;
    ...
    $scope.GetSalesmanWiseLedgerVoucher = function () {

        $scope.ClearData();

        if (!$scope.SalesmanWiseLedgerVoucher.AgentId)
            return;

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.SalesmanWiseLedgerVoucher.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.SalesmanWiseLedgerVoucher.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.SalesmanWiseLedgerVoucher.DateToDet)
            dateTo = new Date(($filter('date')($scope.SalesmanWiseLedgerVoucher.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            AgentId: $scope.SalesmanWiseLedgerVoucher.AgentId
        };
        ...
        $http({
            ...
            $scope.SalesmanWiseLedgerVoucher.ODr = (openingAmt > 0 ? openingAmt : 0);
            $scope.SalesmanWiseLedgerVoucher.OCr = (openingAmt < 0 ? Math.abs(openingAmt) : 0);
            $scope.SalesmanWiseLedgerVoucher.TDr = drAmt;
            $scope.SalesmanWiseLedgerVoucher.TCr = crAmt;
            $scope.SalesmanWiseLedgerVoucher.CDr = (closingAmt > 0 ? closingAmt : 0);
            $scope.SalesmanWiseLedgerVoucher.CCr = (closingAmt < 0 ? Math.abs(closingAmt) : 0);
                                ...
                                return new Promise((resolve) => {
                                    if (value >= 0) {
                                        ...
                                        if (rptTranId > 0) {
                                            ...
                                            $http({
                                                method: 'POST',
                                                url: base_url + "Account/Reporting/PrintSalesmanWiseLedgerVoucher",
                                                ...
                                                if (res.data.IsSuccess && res.data.Data) {
                                                    ...
                                                    var rptPara = {
                                                        ...
                                                        Period: $scope.SalesmanWiseLedgerVoucher.DateFromDet.dateBS + " TO " + $scope.SalesmanWiseLedgerVoucher.DateToDet.dateBS,
                                                        ODr: $scope.SalesmanWiseLedgerVoucher.ODr,
                                                        OCr: $scope.SalesmanWiseLedgerVoucher.OCr,
                                                        TDr: $scope.SalesmanWiseLedgerVoucher.TDr,
                                                        TCr: $scope.SalesmanWiseLedgerVoucher.TCr,
                                                        CDr: $scope.SalesmanWiseLedgerVoucher.CDr,
                                                        CCr: $scope.SalesmanWiseLedgerVoucher.CCr
                                                    };
    ...
    $scope.GetDataForPrint = function () {
        ...
        RptParamentersColl.push({
            ...
            {
                Name: 'Ledger',
                Value: $scope.SalesmanWiseLedgerVoucher.LedgerDetails.Name
            },
            {
                Name: 'Address',
                Value: ($scope.SalesmanWiseLedgerVoucher.LedgerDetails.Address ? $scope.SalesmanWiseLedgerVoucher.LedgerDetails.Address : '')
            },
            {
                Name: 'MobileNo',
                Value: ($scope.SalesmanWiseLedgerVoucher.LedgerDetails.MobileNo1 ? $scope.SalesmanWiseLedgerVoucher.LedgerDetails.MobileNo1 : '')
            },
            {
                Name: 'PanVatNo',
                Value: ($scope.SalesmanWiseLedgerVoucher.LedgerDetails.PanVat ? $scope.SalesmanWiseLedgerVoucher.LedgerDetails.PanVat : '')
            },
            {
                Name: 'TelNo',
                Value: ($scope.SalesmanWiseLedgerVoucher.LedgerDetails.MobileNo2 ? $scope.SalesmanWiseLedgerVoucher.LedgerDetails.MobileNo2 : '')
            },
            {
                Name: 'EmailId',
                Value: ($scope.SalesmanWiseLedgerVoucher.LedgerDetails.EmailId ? $scope.SalesmanWiseLedgerVoucher.LedgerDetails.EmailId : '')
            },
            {
                Name: 'ODr',
...
$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "SalesmanWiseLedgerVoucher.xlsx");
            }
