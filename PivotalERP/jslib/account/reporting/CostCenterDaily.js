...
app.controller("CostCenterDailyController", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'CostCenterDaily.csv',


            sheetName: 'CostCenterDaily'
        };
    ...
    function LoadData() {
        ...
        var columnDefs = [
            { headerName: "Particulars", width: 250, field: "LedgerName", pinned: 'left',cellStyle:{'text-align':'text'} },
            { headerName: "Opening Amount", width: 200, field: "Opening", cellStyle: { 'text-align': 'right' }, filter: "agNumberColumnFilter", valueFormatter: function (params) { return Numberformat(params.value); }, },
            { headerName: "Transaction Dr", width: 200, field: "DrAmount", cellStyle: { 'text-align': 'right' }, filter: "agNumberColumnFilter", valueFormatter: function (params) { return Numberformat(params.value); }, },
            { headerName: "Transaction Cr", width: 200, field: "CrAmount", cellStyle: { 'text-align': 'right' }, filter: "agNumberColumnFilter", valueFormatter: function (params) { return Numberformat(params.value); }, },
            { headerName: "Closing Balance", width: 200, field: "Closing", cellStyle: { 'text-align': 'right' }, filter: "agNumberColumnFilter", valueFormatter: function (params) { return Numberformat(params.value); }, },
...
$timeout(function () {
    ...
    $scope.GetCostCenterDaily = function () {
        $scope.ClearData();
        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.CostCenterDaily.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.CostCenterDaily.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.CostCenterDaily.DateToDet)
            dateTo = new Date(($filter('date')($scope.CostCenterDaily.DateToDet.dateAD, 'yyyy-MM-dd')));

        $scope.DataColl = []; //declare an empty array
        $scope.gridOptions.api.setRowData($scope.DataColl);

        var beData = {
            CostCenterId: $scope.CostCenterDaily.CostCenterId,
            DateFrom: dateFrom,
            DateTo: dateTo,
        };
        ...
        $http({
            method: "POST",
            url: base_url + "Account/Reporting/GetAllCostCenterDaily",
   ...
   $scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "CostCenterDaily.xlsx");
            }
