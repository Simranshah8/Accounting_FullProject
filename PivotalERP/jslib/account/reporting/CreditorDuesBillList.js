...
app.controller("CreditorDuesBillList", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'CreditorDuesBillList.csv',
            sheetName: 'CreditorDuesBillList'
        };
 ...
 $scope.GenConfig = {};
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.CreditorDuesBillList.DateFrom_TMP = new Date(comDet.StartDate);
                }
        ...
        var columnDefs = [
            ...
            { headerName: "Party", width: 180, field: "Party",cellStyle:{'text-align':'left'} },
            { headerName: "MobileNo", width: 180, field: "MobileNo",cellStyle:{'text-align':'center'} },
            { headerName: "EmailId", width: 180, field: "EmailId",cellStyle:{'text-align':'left'} },
            { headerName: "PanVat", width: 180, field: "PanVat",cellStyle:{'text-align':'center'} },
            { headerName: "GroupName", width: 180, field: "GroupName",cellStyle:{'text-align':'left'} },
            { headerName: "ParentGroupName", width: 180, field: "ParentGroupName",cellStyle:{'text-align':'left'} },
            ...
            { headerName: "InvoiceNo", width: 180, field: "InvoiceNo" ,cellStyle:{'text-align':'center'}},
...
$timeout(function () {
    ...
    $scope.GetCreditorDuesBillList = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.CreditorDuesBillList.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.CreditorDuesBillList.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.CreditorDuesBillList.DateToDet)
            dateTo = new Date(($filter('date')($scope.CreditorDuesBillList.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            ...
            DateFrom: dateFrom,
            DateTo: dateTo,
            LedgerGroupId: $scope.CreditorDuesBillList.LedgerGroupId,
            SalesLedgerId: $scope.CreditorDuesBillList.SalesLedgerId,
            ledgerIdColl: $scope.CreditorDuesBillList.ledgerIdColl,
        };
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "CreditorDuesBillList.xlsx");
            }
