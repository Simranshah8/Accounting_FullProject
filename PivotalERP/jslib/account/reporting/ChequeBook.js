...
app.controller("ChequeBook", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
 var PrintPreviewAs = 1;
  const contextMenu = GlobalServices.createElementForMenu();
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'ChequeBook.csv',
            sheetName: 'ChequeBook'
        };
  ...
  $scope.GenConfig = {};
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.ChequeBook.DateFrom_TMP = new Date(comDet.StartDate);
                }
        ...
        var columnDefs = [
            ...
            {
                headerName: "Date(B.S.)", width: 250, field: "NVoucherDate", cellRenderer: 'agGroupCellRenderer',cellStyle:{'text-align':'center'},
            ...
            { headerName: "Particulars", width: 250, field: "Particulars",cellStyle:{'text-align':'right'} },

            { headerName: "ChequeNo", width: 250, field: "DrAChequeNomount",cellStyle:{'text-align':'center'} },
            { headerName: "ChequeDate", width: 250, field: "ChequeDate",cellStyle:{'text-align':'center'} },
            { headerName: "Remarks", width: 250, field: "ChequeRemarks",cellStyle:{'text-align':'left'} },
            ...
            { headerName: "DebitAmount", width: 250, field: "DrAmount",cellStyle:{'text-align':'right'} },
            { headerName: "CreditAmount", width: 250, field: "CrAmount",cellStyle:{'text-align':'right'} },
...
$timeout(function () {
    ...
    $scope.GetChequeBook = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.ChequeBook.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.ChequeBook.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.ChequeBook.DateToDet)
            dateTo = new Date(($filter('date')($scope.ChequeBook.DateToDet.dateAD, 'yyyy-MM-dd')));

        $scope.DataColl = []; //declare an empty array
        $scope.gridOptions.api.setRowData($scope.DataColl);

        var beData = {
            LedgerId: $scope.ChequeBook.LedgerId,
            DateFrom: dateFrom,
            DateTo: dateTo,
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetChequeBook",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "ChequeBook.xlsx");
            }
