app.controller("LedgerGroupDayBook", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
var PrintPreviewAs = 1;
 const contextMenu = GlobalServices.createElementForMenu();
    LoadData();
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'LedgerGroupDayBook.csv',
            sheetName: 'LedgerGroupDayBook'
        };
    ...
    function LoadData() {
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.LedgerGroupDayBook.DateFrom_TMP = new Date(comDet.StartDate);
                }
        ...
        $scope.columnDefs = [
            {
                headerName: "Date(A.D.)", width: 250, field: "VoucherDate", cellRenderer: 'agGroupCellRenderer',cellStyle: { 'text-align': 'center' },
            ...
            {
                headerName: "Date(B.S.)", width: 250, field: "VoucherDateBS", cellRenderer: 'agGroupCellRenderer',cellStyle: { 'text-align': 'center' },
            ...
            { headerName: "Debit", width: 250, field: "DrAmt", filter: 'agTextColumnFilter', cellStyle: { 'text-align': 'right' }},
            { headerName: "Credit", width: 250, field: "CrAmt", filter: 'agTextColumnFilter', cellStyle: { 'text-align': 'right' }},
...
$timeout(function () {
    ...
    $scope.ClearData = function () {
        ...
        $scope.LedgerGroupDayBook.ODr = 0;
        $scope.LedgerGroupDayBook.OCr = 0;
        $scope.LedgerGroupDayBook.TDr = 0;
        $scope.LedgerGroupDayBook.TCr = 0;
        $scope.LedgerGroupDayBook.CDr = 0;
        $scope.LedgerGroupDayBook.CCr = 0;
    ...
    $scope.GetLedgerGroupDayBook = function () {

        $scope.ClearData();

        if (!$scope.LedgerGroupDayBook.LedgerGroupId)
            return;

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.LedgerGroupDayBook.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.LedgerGroupDayBook.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.LedgerGroupDayBook.DateToDet)
            dateTo = new Date(($filter('date')($scope.LedgerGroupDayBook.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            LedgerGroupId: $scope.LedgerGroupDayBook.LedgerGroupId
        };
        ...
        $http({
            method: "post",
            url: base_url + "Account/Reporting/GetLedgerGroupDayBook",
            ...
            $scope.LedgerGroupDayBook.ODr = (openingAmt > 0 ? openingAmt : 0);
            $scope.LedgerGroupDayBook.OCr = (openingAmt < 0 ? Math.abs(openingAmt) : 0);
            $scope.LedgerGroupDayBook.TDr = drAmt;
            $scope.LedgerGroupDayBook.TCr = crAmt;
            $scope.LedgerGroupDayBook.CDr = (closingAmt > 0 ? closingAmt : 0);
            $scope.LedgerGroupDayBook.CCr = (closingAmt < 0 ? Math.abs(closingAmt) : 0);
                                ...
                                return new Promise((resolve) => {
                                    if (value >= 0) {
                                        ...
                                        if (rptTranId > 0) {
                                            ...
                                            $http({
                                                method: 'POST',
                                                url: base_url + "Account/Reporting/PrintLedgerGroupDayBook",
                                                ...
                                                if (res.data.IsSuccess && res.data.Data) {
                                                    ...
                                                    var rptPara = {
                                                        ...
                                                        Period: $scope.LedgerGroupDayBook.DateFromDet.dateBS + " TO " + $scope.LedgerGroupDayBook.DateToDet.dateBS,
                                                        ODr: $scope.LedgerGroupDayBook.ODr,
                                                        OCr: $scope.LedgerGroupDayBook.OCr,
                                                        TDr: $scope.LedgerGroupDayBook.TDr,
                                                        TCr: $scope.LedgerGroupDayBook.TCr,
                                                        CDr: $scope.LedgerGroupDayBook.CDr,
                                                        CCr: $scope.LedgerGroupDayBook.CCr
                                                    };
    ...
    $scope.GetDataForPrint = function () {
        ...
        RptParamentersColl.push({
            ...
            {
                Name: 'Ledger',
                Value: $scope.LedgerGroupDayBook.LedgerDetails.Name
            },
            {
                Name: 'Address',
                Value: ($scope.LedgerGroupDayBook.LedgerDetails.Address ? $scope.LedgerGroupDayBook.LedgerDetails.Address : '')
            },
            {
                Name: 'MobileNo',
                Value: ($scope.LedgerGroupDayBook.LedgerDetails.MobileNo1 ? $scope.LedgerGroupDayBook.LedgerDetails.MobileNo1 : '')
            },
            {
                Name: 'PanVatNo',
                Value: ($scope.LedgerGroupDayBook.LedgerDetails.PanVat ? $scope.LedgerGroupDayBook.LedgerDetails.PanVat : '')
            },
            {
                Name: 'TelNo',
                Value: ($scope.LedgerGroupDayBook.LedgerDetails.MobileNo2 ? $scope.LedgerGroupDayBook.LedgerDetails.MobileNo2 : '')
            },
            {
                Name: 'EmailId',
                Value: ($scope.LedgerGroupDayBook.LedgerDetails.EmailId ? $scope.LedgerGroupDayBook.LedgerDetails.EmailId : '')
            },
            {
                Name: 'ODr',
...
$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "LedgerGroupDayBook.xlsx");
            }
