app.controller('OutgoingPaymentController',  function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    ...
    function LoadData() {
        ...
        $scope.ForCustomerVendor = [{ id: 1, text: 'Customer' }, { id: 2, text: 'Vendor' }];
        ...
        if (VoucherType) {
            ...
            $http({
                ...
                if (res1.data.IsSuccess && res1.data.Data) {
                    ...
                    $scope.AllCostCenterColl_Qry = mx(res1.data.Data);
                }
            }, function (reason) {
                Swal.fire('Failed' + reason);
            });
 ...
 <div class="input-group-prepend">
                        ...
                        $scope.$apply(function () {
                            if ($scope.SelectedVoucher) {
                                ...
                                if ($scope.SelectedVoucher.LedgerId > 0)
                                    $scope.beData.PartyLedgerId = $scope.SelectedVoucher.LedgerId;
    ...
    $scope.ChangeSelection = function (curRow) {
        if (curRow.IsSelected == true) {
            curRow.PaidAmt = curRow.Amount;
        } else {
            curRow.PaidAmt = 0;
        }

        $scope.ChangeBillWiseAmt('paidAmt');
    }
    $scope.CurBillRefColl = [];
    var lastPartyId = null;

    $scope.ChangePartyLedger = function (ledDet) {

        if (ledDet == null || ledDet.PartyLedgerId == null || ledDet.PartyLedgerId == undefined || lastPartyId != ledDet.PartyLedgerId) {
            $scope.CurBillRefColl = [];
            lastPartyId = ledDet.PartyLedgerId;
        }
        ...
        $timeout(function () {
            ...
            if (ledDet) {
                ledDet.CurrentBal = 0;
                if (ledDet.PartyLedgerId && ledDet.PartyLedgerId > 0 && ledDet.PartyLedgerDetails) {
                    ledDet.CurrentBal = ledDet.PartyLedgerDetails.Closing;

                    if (ledDet.PartyLedgerDetails.CostCentersAreApplied == true) {
        ...
        $timeout(function () {
            if (ledDet) {
                ...
                if (ledDet.PartyLedgerId && ledDet.PartyLedgerId > 0 && ledDet.PartyLedgerDetails) {
                    ...
                    if (!$scope.CurBillRefColl || $scope.CurBillRefColl.length == 0) {

                        var forAll = ($scope.beData.DisplayAllTransaction == true ? true : false);
                        $http({
                            method: 'GET',
                            url: base_url + "Inventory/Transaction/getPurchaseDuesBillList?ledgerId=" + ledDet.PartyLedgerId + '&branchId=' + $scope.SelectedVoucher.BDId + "&billwise=" + (ledDet.BillWise ? ledDet.BillWise : false) + "&forAll=" + forAll,
                            dataType: "json"
                        }).then(function (res) {
                            if (res.data.IsSuccess && res.data.Data) {
                                $scope.CurBillRefColl = res.data.Data;
                            }
                        }, function (reason) {
                            Swal.fire('Failed' + reason);
                        });
        ...
        $timeout(function () {
            if (ledDet.PartyLedgerDetails && ledDet.PartyLedgerDetails.UDFColl && ledDet.PartyLedgerDetails.UDFColl.length > 0) {
                ...
                if (ledDet.LedgerUDFColl.length == 0) {
                    ...
                    var ledUdfColl = [];
                    angular.forEach(ledDet.PartyLedgerDetails.UDFColl, function (udf) {
                        var ud = {
                            SNo: udf.SNo,
                            Name: udf.Label,
                            Value: udf.DefaultValue,
    ...
    $scope.ChangeCashBankLedger = function (ledDet) {
        ...
        if (ledDet == null || ledDet.CashBankLedgerId == null || ledDet.CashBankLedgerId == undefined || lastPartyId != ledDet.CashBankLedgerId) {
            $scope.CurBillRefColl = [];
            lastPartyId = ledDet.CashBankLedgerId;
        }
    ...
    $scope.getDuesBillForRec = function (ledDet) {
        ...
        if (!ledDet || !ledDet.PartyLedgerId)
            return;
        ...
        $http({
            method: 'GET',
            //url: base_url + "Inventory/Transaction/getPurchaseDuesBillList?ledgerId=" + ledDet.LedgerId + '&branchId=' + $scope.SelectedVoucher.BDId + "&billwise=" + ledDet.LedgerDetails.BillWiseDues +"&forAll="+forAll,
            url: base_url + "Inventory/Transaction/getPurchaseDuesBillList?ledgerId=" + ledDet.PartyLedgerId + '&branchId=' + $scope.SelectedVoucher.BDId + "&billwise=" + (ledDet.BillWise ? ledDet.BillWise : false) + "&forAll=" + forAll,
            dataType: "json"
        }).then(function (res) {
            hidePleaseWait();
        ...
        if ($scope.CurLedgerAllocation.CostCenterColl) {
            if ($scope.CurLedgerAllocation.CostCenterColl.length > ind + 1) {
                $scope.CurLedgerAllocation.CostCenterColl.splice(ind + 1, 0, {
                    ...
                    DrAmount: 0,
                    CrAmount: 0,
                    DrCr: $scope.CurLedgerAllocation.DrCr,
                })
            } else {
    ...
    $scope.GetData = function () {
        ...
        var tmpPayment = {
            ...
            ForCV: $scope.beData.ForCV,
            ProjectId: $scope.beData.ProjectId,
            IECostCenterId: $scope.beData.IECostCenterId,
            IELedgerId: null,
            VoucherType: $scope.SelectedVoucher.VoucherType,
            CashBankLedgerId: $scope.beData.CashBankLedgerId,
        };

        if ($scope.beData.IECostCenterId > 0) {
            var findIECC = $scope.AllCostCenterColl_Qry.firstOrDefault(p1 => p1.CostCenterId == $scope.beData.IECostCenterId);
            if (findIECC) {
                tmpPayment.IELedgerId = findIECC.LedgerId;
            }
        }
        ...
        var drLedAllocation = {
            DrCr: 1,
            LedgerId: $scope.beData.PartyLedgerId,
            AgentId: 0,
            LFNO: '',
            Narration: '',
        ...
        if ($scope.CurBillRefColl && $scope.CurBillRefColl.length > 0 && $scope.beData.NoBillRef != true) {
            angular.forEach($scope.CurBillRefColl, function (br) {
                if (br.PaidAmt > 0) {
                    var billRef = {
                        Amount: br.PaidAmt,
                        BillRefType: 2,
        ...
        if ($scope.beData.CheckDetails && $scope.beData.CheckDetails.ChequeDateDet) {
            cbLedAllocation.CheckDetails.ChequeId = $scope.beData.CheckDetails.ChequeId;
            cbLedAllocation.CheckDetails.ChequeNo = $scope.beData.CheckDetails.ChequeNo;
            cbLedAllocation.CheckDetails.AccountNo = $scope.beData.CheckDetails.AccountNo;
            cbLedAllocation.CheckDetails.Remarks = $scope.beData.CheckDetails.Remarks;
            cbLedAllocation.CheckDetails.CheckType = $scope.beData.CheckDetails.CheckType;
            cbLedAllocation.CheckDetails.PartyName = $scope.beData.CheckDetails.PartyName;
            cbLedAllocation.CheckDetails.ChequeDate = $filter('date')(new Date($scope.beData.CheckDetails.ChequeDateDet.dateAD), 'yyyy-MM-dd');
        }

        if ($scope.beData.FundDetails && $scope.beData.FundDetails.TransactionRefNo && $scope.beData.FundDetails.TransactionRefNo.length > 0) {
            cbLedAllocation.FundDetails.FundTransferTypeId = $scope.beData.FundDetails.FundTransferTypeId;
            cbLedAllocation.FundDetails.TransactionRefNo = $scope.beData.FundDetails.TransactionRefNo;
            cbLedAllocation.FundDetails.ToAccount = $scope.beData.FundDetails.ToAccount;
            cbLedAllocation.FundDetails.BeneficiaryName = $scope.beData.FundDetails.BeneficiaryName;
            cbLedAllocation.FundDetails.BeneficiaryBankName = $scope.beData.FundDetails.BeneficiaryBankName;
            cbLedAllocation.FundDetails.Amount = drLedAllocation.CrAmount;
        }
        ...
        angular.forEach($scope.beData.LedgerAllocationColl, function (ledA) {
            if (ledA.LedgerId && ledA.LedgerId > 0) {
                ...
                var crLedAllocation = {
                    ...
                    DrAmount: (ledA.DrCr==1 ? ledA.DrAmount : 0),
                    CrAmount: (ledA.DrCr == 2 ? ledA.CrAmount : 0),
                ...
                if (ledA.CostCenterColl) {
                    angular.forEach(ledA.CostCenterColl, function (cc) {
                        if (cc.CostCenterId && cc.CostCenterId > 0 && (cc.DrAmount != 0 || cc.CrAmount != 0) ) {
                            var ccAllocation = {
                                CostCategoriesId: (cc.CostCenterDetails ? cc.CostCenterDetails.CostCategoryId : cc.CostCategoriesId),
                                CostCenterId: cc.CostCenterId,
        ...
        if (cbLedAllocation.LedgerId > 0)
        {
            var vCrLedAllocation = {
                ...
                Narration: '',
                DrAmount: 0,
                CrAmount: mx($scope.beData.LedgerAllocationColl).sum(p1 => p1.DrAmount - p1.CrAmount),
                ForBranchId: $scope.beData.ForBranchId,
                Dimension1: null,
                Dimension2: null,
                Dimension3: null,
            ...
            tmpPayment.LedgerAllocationColl.push(vCrLedAllocation);

            drLedAllocation.DrAmount = vCrLedAllocation.CrAmount;
            drLedAllocation.CrAmount = 0;            
            tmpPayment.LedgerAllocationColl.push(drLedAllocation);

            cbLedAllocation.CrAmount = vCrLedAllocation.CrAmount;
            cbLedAllocation.DrAmount = 0;
            tmpPayment.LedgerAllocationColl.push(cbLedAllocation);
        }  
   ...
   //     });
        ...
        if (tran.ForCV == undefined || tran.ForCV == null)
            tran.ForCV = 2;

        $scope.beData.ProjectId = tran.ProjectId;
        $scope.beData.TranId = tran.TranId;
        $scope.beData.ForCV = tran.ForCV;
        ...
        $scope.beData.IELedgerId = tran.IELedgerId;
        $scope.beData.IECostCenterId = tran.IECostCenterId;
        $scope.beData.CashBankLedgerId = tran.CashBankLedgerId;
        $scope.beData.PartyLedgerId = tran.AutoPartyLedgerId;
        ...
        var drLedAllocation = mx(tran.LedgerAllocationColl).firstOrDefault(p1 => p1.DrCr == 1 && p1.LedgerId == tran.AutoPartyLedgerId);
        if (drLedAllocation) {
            ...
            if ($scope.beData.CostCenterColl && $scope.beData.CostCenterColl.length == 1) {
                $scope.beData.Dimension7 = $scope.beData.CostCenterColl[0].CostCenterId;
            }

            if (drLedAllocation.BillRefColl && drLedAllocation.BillRefColl.length > 0) {
                $scope.CurBillRefColl = angular.copy(drLedAllocation.BillRefColl);

                $scope.CurBillRefColl.forEach(function (br) {
        ...
        var cbLedAllocation = mx(tran.LedgerAllocationColl).firstOrDefault(p1 => p1.DrCr == 2 && p1.LedgerId == tran.CashBankLedgerId);
        if (cbLedAllocation) {
            
            if (cbLedAllocation.CheckDetails && cbLedAllocation.CheckDetails.ChequeDate) {
                cbLedAllocation.CheckDetails.ChequeDate_TMP = new Date(cbLedAllocation.CheckDetails.ChequeDate);
                $scope.beData.CheckDetails = cbLedAllocation.CheckDetails;
            }
        ...
        angular.forEach(tran.LedgerAllocationColl, function (ledAll) {
            ...
            if (ledAll.LedgerId!=tran.AutoPartyLedgerId && ledAll.LedgerId!=tran.CashBankLedgerId)
    ...
    $scope.ChangeCrAmount = function (curLA,col) {
        var total = 0;
    ...
    $scope.ChangeBillWiseAmt = function (colName) {
        ...
        $scope.ChangeCrAmount($scope.CurLedgerAllocation,colName);
