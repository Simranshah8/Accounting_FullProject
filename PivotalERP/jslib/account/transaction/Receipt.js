app.controller('ReceiptController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
 ...
 <div class="input-group-prepend">
                ...
                if (ledDet.LedgerId && ledDet.LedgerId > 0 && ledDet.LedgerDetails) {
                    ...
                    if (ledDet.LedgerDetails.BillWiseAdjustment == true) {
                        ...
                        if (!ledDet.BillRefColl || ledDet.BillRefColl.length == 0) {
                            $http({
                                method: 'GET',
                                url: base_url + "Inventory/Transaction/getDuesBillList?ledgerId=" + ledDet.LedgerId + '&branchId=' + $scope.SelectedVoucher.BDId+"&billwise=false",
                                dataType: "json"
                            }).then(function (res) {
                                if (res.data.IsSuccess && res.data.Data) {
    ...
    $scope.GetData = function () {
        ...
        if ($scope.beData.FundDetails && $scope.beData.FundDetails.TransactionRefNo && $scope.beData.FundDetails.TransactionRefNo.length > 0) {
            ...
            drLedAllocation.FundDetails.Amount = drLedAllocation.DrAmount;
        }
