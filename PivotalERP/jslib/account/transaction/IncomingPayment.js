app.controller('IncomingPaymentController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    ...
    function LoadData() {
        ...
        $scope.VoucherSearchOptions = [{ text: 'AutoVoucherNo', value: 'TS.AutoVoucherNo', searchType: 'Number' }, { text: 'VoucherNo', value: 'TS.AutoManualNo', searchType: 'text' }, { text: 'VoucherNo', value: 'TS.AutoManualNo', searchType: 'text' }, { text: 'RefNo', value: 'TS.[RefNo]', searchType: 'text' }, { text: 'VoucherDate', value: 'TS.VoucherDate', searchType: 'date' }, { text: 'Voucher Name', value: 'V.VoucherName', searchType: 'text' }, { text: 'CostClass', value: 'CC.Name', searchType: 'text' }, { text: 'Ledger', value: 'Led.Name', searchType: 'text' }, { text: 'Amount', value: 'LAR.DrAmount', searchType: 'Number' }, { text: 'Party', value: 'PLed.Name', searchType: 'text' },];
 ...
 <div class="input-group-prepend">
    ...
    $scope.ChangeCashBankLedger = function (ledDet) {
        ...
        if (ledDet == null || ledDet.PartyLedgerId == null || ledDet.PartyLedgerId == undefined || lastPartyId != ledDet.LedgerId ) {
            $scope.CurBillRefColl = [];
            lastPartyId = ledDet.LedgerId;
        }
        ...
        $timeout(function () {
            ...
            if (ledDet) {
                ledDet.CurrentBal = 0;
                if (ledDet.PartyLedgerId && ledDet.PartyLedgerId > 0 && ledDet.LedgerDetails) {
            ...
            if (ledDet) {
                ...
                if (ledDet.PartyLedgerId && ledDet.PartyLedgerId > 0 && ledDet.LedgerDetails) {
                    ...
                    if (!$scope.CurBillRefColl || $scope.CurBillRefColl.length == 0) {
                        $http({
                            method: 'GET',
                            url: base_url + "Inventory/Transaction/getDuesBillList?ledgerId=" + ledDet.PartyLedgerId + '&branchId=' + $scope.SelectedVoucher.BDId + "&billwise=true",
                            dataType: "json"
                        }).then(function (res) {
                            if (res.data.IsSuccess && res.data.Data) {
    ...
    $scope.getDuesBillForRec = function (ledDet) {
        ...
        $http({
            method: 'GET',
            url: base_url + "Inventory/Transaction/getDuesBillList?ledgerId=" + ledDet.LedgerId + '&branchId=' + $scope.SelectedVoucher.BDId + "&billwise=" + ledDet.LedgerDetails.BillWiseDues,
            dataType: "json"
        }).then(function (res) {
            $scope.loadingstatus = "stop";
            hidePleaseWait();
    ...
    $scope.GetData = function () {
        ...
        if ($scope.beData.IECostCenterId > 0) {
            ...
            if (findIECC) {
                tmpReceipt.IELedgerId = findIECC.LedgerId;
            }
        }
        ...
        var drAmt = mx($scope.beData.LedgerAllocationColl).sum(p1 => p1.DrAmount - p1.CrAmount);
        ...
        if (cbLedAllocation.LedgerId > 0) {
            ...
            tmpReceipt.LedgerAllocationColl.push(vCrLedAllocation);
              
            cbLedAllocation.CrAmount = vCrLedAllocation.DrAmount;
            cbLedAllocation.DrAmount = 0;
            tmpReceipt.LedgerAllocationColl.push(cbLedAllocation);
        }
    ...
    $scope.SetData = function (tran) {
        ...
        var drLedAllocation = mx(tran.LedgerAllocationColl).firstOrDefault(p1 => p1.DrCr == 2 && p1.LedgerId == tran.AutoPartyLedgerId);
        if (drLedAllocation) {
            lastPartyId = drLedAllocation.LedgerId;
            ...
            if ($scope.beData.CostCenterColl && $scope.beData.CostCenterColl.length == 1) {
                $scope.beData.Dimension8 = $scope.beData.CostCenterColl[0].CostCenterId;
            }
        ...
        var cbLedAllocation = mx(tran.LedgerAllocationColl).firstOrDefault(p1 => p1.DrCr == 1 && p1.LedgerId == tran.CashBankLedgerId);
        if (cbLedAllocation) {
    ...
    $scope.ClearData = function () {
        ...
        $scope.beData.VoucherDateADDet = null;
        $scope.beData.VoucherDateDet = null;
        $scope.beData.VoucherDate = null;
        $scope.beData.VoucherDate_TMP = null;
        $timeout(function () {
            $scope.getVoucherNo();
        })
