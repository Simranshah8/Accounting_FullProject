app.controller('WorkingShiftController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.IsValidCreate = function () {
		if (!$scope.newWorkingShift.EmployeeId || $scope.newWorkingShift.EmployeeId==0) {
	...
	$scope.CallSaveUpdateCreate = function () {
		...
		$http({
			method: 'POST',
			url: base_url + "HR/Master/SaveEmpWorkingShift",
	...
	$scope.GetAllCreateList = function () {
		...
		$http({
			method: 'POST',
			url: base_url + "HR/Master/GetAllEmpWorkingShift",
			dataType: "json"
		}).then(function (res) {
			hidePleaseWait();
			$scope.loadingstatus = "stop";
	...
	$scope.GetCreateById = function () {
		...
		showPleaseWait();
		var para = {
			EmployeeId: $scope.newWorkingShift.EmployeeId
		};

		$http({
			method: 'POST',
			url: base_url + "HR/Master/GetEmpWorkingShiftById",
			dataType: "json",
	...
	$scope.DelCreateById = function (refData) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				$http({
					method: 'POST',
					url: base_url + "HR/Master/DelEmpWorkingShift",
					dataType: "json",
					data: JSON.stringify(para)
				}).then(function (res) {
					hidePleaseWait();
