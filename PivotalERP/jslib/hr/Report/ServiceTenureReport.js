...
app.controller("ServiceTenureReportController", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {

	LoadData();
	$scope.onBtExportCSV = function () {
		var params = {
			fileName: 'ServiceTenureReport.csv',
			sheetName: 'ServiceTenureReport'
		};
	...
	function LoadData() {
		...
		$scope.TenureReportColl = [{ id: 1, text: 'Date of Joining' }, { id: 2, text: 'Date of Permanent' }, { id: 3, text: 'Date of Retirement' }];
		...
		$scope.columnDefs = [
			...
			{ field: "PersonalContactNo", headerName: "Personal Contact No", width: 140, filter: 'agTextColumnFilter', cellStyle: { 'textAlign': 'left' } },
	...
	$scope.ClearDate = function () {
		if ($scope.newFilter.TenureReport == 1) {
			$scope.newFilter.JoinDateFrom_TMP = null;
			$scope.newFilter.JoinDateTo_TMP = null;
			$scope.newFilter.PermanentDateFrom_TMP = null;
			$scope.newFilter.PermanentDateTo_TMP = null;
			$scope.newFilter.RetireDateFrom_TMP = null;
			$scope.newFilter.RetireDateTo_TMP = null;
		} else if ($scope.newFilter.TenureReport == 2) {
			$scope.newFilter.JoinDateFrom_TMP = null;
			$scope.newFilter.JoinDateTo_TMP = null;
			$scope.newFilter.PermanentDateFrom_TMP = null;
			$scope.newFilter.PermanentDateTo_TMP = null;
			$scope.newFilter.RetireDateFrom_TMP = null;
			$scope.newFilter.RetireDateTo_TMP = null;
		} else if ($scope.newFilter.TenureReport == 3) {
			$scope.newFilter.JoinDateFrom_TMP = null;
			$scope.newFilter.JoinDateTo_TMP = null;
			$scope.newFilter.PermanentDateFrom_TMP = null;
			$scope.newFilter.PermanentDateTo_TMP = null;
	...
	$scope.GetServiceTenureReport = function () {
		$scope.DataColl = [];
		var para = {

			JoinDateFrom: $scope.newFilter.JoinDateFromDet?.dateAD
				? $filter('date')($scope.newFilter.JoinDateFromDet.dateAD, 'yyyy-MM-dd')
				: null,

			JoinDateTo: $scope.newFilter.JoinDateToDet?.dateAD
				? $filter('date')($scope.newFilter.JoinDateToDet.dateAD, 'yyyy-MM-dd')
				: null,

			PermanentDateFrom: $scope.newFilter.PermanentDateFromDet?.dateAD
				? $filter('date')($scope.newFilter.PermanentDateFromDet.dateAD, 'yyyy-MM-dd')
				: null,

			PermanentDateTo: $scope.newFilter.PermanentDateToDet?.dateAD
				? $filter('date')($scope.newFilter.PermanentDateToDet.dateAD, 'yyyy-MM-dd')
				: null,

			RetireDateFrom: $scope.newFilter.RetireDateFromDet?.dateAD
				? $filter('date')($scope.newFilter.RetireDateFromDet.dateAD, 'yyyy-MM-dd')
				: null,

			RetireDateTo: $scope.newFilter.RetireDateToDet?.dateAD
				? $filter('date')($scope.newFilter.RetireDateToDet.dateAD, 'yyyy-MM-dd')
				: null,
			...
			CompanyId: ($scope.newFilter.CompanyId && $scope.newFilter.CompanyId.length > 0 ? $scope.newFilter.CompanyId.toString() : '')
		};
