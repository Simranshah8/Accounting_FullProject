app.controller('AllowPayHeadingController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.SaveAllowPayHeading = function () {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				$http({
					method: 'POST',
					url: base_url + "HR/Transaction/SaveAllowPayHeading",
	...
	$scope.GetPayHeadingForAllow = function () {
		...
		$http({
			method: 'GET',
			url: base_url + "HR/Transaction/GetAllEmployeeForAllowPayHeading?BranchId=" + $scope.newFilter.BranchId + "&DepartmentId=" + $scope.newFilter.DepartmentId + "&CategoryId=" + $scope.newFilter.CategoryId + "&CompanyRelationshipId=" + $scope.newFilter.CompanyRelationshipId,
			dataType: "json"
		}).then(function (res) {
			hidePleaseWait();
			$scope.loadingstatus = "stop";
