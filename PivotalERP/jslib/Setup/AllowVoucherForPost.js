...
app.controller('AllowVoucherForPostController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.GetAllowBackDateEntryList = function () {
		...
		var para = {
			forUserId: ($scope.newAllowBackDateEntry.UserWise == 2 ? null : $scope.newAllowBackDateEntry.UserId),
			forGroupId: ($scope.newAllowBackDateEntry.UserWise == 1 ? null : $scope.newAllowBackDateEntry.GroupId)
		};

		if (para.forUserId > 0 || para.forGroupId > 0) {
			...
			$http({
				method: 'POST',
				url: base_url + "Setup/Security/GetAllowVoucherForPost",
				dataType: "json",
	...
	$scope.CallSaveUpdateAllowBackDateEntry = function () {
		...
		var forId = 0;
		var forType = $scope.newAllowBackDateEntry.UserWise;

		if (forType == 1)
			forId = $scope.newAllowBackDateEntry.UserId;
		else
			forId = $scope.newAllowBackDateEntry.GroupId;

		$http({
			method: 'POST',
			url: base_url + "Setup/Security/SaveAllowVoucherForPost",
