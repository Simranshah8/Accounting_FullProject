...
app.controller('partsDemandGodownController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.CheckAllForTransaction = function () {
		...
		angular.forEach(tmpData, function (ent) {
			...
			ent.ForReporting = $scope.newAllowGodown.ForTransaction;
		});
	}
	...
	$scope.GetAllowGodownList = function () {
		...
		$http({
			method: 'POST',
			url: base_url + "Setup/Security/GetAllPartsDemandGodown",
			dataType: "json",
	...
	$scope.CallSaveUpdateAllowGodown = function () {
		...
		angular.forEach($scope.AllowGodownList, function (g) {
			g.ForReporting = false;
		});
		...
		$http({
			method: 'POST',
			url: base_url + "Setup/Security/SavePartsDemandGodown",
