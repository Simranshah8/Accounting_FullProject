...
app.controller('PharmacySalesReturnController', function ($scope, $http, $timeout, $filter, GlobalServices) {
    $scope.Title = 'Pharmacy Sales Return';
    ...
    $scope.ChangeBuyerSelection = function () {
        ...
        if ($scope.beData.PharmacySalesReturnDetail.Buyes && $scope.beData.PharmacySalesReturnDetail.Buyes.length > 2) {
            var findP = $scope.CashPartyColl_Qry.firstOrDefault(p1 => p1.Buyes == $scope.beData.PharmacySalesReturnDetail.Buyes);
            if (findP) {
                $scope.beData.PharmacySalesReturnDetail.SalesTaxNo = findP.SalesTaxNo;
                $scope.beData.PharmacySalesReturnDetail.Address = findP.Address;
                $scope.beData.PharmacySalesReturnDetail.PhoneNo = findP.PhoneNo;
            }
        }
    }
    ...
    $scope.ChangeBatch = function (itemDet) {
        if (itemDet.Batch && itemDet.Batch.length > 0 && itemDet.productDetail) {
            ...
            if (findB) {
                ...
                if (itemDet.SalesRate || itemDet.SalesRate > 0)
                    itemDet.Rate = itemDet.SalesRate;
            }
    ...
    $scope.PartySelectionChange = function (partyDet) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            if (partyDet.PartyLedger) {
                if (partyDet.PharmacySalesReturnDetail) {
                    if ($scope.comDet.Maintain == 3) {

                        if (!partyDet.PharmacySalesReturnDetail.Buyes)
                            partyDet.PharmacySalesReturnDetail.Buyes = partyDet.PartyLedger.Name;

                        if (!partyDet.PharmacySalesReturnDetail.Address)
                            partyDet.PharmacySalesReturnDetail.Address = partyDet.PartyLedger.Address;

                        if (!partyDet.PharmacySalesReturnDetail.SalesTaxNo)
                            partyDet.PharmacySalesReturnDetail.SalesTaxNo = partyDet.PartyLedger.PanVat;

                        if (!partyDet.PharmacySalesReturnDetail.PhoneNo)
                            partyDet.PharmacySalesReturnDetail.PhoneNo = partyDet.PartyLedger.MobileNo1;
                    ...
                    partyDet.PharmacySalesReturnDetail.BuyesId = null;
                }
            }

            if ($scope.SelectedVoucher.ActivePartyDetails == true && $scope.comDet.Maintain != 3)
                $('#frmPharmacySalesReturnDetailsModel').modal('show');
            ...
            if ($scope.comDet.Maintain != 3)
                partyDet.PharmacySalesReturnDetail = {};

            //partyDet.ItemDetailsColl = [];
            //$scope.AddRowInItemDetails(0);
            $scope.RecalculateAdditioncalCost();
            $scope.CalculateTotalAndSubTotal();
            $('#frmPharmacySalesReturnDetailsModel').modal('hide');
        }
    };
            ...
            $http({
                ...
                if (res1.data.IsSuccess && res1.data.Data) {
                    ...
                    if (refVType == 1) {
                        ...
                        $scope.beData.PharmacySalesReturnDetail.Description = tmpdata.RefNo;

                        $scope.beData.PharmacySalesReturnDetail.Goods = tmpdata.DeliveryNoteDetail.RegdNo;

                        $scope.beData.PharmacySalesReturnDetail.Buyes = tmpdata.DeliveryNoteDetail.Buyes;
                        $scope.beData.PharmacySalesReturnDetail.Address = tmpdata.DeliveryNoteDetail.Address;
                        $scope.beData.PharmacySalesReturnDetail.PhoneNo = tmpdata.DeliveryNoteDetail.PhoneNo;
                        $scope.beData.PharmacySalesReturnDetailSalesTaxNo = tmpdata.DeliveryNoteDetail.SalesTaxNo;

                        $scope.beData.PharmacySalesReturnDetail.DriverName = tmpdata.DeliveryNoteDetail.DriverName;
                        $scope.beData.PharmacySalesReturnDetail.DriverContactNo = tmpdata.DeliveryNoteDetail.DriverContactNo;
                    }
                    else if (refVType == 4) {
                        $scope.beData.PharmacySalesReturnDetail.Buyes = tmpdata.Buyes;
                        $scope.beData.PharmacySalesReturnDetail.Address = tmpdata.Address;
                        $scope.beData.PharmacySalesReturnDetail.PhoneNo = tmpdata.PhoneNo;
                        $scope.beData.PharmacySalesReturnDetail.SalesTaxNo = tmpdata.SalesTaxNo;
                        $scope.beData.PharmacySalesReturnDetail.CreditDays = tmpdata.CreditDays;
                        $scope.beData.PharmacySalesReturnDetail.Description = tmpdata.Description;

                        $scope.beData.PharmacySalesReturnDetail.OwnerName = tmpdata.OwnerName;
                        $scope.beData.PharmacySalesReturnDetail.OwnerContactNo = tmpdata.OwnerContactNo;
                        $scope.beData.PharmacySalesReturnDetail.DriverName = tmpdata.DriverName;
                        $scope.beData.PharmacySalesReturnDetail.DriverContactNo = tmpdata.DriverContactNo;
                        $scope.beData.PharmacySalesReturnDetail.DriverAddress = tmpdata.DriverAddress;
                        $scope.beData.PharmacySalesReturnDetail.LicenseNo = tmpdata.LicenseNo;

                        $scope.beData.PharmacySalesReturnDetail.Goods = tmpdata.Goods;
                        $scope.beData.PharmacySalesReturnDetail.Quantity = tmpdata.Quantity;

                        $scope.beData.PharmacySalesReturnDetail.TotalWT = tmpdata.TotalWT;
                        $scope.beData.PharmacySalesReturnDetail.FreightRate = tmpdata.FreightRate;
                        $scope.beData.PharmacySalesReturnDetail.AdvancePayment = tmpdata.AdvancePayment;

                        $scope.beData.PharmacySalesReturnDetail.OtherRefereces = tmpdata.OtherRefereces;
                        $scope.beData.PharmacySalesReturnDetail.TermsOfPayment = tmpdata.TermsOfPayment;
                        $scope.beData.PharmacySalesReturnDetail.TermsOfDelivery = tmpdata.TermsOfDelivery;
                        $scope.beData.PharmacySalesReturnDetail.Destination = tmpdata.Destination;
                        $scope.beData.PharmacySalesReturnDetail.DeliveryThrough = tmpdata.DeliveryThrough;
                        $scope.beData.PharmacySalesReturnDetail.DeliveryDocNo = tmpdata.DeliveryDocNo;
                    }
    ...
    $scope.SavePharmacySalesReturn = function () {
        ...
        if ($scope.IsValidData() == true) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/SaveUpdatePharmacySalesReturn",
    ...
    $scope.GetData = function () {
        ...
        var tmpSales = {
            ...
            AditionalCostColl: [],
            PharmacySalesReturnDetail: $scope.beData.PharmacySalesReturnDetail,
            GodownId: $scope.beData.GodownId,
        };
        ...
        if ($scope.beData.VoucherId && $scope.beData.VoucherId > 0) {
            if ($scope.beData.CostClassId && $scope.beData.CostClassId > 0) {
                var para = {
                    voucherId: $scope.beData.VoucherId,
                    costClassId: $scope.beData.CostClassId,
                    voucherDate: $scope.beData.VoucherDateDet ? ($filter('date')(new Date($scope.beData.VoucherDateDet.dateAD)), 'yyyy-MM-dd') : new Date()
                };
                ...
                if (res.data.IsSuccess && res.data.Data) {
                    ...
                    if (templatesColl && templatesColl.length > 0) {
                        ...
                        if (rptTranId > 0) {
                            ...
                            //        printJS({ printable: res.data.ResponseMSG, type: 'pdf', showModal: true, modalMessage:'Wait for printing' });
    ...
    $scope.ShowSSFDetails = function () {
        if ($scope.beData.RefNo && $scope.beData.RefNo.length > 0 && $scope.beData.PharmacySalesReturnDetail.OtherRefereces && $scope.beData.PharmacySalesReturnDetail.OtherRefereces.length > 0) {
            if ($scope.beData.PharmacySalesReturnDetail.SSF)
                $('#frmSSFDetails').modal('show');
        }
    };
    $scope.SSFCodeLostFocus = function () {
        if ($scope.beData.PharmacySalesReturnDetail.OtherRefereces && $scope.beData.PharmacySalesReturnDetail.OtherRefereces.length > 0) {

        }
        else {
            $scope.beData.PharmacySalesReturnDetail.SSF = null;
        }
    }
    $scope.GetPatientById = function () {
        $scope.beData.PharmacySalesReturnDetail.SSF = null;
        $scope.beData.PharmacySalesReturnDetail.PatientId = null;
        if ($scope.beData.RefNo && $scope.beData.RefNo.length > 0) {
            ...
            $http({
                ...
                if (patient.IsSuccess == true) {
                    $scope.beData.PharmacySalesReturnDetail.Buyes = patient.PatientName;
                    $scope.beData.PharmacySalesReturnDetail.Address = patient.Address;
                    $scope.beData.PharmacySalesReturnDetail.PhoneNo = patient.MobileNo;
                    $scope.beData.PharmacySalesReturnDetail.OtherRefereces = patient.ClaimCode;
                    $scope.beData.PharmacySalesReturnDetail.SSF = patient.SSF;
                    $scope.beData.PharmacySalesReturnDetail.PatientId = patient.PatientId;

                } else {
                    $scope.beData.PharmacySalesReturnDetail.Buyes = '';
                    $scope.beData.PharmacySalesReturnDetail.Address = '';
                    $scope.beData.PharmacySalesReturnDetail.PhoneNo = '';
                    $scope.beData.PharmacySalesReturnDetail.OtherRefereces = '';
