app.controller('SalesOrderController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
 ...
 $scope.DefaultKeyValues_JSON = null;
        ...
        $scope.PendingRefVoucherColl = [];
        $scope.RefVoucherTypeColl = [{ id: 2, text: 'MaterialRequest(Indent)' }, { id: 1, text: 'SalesQuotation' }];
 ...
 $scope.EPDet = {};
    ...
    $scope.ProductSelectionChange = function (itemDet, ind) {
        ...
        if (itemDet.ProductId>0 && itemDet.ProductId != itemDet.LastProductId) {
            itemDet.ActualQty = 0;
            itemDet.BilledQty = 0;
            itemDet.Rate = 0;
            itemDet.ClosingQty = '';
    ...
    $scope.PartySelectionChange = function (partyDet) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            if (partyDet.PartyLedger) {
                ...
                if ($scope.lastPartyLedgerId != partyDet.PartyLedgerId && isModify == false) {
                    ...
                    if (partyDet.PartyLedger.BillToColl && partyDet.PartyLedger.BillToColl.length > 0)
                        partyDet.SalesOrderDetail.BillToId = partyDet.PartyLedger.BillToColl[0].Id;

                    if (partyDet.PartyLedger.ShipToColl && partyDet.PartyLedger.ShipToColl.length > 0)
                        partyDet.SalesOrderDetail.ShipToId = partyDet.PartyLedger.ShipToColl[0].Id;

                    $scope.ChangeBillAddress();
                    $scope.ChangeShipAddress();

                    if (partyDet.SalesOrderDetail && isModify == false)
                        partyDet.SalesOrderDetail.CreditDays = partyDet.PartyLedger.CreditLimitDays;
            ...
            if ($scope.SelectedVoucher.ActivePartyDetails == true)
                $('#frmSalesOrderDetailsModel').modal('show');
            ...
            $scope.RecalculateAdditioncalCost();
            $scope.CalculateTotalAndSubTotal();
            $('#frmSalesOrderDetailsModel').modal('hide');
        }
    ...
    $scope.ChangeBillAddress = function () {
        ...
        if ($scope.beData.PartyLedger.BillToColl) {
            var find = mx($scope.beData.PartyLedger.BillToColl).firstOrDefault(p1 => p1.Id == $scope.beData.SalesOrderDetail.BillToId);
            if (find) {
                $scope.beData.SalesOrderDetail.BillToAddress = find.ConcatAddress;
            }
        }
    }
    $scope.ChangeShipAddress = function () {
        if ($scope.beData.PartyLedger.ShipToColl) {
            var find = mx($scope.beData.PartyLedger.ShipToColl).firstOrDefault(p1 => p1.Id == $scope.beData.SalesOrderDetail.ShipToId);
            if (find) {
                $scope.beData.SalesOrderDetail.ShipToAddress = find.ConcatAddress;
            }
        }
    }
        ...
        if ($scope.beData.RefVoucherType && refItem && refItem.IsSelected == true) {
            ...
            $http({
                ...
                if (res1.data.IsSuccess && res1.data.Data) {
                    ...
                    $scope.beData.SalesOrderDetail.TermsOfPayment = tranDet.TermsOfPayment;
                    $scope.beData.SalesOrderDetail.OtherRefereces = tranDet.OtherRefereces;
                    $scope.beData.SalesOrderDetail.TermsOfDelivery = tranDet.TermsOfDelivery;
                    $scope.beData.SalesOrderDetail.DeliveryThrough = tranDet.DeliveryThrough;                    
                    $scope.beData.SalesOrderDetail.DeliveryDocNo = tranDet.DeliveryDocNo;

                    $scope.beData.SalesOrderDetail.DispatchedThrough = tranDet.DeliveryThrough;
                    $scope.beData.SalesOrderDetail.DispatchDocNo = tranDet.DeliveryDocNo;

                    $scope.beData.SalesOrderDetail.Destination = tranDet.Destination;
                    $scope.beData.SalesOrderDetail.SalesQuotationTermsOfPayment = tranDet.SalesQuotationTermsOfPayment;
                    $scope.beData.SalesOrderDetail.SalesQuotationOtherRefereces = tranDet.SalesQuotationOtherRefereces;
                    $scope.beData.SalesOrderDetail.SalesQuotationTermsOfDelivery = tranDet.SalesQuotationTermsOfDelivery;
                    $scope.beData.SalesOrderDetail.Buyes = tranDet.Buyes;
                    $scope.beData.SalesOrderDetail.Address = tranDet.Address;
                    $scope.beData.SalesOrderDetail.SalesTaxNo = tranDet.SalesTaxNo;
                    $scope.beData.SalesOrderDetail.CSTNumber = tranDet.CSTNumber;
                    $scope.beData.SalesOrderDetail.Notes = tranDet.Notes;
                    $scope.beData.SalesOrderDetail.PhoneNo = tranDet.PhoneNo;
                    $scope.beData.SalesOrderDetail.Description = tranDet.Description;
                    $scope.beData.SalesOrderDetail.OwnerName = tranDet.OwnerName;
                    $scope.beData.SalesOrderDetail.OwnerContactNo = tranDet.OwnerContactNo;
                    $scope.beData.SalesOrderDetail.DriverAddress = tranDet.DriverAddress;
                    $scope.beData.SalesOrderDetail.DriverName = tranDet.DriverName;
                    $scope.beData.SalesOrderDetail.DriverContactNo = tranDet.DriverContactNo;
                    $scope.beData.SalesOrderDetail.LicenseNo = tranDet.LicenseNo;
                    $scope.beData.SalesOrderDetail.Goods = tranDet.Goods;
                    $scope.beData.SalesOrderDetail.Quantity = tranDet.Quantity;
                    $scope.beData.SalesOrderDetail.FreightRate = tranDet.FreightRate;
                    $scope.beData.SalesOrderDetail.AdvancePayment = tranDet.AdvancePayment;
                    $scope.beData.SalesOrderDetail.TotalWT = tranDet.TotalWT;
                    $scope.beData.SalesOrderDetail.ContactNo = tranDet.ContactNo;
                    $scope.beData.SalesOrderDetail.RegdNo = tranDet.RegdNo;
                    $scope.beData.SalesOrderDetail.EngineNo = tranDet.EngineNo;
                    $scope.beData.SalesOrderDetail.ChassisNo = tranDet.ChassisNo;
                    $scope.beData.SalesOrderDetail.Model = tranDet.Model;
                    $scope.beData.SalesOrderDetail.VinNo = tranDet.VinNo;
                    $scope.beData.SalesOrderDetail.CreditDays = tranDet.CreditDays;
                    $scope.beData.SalesOrderDetail.ExportCountry = tranDet.ExportCountry;
                    $scope.beData.SalesOrderDetail.PPNo = tranDet.PPNo;
                    $scope.beData.SalesOrderDetail.PPDate = tranDet.PPDate;
                    $scope.beData.SalesOrderDetail.OtherRefereces1 = tranDet.OtherRefereces1;
                    $scope.beData.SalesOrderDetail.PaymentTermsId = tranDet.PaymentTermsId;
                    $scope.beData.SalesOrderDetail.FreightTypeId = tranDet.FreightTypeId;
                    $scope.beData.Narration = tranData.Narration;
                    $scope.beData.No = tranData.RefNo;
                    $scope.beData.RefNo = tranData.RefNo;
                    $scope.beData.SalesOrderDetail.BillToId = tranDet.BillToId;
                    $scope.beData.SalesOrderDetail.ShipToId = tranDet.ShipToId;
                    $scope.beData.SalesOrderDetail.BillToAddress = tranDet.BillToAddress;
                    $scope.beData.SalesOrderDetail.ShipToAddress = tranDet.ShipToAddress;
        ...
        if (filterData.length > 0) {
            ...
            angular.forEach(filterData, function (fd) {
                var refItem = {
                    ...
                    RefQty: fd.ActualQty,
                    QuotationItemAllocationId: refVType == 1 ? fd.ItemAllocationId : null
                };
            ...
            $timeout(function () {
                ...
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/GetSalesQuotaionDetailsByItemAllocationId",
                    ...
                    if (res1.data.IsSuccess == true) {
                        ...
                        angular.forEach(res1.data.Data.ItemAllocationColl, function (ias) {
                            ...
                            if (isEmptyAmt(ias.TranUnitQty) == 0) {
                                ias.TranUnitQty = ias.ActualQty;
                                ias.TranUnitId = ias.UnitId;
                            }
    ...
    $scope.RefVoucherChange = function (refVType) {
        ...
        var funName = "getPendinSalesQuotation";
    ...
    $scope.SaveSalesOrder = function () {
        ...
        if ($scope.IsValidData() == true) {
            ...
            if ($scope.beData.PartyLedger) {
                if ($scope.beData.PartyLedger.CreditLimitAs == 2 || $scope.beData.PartyLedger.CreditLimitAs == 3) {
                    var vDate = new Date();
                    if ($scope.beData.VoucherDateDet) {
                        vDate = $filter('date')(new Date($scope.beData.VoucherDateDet.dateAD), 'yyyy-MM-dd');
                    }
                    GlobalServices.IsValidCredit($scope.SelectedVoucher.VoucherId, $scope.SelectedCostClass.CostClassId, $scope.beData.PartyLedgerId, vDate, $scope.beData.TranId, $scope.beData.TotalAmount).then(function (res) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $timeout(function () {
                    ...
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/SaveUpdateSalesOrder",
                        headers: { 'Content-Type': undefined },
        ...
        $timeout(function () {
            ...
            if (tran.TranId && tran.TranId > 0) {
                ...
                $timeout(function () {
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/GetSalesOrderById",
                        dataType: "json",
                        data: JSON.stringify(para)
                    }).then(function (res) {
                        $timeout(function () {
    ...
    $scope.GetData = function () {
        ...
        if ($scope.beData.SalesOrderDetail.DueDateDet) {
            $scope.beData.SalesOrderDetail.DueDate = new Date($scope.beData.SalesOrderDetail.DueDateDet.dateAD);
        }

        if (tmpSales.SalesOrderDetail) {
            if (tmpSales.SalesOrderDetail.FreightRate == undefined || tmpSales.SalesOrderDetail.FreightRate == null || tmpSales.SalesOrderDetail.FreightRate === undefined)
                tmpSales.SalesOrderDetail.FreightRate = 0;

            if (tmpSales.SalesOrderDetail.AdvancePayment == undefined || tmpSales.SalesOrderDetail.AdvancePayment == null || tmpSales.SalesOrderDetail.AdvancePayment === undefined)
                tmpSales.SalesOrderDetail.AdvancePayment = 0;

            if (tmpSales.SalesOrderDetail.CreditDays == undefined || tmpSales.SalesOrderDetail.CreditDays == null || tmpSales.SalesOrderDetail.CreditDays === undefined)
                tmpSales.SalesOrderDetail.CreditDays = 0;
        }
        ...
        angular.forEach($scope.beData.UDFFeildsColl, function (udf) {
            if (udf.NameId && udf.NameId.length > 0) {
                ...
                else if (udf.FieldType == 3)
                {
                    var ud = {
                        SNo: (udf.FieldNo ? udf.FieldNo : udf.SNo),
                        Name: udf.Name,
                        Value: udf.UDFValue,
                        AlValue: udf.UDFValueDet ? udf.UDFValueDet.text : udf.UDFValue,
                    };
                    voucherUDFFields.push(ud);
                    voucherKeyVal[udf.NameId] = udf.UDFValueDet ? udf.UDFValueDet.text : udf.UDFValue
                }
                else {
                    var ud = {
                        SNo: (udf.FieldNo ? udf.FieldNo : udf.SNo),
        ...
        if (tmpSales.PaymentTermColl) {
            var vid = $scope.beData.SalesOrderDetail.ReceiptVoucherId;
            if (!vid) {
            ...
            if (itemDet.ProductId && itemDet.ProductId > 0 && itemDet.RowType == 'P') {
                ...
                angular.forEach(itemDet.UDFFeildsColl, function (udf) {
                    ...
                    if (udf.FieldType == 2 || udf.FieldType == 22 || udf.FieldType == 23) {
                        ...
                        ud.Value = udf.UDFValue;
                        ud.AlValue = udf.UDFValueDet ? udf.UDFValueDet.text : udf.UDFValue;
                        itemKeyVal[udf.NameId] = udf.UDFValueDet ? udf.UDFValueDet.text : udf.UDFValue;
                    }
                    else {
                        ud.Value = udf.UDFValue;
                ...
                var itemAllocation = {
                    ...
                    SalesProjectionItemAllocationId: itemDet.SalesProjectionItemAllocationId ? itemDet.SalesProjectionItemAllocationId : 0,
                    IndentItemAllocationId: itemDet.IndentItemAllocationId ? itemDet.IndentItemAllocationId : 0,
                ...
                if (itemDet.FixedProductColl && itemDet.FixedProductColl.length > 0 && $scope.SelectedVoucher.MultipleFixedProduct == true) {
                    ...
                    var disAmtSep = 0
                    angular.forEach(itemDet.FixedProductColl, function (fp) {
                        if (fp.IsSelected == true || itemDet.productDetail.AllowManualInputFixedProduct == true)
                            fCount++;
                    });;
    ...
    $scope.SetData = function (tran) {
        ...
        if (tran.SalesOrderDetail.DueDate) {
            $scope.beData.SalesOrderDetail.DueDate_TMP = new Date(tran.SalesOrderDetail.DueDate);
        }
 ...
 $timeout(function () {                      
                    ...
                    TextColumn: udf.TextColumn,
                                ...
                                if ($scope.SelectedVoucher.ActiveSummaryEntry == true) {
                                    ...
                                    $scope.beData.SalesOrderDetail.Buyes = 'CASH';
                                    $scope.beData.SalesOrderDetail.Address = 'N/A';
                                ...
                                var mtranid = isEmptyAmt($scope.beData.TranId);
                                if ($scope.SelectedVoucher.AditionalChargeColl && $scope.SelectedVoucher.IsAbbInvoice == false && mtranid==0) {
    ...
    $scope.MakeSalesOrder = function () {
        ...
        $timeout(function ()
        {
            ...
            if (selectedItem && selectedItem.ItemColl && selectedItem.ItemColl.length > 0) {
                ...
                var RefItemAllocationColl = selectedItem.ItemColl;
                ...
                if (RefItemAllocationColl && RefItemAllocationColl.length > 0) {
                    ...
                    $scope.beData.PartyLedgerId = selectedItem.BranchCustomerId;
                    $scope.beData.ToBranchId = selectedItem.ToBranchId;
                    if (selectedItem && selectedItem.ItemColl && selectedItem.ItemColl.length > 0) {
                    ...
                    angular.forEach(RefItemAllocationColl, function (fd) {
                        ...
                        if (fd.BalanceQty > 0) {
                            var refItem = {
                                ...
                                ProductId: fd.ProductId,
                                productDetail: null,
                                ActualQty: fd.BalanceQty,
                                BilledQty: fd.BalanceQty,
                                FreeQty: 0,
                                Rate: fd.Rate,
                                DiscountPer: fd.DiscountPer,
                                DiscountAmt: 0,
                                SchameAmt: 0,
                                SchamePer: 0,
                                Amount: (fd.BalanceQty * fd.Rate),
                                Description: fd.Description,
                                QtyPoint: 0,
                                UnitId: fd.UnitId,
                                CanEditRate: false,
                                ALValue1: fd.AlValue1,
                                ALValue2: fd.AlValue2,
                                ALUnitId1: fd.AlUnitId1,
                                ALUnitId2: fd.AlUnitId2,
                                SchemeAmt: 0,
                                SchemeRate: 0,
                                ExciseAbleQty: 0,
                                ExciseAbtAmt: 0,
                                ...
                                GodownId: (fd.GodownId && fd.GodownId > 0 ? fd.GodownId : $scope.beData.GodownId),
                                Narration: fd.Narration,
                                RefQty: fd.BalanceQty,
                                IndentItemAllocationId: fd.ItemAllocationId,
                                OrderItemAllocationId: 0,
                            };
                    ...
                    angular.forEach($scope.SelectedVoucher.AditionalChargeColl, function (ads) {
                        $scope.beData.ItemDetailsColl.push({
                            AutoCharge: true,
                            RowType: 'L',
                            LedgerId: ads.LedgerId,
    ...
    $scope.ClosedIndent = function (tran) {
        Swal.fire({
            title: 'Are you sure you want to close transaction ' + tran.VoucherNo + '?',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/ClosedIndent",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    hidePleaseWait();
    ...
    $scope.ChangeTermsOfPayment = function () {
        if ($scope.beData && $scope.beData.SalesOrderDetail) {
            if ($scope.beData.SalesOrderDetail.TermsOfPayment) {
                var findPM = $scope.PaymentModeColl_Qry.firstOrDefault(p1 => p1.Name == $scope.beData.SalesOrderDetail.TermsOfPayment);
                if (findPM) {
                    $scope.beData.SalesOrderDetail.TypeOfPayment = findPM.TypeOfPayment;
                    $scope.beData.SalesOrderDetail.PaymentTermsId = findPM.PaymentModeId;
                }
            }
        }
    }
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SaveSalesOrder);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
