app.controller('StockDemand', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
 ...
 $scope.EPDet = {};
        ...
        if (VoucherType || VoucherType > 0) {
            ...
            $http({
                ...
                if (res.data.IsSuccess && res.data.Data) {
                    $scope.TargetGodownColl = res.data.Data;                    
                } else {
                    Swal.fire(res.data.ResponseMSG);
                }
    ...
    $scope.SaveSalesStockDemand = function () {
 ...
 $timeout(function () {                      
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SaveSalesStockDemand);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
