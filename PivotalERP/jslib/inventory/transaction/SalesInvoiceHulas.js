...
app.controller('SalesInvoiceController', function ($scope, $http, $timeout, $filter, GlobalServices) {
        ...
        if (VoucherType) {
            ...
            $http({
                ...
                if (res.data.IsSuccess && res.data.Data) {
                    ...
                    $scope.GodownColl_Qry = mx(res.data.Data);
    ...
    $scope.RefreshRowData = function (itemDet, ind) {
        if (itemDet.RowType == 'P') {
            ...
            $http({
                method: 'GET',
                url: base_url + "Global/GetLedgerDetail?LedgerId=" + itemDet.LedgerId + " & VoucherType=" + $scope.SelectedVoucher.VoucherType,
                dataType: "json"
            }).then(function (resLD) {
        ...
        if (itemDet.ProductId == null || itemDet.ProductId == 0) {
            ...
            $timeout(function () {
                ...
                if (itemDet.GodownId > 0) {
                    var findGColl = $scope.GodownColl_Qry.where(p1 => p1.GodownId == itemDet.GodownId).toArray();
                    var godownCode = '';
                    if (findGColl && findGColl.length > 0) {
                        godownCode = findGColl[0].Code;
                    }

                    if (godownCode.length > 0) {
                        $scope.loadingstatus = 'running';
                        showPleaseWait();

                        $http({
                            method: 'GET',
                            url: base_url + "Inventory/Transaction/H_getItemDet?itemCode=" + itemDet.productDetail.Code + " &godownCode=" + godownCode,
                            dataType: "json"
                        }).then(function (resLD) {
                            ...
                            hidePleaseWait();
                            if (resLD.data.IsSuccess && resLD.data.Data) {
                                itemDet.productDetailH = resLD.data.Data;                                
                            }
                        }, function (reason) {
                            alert('Failed' + reason);
                        });
                        ...
                        $scope.$apply(function () {
                            if ($scope.SelectedVoucher) {
                                ...
                                if ($scope.SelectedVoucher.GodownColl && $scope.SelectedVoucher.GodownColl.length > 0) {
                                    ...
                                    var tmpGColl = [];
                                    angular.forEach($scope.GodownColl, function (gc) {
                                        if (gc.BDId == 0 || $scope.SelectedVoucher.BDId == gc.BDId)
                                            tmpGColl.push(gc);
                                    })

                                    if (tmpGColl.length == 1) {
                                        $scope.beData.GodownId = tmpGColl[0].GodownId;
                                        $scope.HideShow.Godown = true;
                                    }
                                    else if (tmpGColl.length > 1) {
                                        $scope.HideShow.Godown = false;
                                        $scope.beData.GodownId = tmpGColl[0].GodownId;
                                    }
                                    else {
                                    ...
                                    if(tmpGColl.length>0)
                                        $scope.SelectedVoucher.VoucherWiseGodownColl = tmpGColl;
                                    else
                                      $scope.SelectedVoucher.VoucherWiseGodownColl = $scope.GodownColl;
                                }
