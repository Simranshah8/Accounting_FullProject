app.controller('StockDemandCancel', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    $scope.Title = 'StockDemandCancel';
    ...
    function LoadData() {
		...
		$scope.VoucherSearchOptions = [{ text: 'FromGodown', value: 'ADS.SourceGodownName', searchType: 'text' }, { text: 'ToGodown', value: 'ADS.TargetGodownName', searchType: 'text' }, { text: 'VoucherNo', value: 'Led.AutoManualNo', searchType: 'text' }, { text: 'RefNo', value: 'TS.[No]', searchType: 'text' }, { text: 'Voucher', value: 'V.VoucherName', searchType: 'text' }, { text: 'VoucherDate', value: 'TS.VoucherDate', searchType: 'date' }, { text: 'Amount', value: 'TS.TotalAmount', searchType: 'number' }];
 ...
 $scope.EPDet = {};
    ...
    $scope.ShowPendingParts = function () {
        

    };
    $scope.ProductSelectionChange = function (itemDet) {
        $scope.sideBarData = itemDet.partySideBarData;
    ...
    $scope.PartySelectionChange = function (partyDet) {
        ...
        if (partyDet.TargetGodownId && partyDet.TargetGodownId > 0) {
            if (partyDet.TargetGodownName) {
                if (partyDet.StockTransferDetails) {
                    if (!partyDet.StockTransferDetails.Date)
                        partyDet.StockTransfer.Date = partyDet.TargetGodownName.Date;

                    if (!partyDet.StockTransferDetails.CreditDays)
                        partyDet.StockTransferDetails.Address = partyDet.TargetGodownName.Address;

                    if (!partyDet.StockTransferDetails.SalesTaxNo)
                        partyDet.StockTransferDetails.SalesTaxNo = partyDet.PartyLedger.SalesTaxNo;

                    if (!partyDet.StockTransfer.ContactNo)
    ...
    $scope.RefVoucherChange = function (refVType) {
        ...
        var funName = "getStockTransfer";

        if (refVType == 1)
            funName = "getStockTransfer";
        else if (refVType == 2)
        ...
        if ($scope.IsValidData() == true) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/SaveUpdateStockDemandCancel",
    ...
    $scope.reloadVoucherDate = function () {
        ...
        $timeout(function () {
            ...
            {
                dtPicker = dtPicker+ '<input type="text"  class="form-control form-control-sm" date-picker ng-model="beData.VoucherDate_TMP" date-detail="beData.VoucherDateDet" confirm-action="getVoucherNoOnly(2)" title ="{{beData.VoucherDateDet.dateAD |dateFormat}}" date-style="2" id ="dtVoucherDateBS" voucherid ="SelectedVoucher.VoucherId" >';
            }
            else if ($scope.SelectedVoucher.DateStyle == 1) //AD
            {
                dtPicker = dtPicker + '<input type="text"  class="form-control form-control-sm" date-picker ng-model="beData.VoucherDate_TMP" date-detail="beData.VoucherDateDet" confirm-action="getVoucherNoOnly(1)" title ="{{ beData.VoucherDateDet.dateBS }}" date-style="1" id ="dtVoucherDateAD" voucherid ="SelectedVoucher.VoucherId" >';
            }
