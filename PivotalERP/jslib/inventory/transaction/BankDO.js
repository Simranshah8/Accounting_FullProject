app.controller('BankDOController', function ($scope, $http, $timeout, $filter, GlobalServices) {
    $scope.Title = 'Bank DO';
...
$scope.VoucherSearchOptions = [{ text: 'EngineNo', value: 'TS.EngineNo', searchType: 'text' }, { text: 'PartyName', value: 'TS.PartyName', searchType: 'text' }, { text: 'MobileNo', value: 'TS.MobileNo', searchType: 'text' }, { text: 'PartyAddress', value: 'TS.PartyAddress', searchType: 'text' }, { text: 'BankName', value: 'TS.BankName', searchType: 'text' }, { text: 'Quotation No.', value: 'TS.AutoManualNo', searchType: 'text' }, { text: 'Voucher', value: 'V.VoucherName', searchType: 'text' }, { text: 'CostClass', value: 'CC.Name', searchType: 'text' }, { text: 'VoucherDate', value: 'TS.VoucherDate', searchType: 'date' }, { text: 'Amount', value: 'TS.TotalAmount', searchType: 'number' }];
    ...
    $scope.AddBankQuotation = function () {
        ...
        $http({
            method: 'POST',
            url: base_url + "Inventory/Transaction/SaveUpdateBankDO",
    ...
    $scope.getQuotationDet = function () {
        ...
        if ($scope.beData.QuotationNo) {
            if ($scope.beData.QuotationNo.length == 0)
                return;
        } else
            return;
        ...
        $http({
            method: 'GET',
            url: base_url + "Inventory/Transaction/getBankQuotationByNo?quotationNo=" + $scope.beData.QuotationNo,
            dataType: "json"
        }).then(function (res) {
            if (res.data.IsSuccess && res.data.Data) {
                ...
                $scope.beData.BankQuotationTranId = vDet.TranId;
                $scope.beData.QuotationAmt = vDet.TotalAmount;
                ...
                $scope.beData.BankQuotationTranId = null;
                $scope.beData.Price = 0;
    ...
    $scope.ChangeAmt = function () {
        ...
        $scope.beData.DOPer = 0;

        if ($scope.beData.QuotationAmt && $scope.beData.QuotationAmt > 0 && $scope.beData.Price && $scope.beData.Price>0) {

            var per = ($scope.beData.Price / $scope.beData.QuotationAmt) * 100;
            $scope.beData.DOPer = per.toFixed(2);
        }
    };
