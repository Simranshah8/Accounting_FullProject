app.controller('DairyPurchaseInvoiceController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
	$scope.Title = 'DairyPurchaseInvoice';
 ...
 $scope.EPDet = {};
    ...
    $scope.AddRowInItemDetails = function (ind) {
        ...
        var itemDet = {
            ...
            ProductLedgerId: ($scope.beData.tranProductDetail ? $scope.beData.tranProductDetail.PurchaseLedgerId : null),
            LedgerId: ($scope.beData.tranProductDetail ? $scope.beData.tranProductDetail.PurchaseLedgerId : null),
            ActualQty: 0,
            BilledQty: 0,
            FreeQty: 0,
            Rate: 0,
    ...
    $scope.GetPurchaseSetup = function (itemDet) {

        var productId = itemDet.ProductId;
        var partyId = itemDet.PartyId > 0 ? itemDet.PartyId : $scope.beData.PartyLedgerId;

        if (productId > 0 && partyId > 0) {
            showPleaseWait();
            $scope.PartywiseList = [];
            var para = {
                ProductId: productId,
    ...
    $scope.ChangeTranProduct = function () {
        ...
        if (findProduct) {
            ...
            angular.forEach($scope.beData.ItemDetailsColl, function (idet) {
                ...
                idet.ProductLedgerId = findProduct.PurchaseLedgerId;
                idet.LedgerId = findProduct.PurchaseLedgerId;
            });
    ...
    $scope.PartySelectionChangeRw = function (partyDet) {
        ...
        if (partyDet.PartyId && partyDet.PartyId > 0) {             
            $scope.GetPurchaseSetup(partyDet);
        }
            ...
            $http({
                ...
                if (res1.data.IsSuccess && res1.data.Data) {
                    ...
                    if (refVType == 1) {
                        ...
                        $scope.beData.PurchaseInvoiceDetail.Suppliers = tmpdata.DeliveryNoteDetail.Buyes;
                        ...
                        $scope.beData.PurchaseInvoiceDetailSalesTaxNo = tmpdata.DeliveryNoteDetail.SalesTaxNo;
                        ...
                        $scope.beData.PurchaseInvoiceDetail.Suppliers = tmpdata.Buyes;
                        $scope.beData.PurchaseInvoiceDetail.Address = tmpdata.Address;
                        $scope.beData.PurchaseInvoiceDetail.PhoneNo = tmpdata.PhoneNo;
                        $scope.beData.PurchaseInvoiceDetail.SalesTaxNo = tmpdata.SalesTaxNo;
    ...
    $scope.ChangeItemRowValue = function (itemDet, col) {
        ...
        if (itemDet.productDetail) {
            ...
            //            $timeout(function () {
            //                $scope.ChangeItemRowValue(itemDet, ind);
            //            });
        ...
        itemDet.Rate = ($filter('number')(itemDet.Rate / rateOf, itemDet.RateDecimal)).parseDBL();
  ...
  $scope.beData.VoucherDate_TMP = new Date(tran.VoucherDate);
   ...
   //     });
        ...
        $scope.beData.TranProductId =((tran.TranProductId==null || tran.TranProductId==undefined) && tran.ItemAllocationColl.length==1 ? tran.ItemAllocationColl[0].ProductId : tran.TranProductId) ;
        $scope.beData.ShiftId = tran.ShiftId;
        $scope.ChangeTranProduct();
        ...
        $scope.GetPurchaseSetup($scope.beData);
