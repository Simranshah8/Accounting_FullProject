app.controller('StockJournal', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
 ...
 //       }
        ...
        $scope.confirmMSG = glSrv.getConfirmMSG();
		        $scope.VoucherSearchOptions = [ { text: 'RefNo', value: 'TS.[No]', searchType: 'text' }, { text: 'Voucher No.', value: 'TS.AutoManualNo', searchType: 'text' }, { text: 'Voucher', value: 'V.VoucherName', searchType: 'text' }, { text: 'CostClass', value: 'CC.Name', searchType: 'text' }, { text: 'VoucherDate', value: 'TS.VoucherDate', searchType: 'date' }, { text: 'Amount', value: 'TS.TotalAmount', searchType: 'number' }];
 ...
 //       });
    ...
    $scope.ProductSelectionChange = function (itemDet,ind) {
        ...
        if (itemDet.ProductId == null) {
            ...
            if (isModify == false || itemDet.LastProductId!=itemDet.ProductId) {
           ...
           // itemDet.ProductLedgerId = itemDet.productDetail.SalesLedgerId;
            ...
            if(isModify==false || itemDet.ProductId!=itemDet.LastProductId)
    ...
    $scope.ChangeItemRowValue = function (itemDet, col) {
        ...
        if (itemDet.productDetail) {
            if (itemDet.Batch && itemDet.Batch.length > 0) {
                var bBal =isEmptyAmt(itemDet.RefQty)+(itemDet.BatchBalQty || itemDet.BatchBalQty > 0 ? itemDet.BatchBalQty : 0 );
        ...
        itemDet.TotalAmount = isEmptyAmt(itemDet.Amount) + isEmptyAmt(itemDet.Makeing) + isEmptyAmt(itemDet.Stone);
    ...
    $scope.AddRowInTargetItemDetails = function (ind, boolAuto) {
     ...
     //   ind = ind + isEmptyAmt((($scope.currentPages.TargetItemDetRow - 1) * ($scope.perPage.TargetItemDetRow)));
    ...
    $scope.TargetProductSelectionChange = function (itemDet,ind) {
        $scope.sideBarData = itemDet.sideBarData;
        ...
        if (itemDet.ProductId == null) {
           ...
           // itemDet.Rate = itemDet.productDetail.PurchaseRate;
            ...
            if ($scope.SelectedVoucher.Product.BatchNo == true && itemDet.productDetail.MaintainBatchWise == true) {
                if (isModify == false &&   $scope.SelectedVoucher.Product) {
            ...
            var itemC = mx($scope.beData.TargetItemDetailsColl).count();
            if (ind == (itemC - 1))
                $scope.AddRowInTargetItemDetails(ind);
        }
                ...
                if (result.isConfirmed) {
                    ...
                    $http({
                        ...
                        transformRequest: function (data) {
                            ...
                            find = 0;
                            angular.forEach($scope.beData.TargetItemDetailsColl, function (ta) {
                                if (ta.ProductId > 0 && ta.ItemFiles && ta.ItemFiles.length == 1) {
                                    var fname = 'fileT' + find;
                                    formData.append(fname, ta.ItemFiles[0]);
                                }
                                find++;
                            });

                            find = 0;
                            angular.forEach($scope.beData.SourceItemDetailsColl, function (ta) {
                                if (ta.ProductId > 0 && ta.ItemFiles && ta.ItemFiles.length == 1) {
                                    var fname = 'fileS' + find;
                                    formData.append(fname, ta.ItemFiles[0]);
                                }
                                find++;
                            });
    ...
    $scope.GetData = function () {
        ...
        angular.forEach($scope.beData.SourceItemDetailsColl, function (itemDet) {
            if (itemDet.ProductId && itemDet.ProductId > 0) {
                ...
                var itemAllocation = {
                    ItemPhotoPath: itemDet.ItemPhotoPath,
    ...
    $scope.SetData = function (tran) {
        ...
        $scope.beData.ModifyDetailsColl = angular.copy(tran.TargetItemColl);
 ...
 $timeout(function () {                      
            ...
            if (curRow.RackList.length == 0) {
                ...
                $timeout(function () {
                    ...
                    if (curRow.GodownId > 0) {
                        GlobalServices.getRackList(curRow.GodownId).then(function (res) {
                            if (res.data.IsSuccess && res.data.Data) {
                                curRow.RackList = res.data.Data;
                            }
                        }, function (reason) {
                            Swal.fire('Failed' + reason);
                        });
    ...
    $scope.CurBOM = {};
    $scope.BOMChange = function (isModify)
    {
        $scope.CurBOM = {};
        if ($scope.beData.BOMTranId > 0) {
            var para = {
            ...
            $http({
                ...
                $timeout(function () {
                    if (res.data.IsSuccess && res.data.Data) {
                        ...
                        $scope.beData.FProductId = res.data.Data.ProductId;
                        $scope.beData.UnitId = res.data.Data.UnitId;
                        $scope.beData.FUnitId = res.data.Data.UnitId;

                        if (isModify == true) {
                            pQty = $scope.beData.Qty;
                            return;
                        }
                        else {
                            if ($scope.beData.POTranId > 0) {
                                ...
                                $scope.beData.Qty = res.data.Data.Qty;
                            }
                        }
    ...
    $scope.ChangeOrderQty = function () {
        if (!$scope.CurBOM || !$scope.CurBOM.ProductId) {
            ...
            if ($scope.beData.BOMTranId > 0) {
                var findBOM = mx($scope.BOMColl).firstOrDefault(p1 => p1.TranId == $scope.beData.BOMTranId);
                if (findBOM) {
                    $scope.CurBOM = findBOM;
                }
            }
        ...
        angular.forEach($scope.beData.SourceItemDetailsColl, function (itemDet) {
            ...
            if (findItem) {
                ...
                itemDet.ActualQty = findItem.Qty;
        ...
        $http({
            ...
            if (res.data.IsSuccess) {
                ...
                angular.forEach(res.data.DataColl, function (fd) {
                    ...
                    var isSouce = fd.ActualQty < 0 ? true : false;
                    fd.ActualQty = isSouce ? fd.ActualQty * -1 : fd.ActualQty;
                    fd.BilledQty = fd.ActualQty;
                    fd.Amount = isSouce ? fd.Amount * -1 : fd.Amount;
                    ...
                    if(isSouce)
                        ...
                        $scope.beData.TargetItemDetailsColl.push(refItem);
                });             
    ...
    $scope.GodownChange = function (gid, curRow, ind) {
        if (curRow && ind > 0) {
            if ($scope.beData) {
                if ($scope.beData.SourceItemDetailsColl) {
                    if ($scope.beData.SourceItemDetailsColl.length > (ind + 1)) {
                        var itemDet = $scope.beData.SourceItemDetailsColl[ind + 1];
    ...
    $scope.GodownChange1 = function (gid, curRow, ind) {
        if (curRow && ind > 0) {
            if ($scope.beData) {
                if ($scope.beData.TargetItemDetailsColl) {
                    if ($scope.beData.TargetItemDetailsColl.length > (ind + 1)) {
                        var itemDet = $scope.beData.TargetItemDetailsColl[ind + 1];
                        if (itemDet.GodownId > 0) {
    ...
    //Added by suresh for preview image and download it
    $scope.onFileChange = function (itemDet, files) {
        if (files && files.length > 0) {
            var file = files[0];
            itemDet.ItemFile = file;
            itemDet.ItemPhotoPath = URL.createObjectURL(file);
        }
    };

    $scope.ShowImagePreview = function (item) {
        $scope.viewImg = {
            ContentPath: '',
            FileType: null
        };

        if (item.ItemPhotoPath && item.ItemPhotoPath.length > 0) {
            $scope.viewImg.ContentPath = item.ItemPhotoPath;
            $scope.viewImg.FileType = 'image';
            $('#viewFiles').modal('show');
        } else if (item.ItemFile) {
            var blob = new Blob([item.ItemFile], { type: item.ItemFile.type });
            $scope.viewImg.ContentPath = URL.createObjectURL(blob);
            $scope.viewImg.FileType = item.ItemFile.type.startsWith('image/') ? 'image' : 'pdf';
