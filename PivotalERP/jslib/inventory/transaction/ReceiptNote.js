...
app.controller('ReceiptNoteController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    ...
    function LoadData() {
        ...
        $scope.RefVoucherTypeColl = [{ id: 1, text: 'Order' }, { id: 2, text: 'Quotation' }, { id: 3, text: 'Delivery Note' }];
        $scope.AgentColl = [];
        $http({
            method: 'GET',
        ...
        $scope.ProductLanedCostColl = [];
        $http({
            method: 'GET',
            url: base_url + "Account/Creation/GetCCForLandedCost",
            dataType: "json",
            //data: JSON.stringify(paraLC)
        }).then(function (resLC) {
            if (resLC.data.IsSuccess && resLC.data.Data) {
                $scope.ProductLanedCostColl = resLC.data.Data;
            }
        }, function (reason) {
            alert('Failed' + reason);
        });
 ...
 $scope.EPDet = {};
    ...
    $scope.GetItemDetailsFromSAPForGRN = function () {
        ...
        if ($scope.beData.SelectSAPTran) {
            if ($scope.beData.SelectSAPTran.TranId > 0) {
                ...
                $http({
                    method: 'GET',
                    url: base_url + "Inventory/Transaction/GetItemDetailsFromSAPForGRN?docEntry=" + $scope.beData.SelectSAPTran.TranId,
                    dataType: "json"
                }).then(function (res) {
                    if (res.data.IsSuccess && res.data.Data)
                    {
                        ...
                        $scope.beData.ReceiptNoteDetail.OtherReferences = $scope.beData.SelectSAPTran.AutoManualNo;

                        var itemColl = res.data.Data;
                        ...
                        angular.forEach(itemColl, function (newItem)
                        {
                            ...
                            $timeout(function () {
                                ...
                                $scope.beData.ItemDetailsColl[ind].ActualQty = newItem.ActualQty;
                                $scope.beData.ItemDetailsColl[ind].BilledQty = newItem.BilledQty;
                                $scope.beData.ItemDetailsColl[ind].Rate = newItem.Rate;
                                $scope.beData.ItemDetailsColl[ind].Amount = newItem.Amount;

                                $scope.beData.ItemDetailsColl[ind].Batch = newItem.BatchNo;

                                $scope.beData.ItemDetailsColl[ind].TranBaseUnitId = newItem.UnitId;
                                $scope.beData.ItemDetailsColl[ind].TranUnitId = newItem.UnitId;
                                $scope.beData.ItemDetailsColl[ind].TranUnitQty = newItem.ActualQty;
                                $scope.beData.ItemDetailsColl[ind].TranRate = newItem.Rate;
                                $scope.beData.ItemDetailsColl[ind].TranAmount = newItem.Amount;

                                $scope.beData.ItemDetailsColl[ind].TranBatch = newItem.BatchNo;
    ...
    $scope.ChangeTransactionUnit = function (itemDet) {
        ...
        if (itemDet.productDetail.AlternetUnitColl) {
            ...
            if (alternetUnit1 && findUnit) {
                ...
                itemDet.Rate = itemDet.TranAmount / baseQty;
                itemDet.Amount = amt;
        ...
        if ($scope.beData.ItemDetailsColl) {
            if ($scope.beData.ItemDetailsColl.length > ind + 1) {
                $scope.beData.ItemDetailsColl.splice(ind + 1, 0, {
                    ...
                    ProductLanedCostColl: angular.copy($scope.ProductLanedCostColl),
                })
            } else {
                $scope.beData.ItemDetailsColl.push({
                    RowType: 'P',
    ...
    $scope.ProductSelectionChange = function (itemDet, ind) {
        ...
        if (itemDet.ProductId == null) {
           ...
           // itemDet.Rate = itemDet.productDetail.PurchaseRate;
            ...
            if (itemDet.OrderItemAllocationId > 0 || itemDet.QuotationItemAllocationId > 0 || itemDet.DeliveryNoteItemAllocationId > 0 ) {
                refStockItem = true;
            }
            ...
            if ((isModify == false || isEmptyNum(itemDet.ItemAllocationId) == 0) && refStockItem == false && (itemDet.IsImport == false || !itemDet.IsImport)) {
                itemDet.Rate = itemDet.productDetail.PurchaseRate;
          ...
          //  itemDet.ProductLedgerId = itemDet.productDetail.PurchaseLedgerId;
           // itemDet.LedgerId = itemDet.productDetail.PurchaseLedgerId;
            ...
            if (itemDet.productDetail) {
                ...
                else if (itemDet.productDetail.IsFixedProduct == true)
                {
                    if (!itemDet.productDetail.FixedProductColl || itemDet.productDetail.FixedProductColl.length == 0) {
                        itemDet.productDetail.FixedProductColl = [];
                    }
                ...
                if ($scope.SelectedVoucher.Product.BatchNo == true && itemDet.productDetail.MultipleBatch == false && itemDet.productDetail.MaintainBatchWise == true) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            if (partyDet.PartyLedger) {
                ...
                if ($scope.lastPartyLedgerId != partyDet.PartyLedgerId && isModify == false) {
                    ...
                    partyDet.ReceiptNoteDetail = {};
                    $scope.lastPartyLedgerId = partyDet.PartyLedgerId;

                    partyDet.ReceiptNoteDetail.TermsOfPayment = partyDet.PartyLedger.PaymentType;

                    partyDet.ReceiptNoteDetail.Suppliers = partyDet.PartyLedger.Name;
                    partyDet.ReceiptNoteDetail.Address = partyDet.PartyLedger.Address;
                    partyDet.ReceiptNoteDetail.SalesTaxNo = partyDet.PartyLedger.PanVat;
                    partyDet.ReceiptNoteDetail.PhoneNo = partyDet.PartyLedger.MobileNo1;

                    if ($scope.beData.SelectSAPTran)
                        partyDet.ReceiptNoteDetail.OtherReferences = $scope.beData.SelectSAPTran.AutoManualNo;
            ...
            if ($scope.SelectedVoucher.ActivePartyDetails == true)
                $('#frmReceiptNoteDetailsModel').modal('show');
            ...
            $scope.RecalculateAdditioncalCost();
            $scope.CalculateTotalAndSubTotal();
            $('#frmReceiptNoteDetailsModel').modal('hide');
        }
    ...
    $scope.getRefVoucherPartyDetails = function (refItem) {
        ...
        if ($scope.beData.RefVoucherType && refItem && refItem.IsSelected == true) {
            ...
            //$scope.RefVoucherTypeColl = [{ id: 1, text: 'PurchaseOrder' }];
            var vType = 7;
            ...
            $http({
                ...
                if (res1.data.IsSuccess && res1.data.Data) {
                    ...
                    $scope.beData.ReceiptNoteDetail.TermsOfPayment = tranDet.TermsOfPayment;
                    $scope.beData.ReceiptNoteDetail.OtherReferences = tranDet.OtherRefereces;
                    $scope.beData.ReceiptNoteDetail.TermsOfDelivery = tranDet.TermsOfDelivery;
                    $scope.beData.ReceiptNoteDetail.DeliveryThrough = tranDet.DeliveryThrough;
                    $scope.beData.ReceiptNoteDetail.DeliveryDocNo = tranDet.DeliveryDocNo;
                    $scope.beData.ReceiptNoteDetail.Destination = tranDet.Destination;
                    $scope.beData.ReceiptNoteDetail.SalesQuotationTermsOfPayment = tranDet.SalesQuotationTermsOfPayment;
                    $scope.beData.ReceiptNoteDetail.SalesQuotationOtherRefereces = tranDet.SalesQuotationOtherRefereces;
                    $scope.beData.ReceiptNoteDetail.SalesQuotationTermsOfDelivery = tranDet.SalesQuotationTermsOfDelivery;
                    $scope.beData.ReceiptNoteDetail.Suppliers = tranDet.Buyes;
                    $scope.beData.ReceiptNoteDetail.Address = tranDet.Address;
                    $scope.beData.ReceiptNoteDetail.SalesTaxNo = tranDet.SalesTaxNo;
                    $scope.beData.ReceiptNoteDetail.CSTNumber = tranDet.CSTNumber;
                    $scope.beData.ReceiptNoteDetail.Notes = tranDet.Notes;
                    $scope.beData.ReceiptNoteDetail.PhoneNo = tranDet.PhoneNo;
                    $scope.beData.ReceiptNoteDetail.Description = tranDet.Description;
                    $scope.beData.ReceiptNoteDetail.OwnerName = tranDet.OwnerName;
                    $scope.beData.ReceiptNoteDetail.OwnerContactNo = tranDet.OwnerContactNo;
                    $scope.beData.ReceiptNoteDetail.DriverAddress = tranDet.DriverAddress;
                    $scope.beData.ReceiptNoteDetail.DriverName = tranDet.DriverName;
                    $scope.beData.ReceiptNoteDetail.DriverContactNo = tranDet.DriverContactNo;
                    $scope.beData.ReceiptNoteDetail.LicenseNo = tranDet.LicenseNo;
                    $scope.beData.ReceiptNoteDetail.Goods = tranDet.Goods;
                    $scope.beData.ReceiptNoteDetail.Quantity = tranDet.Quantity;
                    $scope.beData.ReceiptNoteDetail.FreightRate = tranDet.FreightRate;
                    $scope.beData.ReceiptNoteDetail.AdvancePayment = tranDet.AdvancePayment;
                    $scope.beData.ReceiptNoteDetail.TotalWT = tranDet.TotalWT;
                    $scope.beData.ReceiptNoteDetail.ContactNo = tranDet.ContactNo;
                    $scope.beData.ReceiptNoteDetail.RegdNo = tranDet.RegdNo;
                    $scope.beData.ReceiptNoteDetail.EngineNo = tranDet.EngineNo;
                    $scope.beData.ReceiptNoteDetail.ChassisNo = tranDet.ChassisNo;
                    $scope.beData.ReceiptNoteDetail.Model = tranDet.Model;
                    $scope.beData.ReceiptNoteDetail.VinNo = tranDet.VinNo;
                    $scope.beData.ReceiptNoteDetail.CreditDays = tranDet.CreditDays;
                    $scope.beData.ReceiptNoteDetail.ExportCountry = tranDet.ExportCountry;
                    $scope.beData.ReceiptNoteDetail.PPNo = tranDet.PPNo;
                    $scope.beData.ReceiptNoteDetail.PPDate = tranDet.PPDate;
                    $scope.beData.ReceiptNoteDetail.OtherRefereces1 = tranDet.OtherRefereces1;
                    $scope.beData.ReceiptNoteDetail.PaymentTermsId = tranDet.PaymentTermsId;
                    $scope.beData.ReceiptNoteDetail.FreightTypeId = tranDet.FreightTypeId;
        ...
        if (filterData.length > 0) {
            ...
            angular.forEach(filterData, function (fd) {
                var refItem = {
                    ...
                    RateDecimal: 2,
                    AmountDecimal: 2,
                    GodownId: (refVType == 3 ?  $scope.beData.GodownId :  (fd.GodownId && fd.GodownId > 0 ? fd.GodownId : $scope.beData.GodownId)),
                    ...
                    DeliveryNoteItemAllocationId: refVType == 3 ? fd.ItemAllocationId : null,
                    RefVType: refVType,
                ...
                $http({
                    ...
                    if (res1.data.IsSuccess == true) {
                        ...
                        angular.forEach(res1.data.Data.ItemAllocationColl, function (ias) {
                            ...
                            if (refVType == 3)
                                ias.GodownId = $scope.beData.GodownId;
                            ...
                            ias.ProductLanedCostColl = angular.copy($scope.ProductLanedCostColl);
    ...
    $scope.RefVoucherChange = function (refVType) {
        ...
        var funName = "getPendingPurchaseOrder";
        ...
        var para = "ledgerId=" + $scope.beData.PartyLedgerId + "&agentId=" + agentId + "&voucherDate=" + vDate+"&branchId="+$scope.SelectedVoucher.BDId;
        $http({
            method: 'GET',
    ...
    $scope.ChangeProductLandedCost = function (itemDet, col) {
         
        GlobalServices.getItemLandedCostFormula(itemDet, $scope.beData.ItemDetailsColl, $scope.beData, col);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $timeout(function () {
                        ...
                        $http({
                            method: 'POST',
                            url: base_url + "Inventory/Transaction/SaveUpdateReceiptNote",
                            headers: { 'Content-Type': undefined },
    ...
    $scope.GetTransactionById = function (tran) {
        if (tran.TranId && tran.TranId > 0) {
            ...
            $timeout(function () {
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/GetReceiptNoteById",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    if (res.data.IsSuccess && res.data.Data) {
    ...
    $scope.GetData = function () {
        ...
        var tmpSales = {
            ...
            ReferanceTranId: ($scope.beData.ReferanceTranId ? $scope.beData.ReferanceTranId : 0),
            ...
            AditionalCostColl: [],
            ReceiptNoteDetail: $scope.beData.ReceiptNoteDetail,
            GodownId: $scope.beData.GodownId,
        ...
        if (tmpSales.ReceiptNoteDetail) {
            if (tmpSales.ReceiptNoteDetail.FreightRate == undefined || tmpSales.ReceiptNoteDetail.FreightRate == null || tmpSales.ReceiptNoteDetail.FreightRate === undefined)
                tmpSales.ReceiptNoteDetail.FreightRate = 0;

            if (tmpSales.ReceiptNoteDetail.AdvancePayment == undefined || tmpSales.ReceiptNoteDetail.AdvancePayment == null || tmpSales.ReceiptNoteDetail.AdvancePayment === undefined)
                tmpSales.ReceiptNoteDetail.AdvancePayment = 0;

            if (tmpSales.ReceiptNoteDetail.CreditDays == undefined || tmpSales.ReceiptNoteDetail.CreditDays == null || tmpSales.ReceiptNoteDetail.CreditDays === undefined)
                tmpSales.ReceiptNoteDetail.CreditDays = 0;

            if (tmpSales.ReceiptNoteDetail.PartyInvoiceDateDet) {
                tmpSales.ReceiptNoteDetail.PartyInvoiceDate = $filter('date')(new Date(tmpSales.ReceiptNoteDetail.PartyInvoiceDateDet.dateAD), 'yyyy-MM-dd');
            }
        }
                        ...
                        itemAllocation.ItemDetailsColl.push(
                            {
                                ...
                                LotNo: itemDet.LotNo,
                                BatchCon: itemDet.BatchCon,
                                BatchRemarks: itemDet.BatchRemarks,
                                RackId: itemDet.RackId,
    ...
    $scope.SetData = function (tran) {
        ...
        if (tran.ReceiptNoteDetail.PartyInvoiceDate)
            tran.ReceiptNoteDetail.PartyInvoiceDate_TMP = new Date(tran.ReceiptNoteDetail.PartyInvoiceDate);
        ...
        $scope.beData.ReceiptNoteDetail = tran.ReceiptNoteDetail;
 ...
 <div class="input-group-prepend">
    ...
    $scope.getVoucherNo = function () {
        ...
        $scope.ClearItemDetails();
        $scope.beData.ReceiptNoteDetail = {};
 ...
 $timeout(function () {                      
                    ...
                    TextColumn: udf.TextColumn,
                                ...
                                if ($scope.SelectedVoucher.VoucherProductUDFColl && $scope.SelectedVoucher.VoucherProductUDFColl.length > 0) {
                                    angular.forEach($scope.beData.ItemDetailsColl, function (det) {
                                        det.UDFFeildsColl = [];
                                        det.ProductLanedCostColl = angular.copy($scope.ProductLanedCostColl);
            ...
            if (res.data.IsSuccess) {
                ...
                angular.forEach(res.data.DataColl, function (fd) {
                    ...
                    var refItem = {
                        ...
                        QuotationItemAllocationId: null,
    ...
    $scope.ShowPendingVoucherList = function () {
        ...
        $timeout(function () {
            ...
            $http({
                method: "GET",
                url: base_url + "Inventory/Transaction/getPendingDNForGRND?branchId=" + $scope.SelectedVoucher.BDId,
                dataType: "json"
            }).then(function (res) {
                ...
                if (res.data.IsSuccess && res.data.Data) {
                    ...
                    $scope.PendingVoucherColl = dataColl;
    ...
    $scope.ChangeRefTranSelected = function (ind, tran) {
        ...
        angular.forEach($scope.PendingVoucherColl, function (ra) {
            ...
            if (ra.ReferanceTranId != tran.ReferanceTranId)
                ra.IsSelected = false;
    ...
    $scope.MakeReceiptNote = function () {
        ...
        if (selectedItem) {
            ...
            var funName = "getDNItemDetForGRND?docEntry=" + selectedItem.ReferanceTranId;               
            $http({
                method: 'GET',
                url: base_url + "Inventory/Transaction/" + funName ,
                dataType: "json"
            }).then(function (res1) {
                ...
                if (res1.data.IsSuccess && res1.data.Data) {
                    ...
                    if (RefItemAllocationColl && RefItemAllocationColl.length > 0) {
                        var freightAmt = 0;
                        if (selectedItem.ReceiptNoteDetail.FreightRate > 0)
                            freightAmt = selectedItem.ReceiptNoteDetail.FreightRate;

                        $scope.beData.PartyLedger = null;
                        $scope.beData.RefNo = selectedItem.VoucherNo;
                        $scope.beData.PartyLedgerId = selectedItem.PartyLedgerId;
                        $scope.beData.RefNo = selectedItem.AutoManualNo;
                        $scope.beData.Narration = selectedItem.RefNo;
                        $scope.beData.VoucherDate_TMP = new Date(selectedItem.VoucherDate);
                        $scope.beData.EntryDate_TMP = new Date();
                        $scope.beData.ReferanceTranId = selectedItem.ReferanceTranId;
                        $scope.beData.ReceiptNoteDetail.FreightRate = freightAmt;
                        ...
                        angular.forEach(RefItemAllocationColl, function (fd) {
                            var refItem = {
                                ...
                                AmountDecimal: 2,
                                Batch: fd.Batch,
                                EngineNo:fd.EngineNo,
                                GodownId: (fd.GodownId && fd.GodownId > 0 ? fd.GodownId : $scope.beData.GodownId),
                                Narration: fd.Narration,
                                RefQty: fd.ActualQty,
                                BatchCon: fd.BatchCon,
                                LotNo: fd.LotNo,
                                BatchRemarks:fd.BatchRemarks,
                                IsImport:true,
                            };
                            $scope.beData.ItemDetailsColl.push(refItem);
                        });
                        ...
                        angular.forEach(adCostColl, function (ads) {
                            $scope.beData.ItemDetailsColl.push({
                                ...
                                SchameAmt: 0,
                                SchamePer: 0,
                                Amount: (ads.LedgerId == 12 ? freightAmt :  ads.Amount),
                                Description: '',
                                QtyPoint: 0,
                                UnitId: null,
                                AutoCharge: true,
                        ...
                        if ($scope.beData.PartyLedger) {
                            $scope.beData.ReceiptNoteDetail.Buyes = $scope.beData.PartyLedger.Name;
                            $scope.beData.ReceiptNoteDetail.Address = $scope.beData.PartyLedger.Address;
                            $scope.beData.ReceiptNoteDetail.SalesTaxNo = $scope.beData.PartyLedger.PanVat;
                            $scope.beData.ReceiptNoteDetail.PhoneNo = $scope.beData.PartyLedger.MobileNo1;                             
                        }
                    }
                }
            }, function (reason) {
    ...
    $scope.ChangeTermsOfPayment = function () {
        if ($scope.beData && $scope.beData.ReceiptNoteDetail) {
            if ($scope.beData.ReceiptNoteDetail.TermsOfPayment) {
                var findPM = $scope.PaymentModeColl_Qry.firstOrDefault(p1 => p1.Name == $scope.beData.ReceiptNoteDetail.TermsOfPayment);
                if (findPM) {
                    $scope.beData.ReceiptNoteDetail.TypeOfPayment = findPM.TypeOfPayment;
                    $scope.beData.ReceiptNoteDetail.PaymentTermsId = findPM.PaymentModeId;
                }
            }
        }
    }
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SaveReceiptNote);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
