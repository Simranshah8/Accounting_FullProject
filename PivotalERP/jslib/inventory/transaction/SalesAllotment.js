...
app.controller('SalesAllotmentController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    $scope.Title = 'Sales Allotment';
    ...
    function connectPrintServer() {
        ...
        var websocket = new WebSocketEx('ws://127.0.0.1:12321', '', function () {
            ...
            alert('Unable To Connect With Server. Connection closed.');
        }, function (evt) {
            // On MSG Received
            console.log(evt);
 ...
 $scope.EPDet = {};
  ...
  itemDet.ProductName = itemDet.productDetail.Name;
    ...
    $scope.PartySelectionChange = function (partyDet) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            ...
            if ($scope.SelectedVoucher.ActivePartyDetails == true && $scope.comDet.Maintain != 3) {
                ...
                $('#frmSalesAllotmentDetailsModel').modal('show');
            }
            ...
            $scope.RecalculateAdditioncalCost();
            $scope.CalculateTotalAndSubTotal();
            $('#frmSalesAllotmentDetailsModel').modal('hide');
        }
    };
    ...
    $scope.SaveSalesAllotment = function () {
        ...
        if ($scope.IsValidData() == true) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/SaveUpdateSalesAllotment",
    ...
    $scope.IsValidData = function () {
        ...
        if ($scope.SelectedVoucher.MandatoryCreditDays == true) {
            if (!$scope.beData.SalesInvoiceDetail || !$scope.beData.SalesInvoiceDetail.CreditDays || $scope.beData.SalesInvoiceDetail.CreditDays.length == 0) {
                if ($scope.beData.SalesAllotmentDetail.Buyes != 'CASH SALES PARTY') {
                    result = false;
 ...
 $timeout(function () {                      
    ...
    $scope.GetTransactionById = function (tran) {
        $timeout(function () {
            ...
            if (tran.TranId && tran.TranId > 0) {
                ...
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/GetSalesAllotmentById",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    $timeout(function () {
        ...
        if ($scope.beData.RefVoucherType == 2) {
            ...
            $http({
                ...
                if (res.data.IsSuccess && res.data.Data) {
                    ...
                    $('#newfrmSalesAllotmentDetailsModel').modal('show');
    ...
    $scope.MakeSalesInvice = function () {
        ...
        if (selectedItem && selectedItem.ItemColl && selectedItem.ItemColl.length > 0 && refVType>0) {
                        ...
                        $timeout(function () {
                            ...
                            $http({
                                ...
                                if (res1.data.IsSuccess == true) {
                                    ...
                                    $('#newfrmSalesAllotmentDetailsModel').modal('hide');
                                }
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SaveSalesAllotment);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
