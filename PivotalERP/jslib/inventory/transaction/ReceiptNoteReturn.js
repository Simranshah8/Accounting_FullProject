app.controller('ReceiptNoteReturnController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    ...
    function LoadData() {
        ...
        $scope.RefVoucherTypeColl = [{ id: 1, text: 'ReceiptNote' }];
  ...
  itemDet.ProductName = itemDet.productDetail.Name;
    ...
    $scope.PartySelectionChange = function (partyDet) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            ...
            if ($scope.SelectedVoucher.ActivePartyDetails == true)
                $('#frmReceiptNoteReturnDetailsModel').modal('show');
        } else {
            ...
            $scope.RecalculateAdditioncalCost();
            $scope.CalculateTotalAndSubTotal();
            $('#frmReceiptNoteReturnDetailsModel').modal('hide');
        }
    ...
    $scope.SaveReceiptNoteReturn = function () {
        ...
        if ($scope.IsValidData() == true) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/SaveUpdateReceiptNoteReturn",
                        headers: { 'Content-Type': undefined },
    ...
    $scope.GetTransactionById = function (tran) {
        $timeout(function () {
            ...
            if (tran.TranId && tran.TranId > 0) {
                ...
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/GetReceiptNoteReturnById",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    $timeout(function () {
 ...
 $timeout(function () {                      
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SaveReceiptNoteReturn);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
