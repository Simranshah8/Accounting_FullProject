app.controller('ManufacturingStockJournal', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    $scope.Title = 'ManufacturingStockJournal';
    ...
    function LoadData() {
        ...
        $scope.beData.ProductionDetailsColl.push(
            {
                ProductId: 0,
                productDetail: null,
                ActualQty: 0,
    ...
    $scope.AddRowInProductionDetails = function (ind, boolAuto) {

        if (boolAuto == true) {
            var len = $scope.beData.ProductionDetailsColl.length;
            if ((ind + 1) != len)
                return;

            var selectItem = $scope.beData.ProductionDetailsColl[ind];
            if (!selectItem.ProductId || selectItem.ProductId == null || selectItem.ProductId == 0)
                return;

        }

        if ($scope.beData.ProductionDetailsColl) {
            if ($scope.beData.ProductionDetailsColl.length > ind + 1) {
                $scope.beData.ProductionDetailsColl.splice(ind + 1, 0, {
                    ProductId: 0,
                    productDetail: null,
                    ActualQty: 0,
                    BilledQty: 0,
    ...
    $scope.delRowFromProductionDetails = function (ind) {
        if ($scope.beData.ProductionDetailsColl) {
            if ($scope.beData.ProductionDetailsColl.length > 1) {
                $scope.beData.ProductionDetailsColl.splice(ind, 1);
            }
        }
    }
 ...
 $scope.reloadVoucherDate = function () {
    ...
    $scope.GetStockJournalById = function (beData) {
    ...
    $scope.DelClassById = function (beData, ind) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var para = { TranId: beData.TranId };
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/DelClassById",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    hidePleaseWait();
                    ...
                    Swal.fire(res.data.ResponseMSG);
                    if (res.data.IsSuccess == true) {
                        $scope.VoucherTypeColl.splice(ind, 1);
                    }
                }, function (reason) {
                    Swal.fire('Failed' + reason);
                });
