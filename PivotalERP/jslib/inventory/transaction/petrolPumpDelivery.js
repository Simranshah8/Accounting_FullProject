app.controller("PetrolPumpDelivery", function ($scope, $http, $timeout, $filter, $compile,GlobalServices, $document) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            if (partyDet.PartyLedger) {
                if (partyDet.DeliveryNoteDetail) {
                    ...
                    partyDet.DeliveryNoteDetail.BuyesId = null;
                }
            }
    ...
    $scope.ChangeBuyerSelection = function () {
        ...
        if ($scope.beData.DeliveryNoteDetail.Buyes && $scope.beData.DeliveryNoteDetail.Buyes.length > 2) {
            var findP = $scope.CashPartyColl_Qry.firstOrDefault(p1 => p1.Buyes == $scope.beData.DeliveryNoteDetail.Buyes);
            if (findP) {
                $scope.beData.DeliveryNoteDetail.SalesTaxNo = findP.SalesTaxNo;
                $scope.beData.DeliveryNoteDetail.Address = findP.Address;
                $scope.beData.DeliveryNoteDetail.PhoneNo = findP.PhoneNo;
            }
        }
    }
    ...
    $scope.GetData = function () {
        ...
        if (tmpSales.DeliveryNoteDetail) {
            ...
            tmpSales.DeliveryNoteDetail.TermsOfPayment = $scope.beData.BillingAs == 1 ? 'CASH' : 'CREDIT';            
            if (tmpSales.DeliveryNoteDetail.Buyes && $scope.beData.BillingAs==1) {
                tmpSales.DeliveryNoteDetail.Buyes = 'CASH( ' + tmpSales.DeliveryNoteDetail.Buyes + ')';
            }
        }
