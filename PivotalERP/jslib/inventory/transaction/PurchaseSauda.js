...
app.controller('PurchaseSaudaController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    ...
    window.addEventListener('keydown', function (event) {
        if (event.key === 'Escape') {
            $('#frmPurchaseSaudaDetailsModel').modal('hide');
        }
    })
    ...
    function LoadData() {
        ...
        $scope.RowTypeColl = [{ id: 1, text: 'Product' }, { id: 2, text: 'Group' }];
        //Standard, CashDiscount, DN_CN_AppliedOnly, DN_CN_CreditNote, FormulaBased
        $scope.RebateTypeColl = [{ id: 1, text: 'Standard' }, { id: 2, text: 'CashDiscount' }, { id: 3, text: 'DN_CN_AppliedOnly' }, { id: 4, text: 'DN_CN_CreditNote' }, { id: 5, text: 'FormulaBased' }];
        //% / Fixed / PerUnit
        $scope.RebateTypeCalColl = [{ id: 1, text: 'Per(%)' }, { id: 2, text: 'Fixed' }, { id: 3, text: 'PerUnit' }]

        //Invoice / Payment
        $scope.RebateApplicableOnColl = [{ id: 1, text: 'Invoice' }, { id: 2, text: 'Payment' },];
        ...
        $scope.VoucherStatusColl = GlobalServices.getVoucherStatus();
        ...
        $scope.VoucherSearchOptions = [{ text: 'PartyName', value: 'ADS.Buyes', searchType: 'text' }, { text: 'PanVat', value: 'ADS.SalesTaxNo', searchType: 'text' }, { text: 'PartyLedger', value: 'Led.Name', searchType: 'text' }, { text: 'RefNo', value: 'TS.[No]', searchType: 'text' }, { text: 'Sauda No.', value: 'TS.AutoManualNo', searchType: 'text' }, { text: 'Voucher', value: 'V.VoucherName', searchType: 'text' }, { text: 'CostClass', value: 'CC.Name', searchType: 'text' }, { text: 'VoucherDate', value: 'TS.VoucherDate', searchType: 'date' }, { text: 'Amount', value: 'TS.TotalAmount', searchType: 'number' },
            ...
            if (res.data.IsSuccess && res.data.Data) {
                ...
                if ($scope.EPDet) {
                    ...
                    for (key in $scope.EPDet) {
                        ...
                        else if (key.startsWith("PurchaseSaudaDetail") == true) {
                            var proName = key.replace("PurchaseSaudaDetail.", "");

                            var findEP = $scope.EPDet[key];
                            if (findEP && findEP.DefaultValue) {
                                $scope.beData.PurchaseSaudaDetail[proName] = findEP.DefaultValue;
                            }
                        }
                    }
    ...
    $scope.ChangeBuyerSelection = function () {
        ...
        if ($scope.beData.PurchaseSaudaDetail.Buyes && $scope.beData.PurchaseSaudaDetail.Buyes.length > 2) {
            var findP = $scope.CashPartyColl_Qry.firstOrDefault(p1 => p1.Buyes == $scope.beData.PurchaseSaudaDetail.Buyes);
            if (findP) {
                $scope.beData.PurchaseSaudaDetail.SalesTaxNo = findP.SalesTaxNo;
                $scope.beData.PurchaseSaudaDetail.Address = findP.Address;
                $scope.beData.PurchaseSaudaDetail.PhoneNo = findP.PhoneNo;
            }
        }
    }
    ...
    $scope.ProductSelectionChange = function (itemDet, ind) {
        ...
        if (itemDet.ProductId == null || itemDet.ProductId == 0) {
            ...
            if (itemDet.productDetail.IsFixedProduct == true) {
                if (!itemDet.FixedProductColl || itemDet.FixedProductColl.length == 0 || (!itemDet.productDetail.FixedProductColl && ((!itemDet.FixedProductColl || itemDet.FixedProductColl.length == 0) && $scope.SelectedVoucher.MultipleFixedProduct == true))) {
                    $http({
                        method: 'GET',
    ...
    $scope.PartySelectionChange = function (partyDet) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
                ...
                if ($scope.lastPartyLedgerId != partyDet.PartyLedgerId && isModify == false) {
                    ...
                    if ($scope.comDet.Maintain != 3) {
                        ...
                        if ($scope.HideShow.ActiveSummaryEntry != false)
                            partyDet.PurchaseSaudaDetail = {};

                        if (isModify == false && $scope.lastPartyLedgerId != partyDet.PartyLedgerId)
                            partyDet.PurchaseSaudaDetail.TermsOfPayment = partyDet.PartyLedger.PaymentType;

                        if ($scope.EPDet) {
                            for (key in $scope.EPDet) {
                                if (key.startsWith("PurchaseSaudaDetail") == true) {
                                    var proName = key.replace("PurchaseSaudaDetail.", "");
                    ...
                    if (partyDet.PartyLedger.BillToColl && partyDet.PartyLedger.BillToColl.length > 0)
                        partyDet.PurchaseSaudaDetail.BillToId = partyDet.PartyLedger.BillToColl[0].Id;

                    if (partyDet.PartyLedger.ShipToColl && partyDet.PartyLedger.ShipToColl.length > 0)
                        partyDet.PurchaseSaudaDetail.ShipToId = partyDet.PartyLedger.ShipToColl[0].Id;

                    $scope.ChangeBillAddress();
                    $scope.ChangeShipAddress();

                    if (isModify == false)
                        partyDet.PurchaseSaudaDetail.CreditDays = partyDet.PartyLedger.CreditLimitDays;

                    if ($scope.comDet.Maintain == 3) {

                        var isCash = false;
                        var pt = mx($scope.PaymentTermColl_Qry).firstOrDefault(p1 => p1.id == $scope.beData.PurchaseSaudaDetail.PaymentTermsId);
                        if (pt)
                            isCash = pt.IsCash;

                        if (isCash == false) {
                            partyDet.PurchaseSaudaDetail.Buyes = partyDet.PartyLedger.Name;
                            partyDet.PurchaseSaudaDetail.Address = partyDet.PartyLedger.Address;
                            partyDet.PurchaseSaudaDetail.SalesTaxNo = partyDet.PartyLedger.PanVat;
                            partyDet.PurchaseSaudaDetail.PhoneNo = partyDet.PartyLedger.MobileNo1;
                        } else {
                            if (!partyDet.PurchaseSaudaDetail.Buyes)
                                partyDet.PurchaseSaudaDetail.Buyes = partyDet.PartyLedger.Name;

                            if (!partyDet.PurchaseSaudaDetail.Address)
                                partyDet.PurchaseSaudaDetail.Address = partyDet.PartyLedger.Address;

                            if (!partyDet.PurchaseSaudaDetail.SalesTaxNo)
                                partyDet.PurchaseSaudaDetail.SalesTaxNo = partyDet.PartyLedger.PanVat;

                            if (!partyDet.PurchaseSaudaDetail.PhoneNo)
                                partyDet.PurchaseSaudaDetail.PhoneNo = partyDet.PartyLedger.MobileNo1;
                        }
                    ...
                    partyDet.PurchaseSaudaDetail.BuyesId = null;
            ...
            //    $('#frmPurchaseSaudaDetailsModel').modal('show');
            //}
    ...
    $scope.ChangeBillAddress = function () {
        ...
        if ($scope.beData.PartyLedger.BillToColl) {
            var find = mx($scope.beData.PartyLedger.BillToColl).firstOrDefault(p1 => p1.Id == $scope.beData.PurchaseSaudaDetail.BillToId);
            if (find) {
                $scope.beData.PurchaseSaudaDetail.BillToAddress = find.ConcatAddress;
            }
        }
    }
    $scope.ChangeShipAddress = function () {
        if ($scope.beData.PartyLedger.ShipToColl) {
            var find = mx($scope.beData.PartyLedger.ShipToColl).firstOrDefault(p1 => p1.Id == $scope.beData.PurchaseSaudaDetail.ShipToId);
            if (find) {
                $scope.beData.PurchaseSaudaDetail.ShipToAddress = find.ConcatAddress;
            }
        }
    }
    ...
    $scope.CalculateTotalAndSubTotal = function () {
        ...
        if ($scope.SelectedVoucher) {
            ...
            if ($scope.beData.PurchaseSaudaDetail && $scope.beData.PurchaseSaudaDetail.SSF) {
                var per = isEmptyAmt($scope.beData.PurchaseSaudaDetail.SSF.SchemePer);
    ...
    $scope.SavePurchaseSauda = function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $timeout(function () {
                            ...
                            $http({
                                method: 'POST',
                                url: base_url + "Inventory/Transaction/SaveUpdatePurchaseSauda",
    ...
    $scope.IsValidData = function () {
        ...
        if (!$scope.beData.SigningDateDet) {
            result = false;
            Swal.fire('Please ! Enter Signing Date');
            return result;
        }
        ...
        if ($scope.comDet.Maintain == 3) {
            if (!$scope.beData.PurchaseSaudaDetail.PaymentTermsId || $scope.beData.PurchaseSaudaDetail.PaymentTermsId == 0) {
                result = false;
        ...
        if ($scope.SelectedVoucher.MandatoryCreditDays == true) {
            if (!$scope.beData.PurchaseSaudaDetail || !$scope.beData.PurchaseSaudaDetail.CreditDays || $scope.beData.PurchaseSaudaDetail.CreditDays.length == 0) {
                if ($scope.beData.PurchaseSaudaDetail.Buyes != 'CASH SALES PARTY') {
                    result = false;
        ...
        if ($scope.SelectedVoucher.AllowCashPurchaseSale == true && ($scope.beData.PurchaseSaudaDetail.TermsOfPayment == 'CASH' || $scope.beData.PurchaseSaudaDetail.TypeOfPayment == 1 || $scope.beData.PurchaseSaudaDetail.TypeOfPayment == 2)) {
    ...
    $scope.GetData = function () {
        ...
        var tmpSales = {
            ...
            RebateColl:angular.copy($scope.beData.RebateColl),
            ItemAllocationColl: [],
            // AditionalCostColl: $scope.beData.AditionalCostColl,
            AditionalCostColl: [],
            PurchaseSaudaDetail: $scope.beData.PurchaseSaudaDetail,
            GodownId: $scope.beData.GodownId,
            Attributes: '',
            PaymentTermColl: [],
            ...
            DealBy: $scope.beData.DealBy,
            DealNumber: $scope.beData.DealNumber,
            StartDate: $scope.beData.StartDateDet ? $filter('date')(new Date($scope.beData.StartDateDet.dateAD), 'yyyy-MM-dd') : null,
            EndDate: $scope.beData.EndDateDet ? $filter('date')(new Date($scope.beData.EndDateDet.dateAD), 'yyyy-MM-dd') : null,            
            AgreementType: $scope.beData.AgreementType,
            SigningDate: $scope.beData.SigningDateDet ? $filter('date')(new Date($scope.beData.SigningDateDet.dateAD), 'yyyy-MM-dd') : null,
        };
        ...
        if (tmpSales.PurchaseSaudaDetail) {
            if (tmpSales.PurchaseSaudaDetail.FreightRate == undefined || tmpSales.PurchaseSaudaDetail.FreightRate == null || tmpSales.PurchaseSaudaDetail.FreightRate === undefined)
                tmpSales.PurchaseSaudaDetail.FreightRate = 0;

            if (tmpSales.PurchaseSaudaDetail.AdvancePayment == undefined || tmpSales.PurchaseSaudaDetail.AdvancePayment == null || tmpSales.PurchaseSaudaDetail.AdvancePayment === undefined)
                tmpSales.PurchaseSaudaDetail.AdvancePayment = 0;

            if (tmpSales.PurchaseSaudaDetail.CreditDays == undefined || tmpSales.PurchaseSaudaDetail.CreditDays == null || tmpSales.PurchaseSaudaDetail.CreditDays === undefined)
                tmpSales.PurchaseSaudaDetail.CreditDays = 0;
        } else {
            tmpSales.PurchaseSaudaDetail = {};
            tmpSales.PurchaseSaudaDetail.FreightRate = 0;
            tmpSales.PurchaseSaudaDetail.AdvancePayment = 0;
            tmpSales.PurchaseSaudaDetail.CreditDays = 0;
        }
        ...
        var ssfPer = 0;
        if ($scope.beData.PurchaseSaudaDetail.SSF) {
            if ($scope.beData.PurchaseSaudaDetail.SSF.SchamePer > 0) {
                ssfPer = $scope.beData.PurchaseSaudaDetail.SSF.SchamePer;
            }
        }

        if (tmpSales.PaymentTermColl) {
            var vid = $scope.beData.PurchaseSaudaDetail.ReceiptVoucherId;
            if (!vid) {
                if ($scope.RecVoucherTypeColl && $scope.RecVoucherTypeColl.length > 0)
        ...
        angular.forEach($scope.beData.ItemDetailsColl, function (itemDet) {
            if (((itemDet.ProductId && itemDet.ProductId > 0) || (itemDet.ProductGroupId && itemDet.ProductGroupId > 0)) && (itemDet.RowType == 'P' || itemDet.RowType == 'S')) {
                ...
                var itemAllocation = {
                    ...
                    RowTypes:itemDet.RowTypeId,
                    ProductId: isEmptyNum(itemDet.ProductId),
                    ProductGroupId:itemDet.ProductGroupId,
                    ...
                    UnitId: (itemDet.UnitId==null || itemDet.UnitId==undefined ? 1 : itemDet.UnitId),
                ...
                if (itemDet.productDetail && itemDet.FixedProductColl && itemDet.FixedProductColl.length > 0 && $scope.SelectedVoucher.MultipleFixedProduct == true && itemDet.FixedProductColl[0].EngineNo && itemDet.FixedProductColl[0].EngineNo.length > 0) {
                    var fCount = 0;
                    var disAmtSep = 0
                    angular.forEach(itemDet.FixedProductColl, function (fp) {
                ...
                else if (itemDet.productDetail && itemDet.productDetail.BatchWiseColl && itemDet.productDetail.BatchWiseColl.length > 0 && itemDet.productDetail.MultipleBatch == true) {
                    ...
                    itemAllocation.ItemDetailsColl.push(
                        {
                            RowType:itemAllocation.RowType,
                            ProductId: itemAllocation.ProductId,
                            ProductGroupId:itemAllocation.ProductGroupId,
    ...
    $scope.SetData = function (tran) {
        ...
        $scope.beData.DealBy = tran.DealBy;
        $scope.beData.DealNumber = tran.DealNumber;

        if (tran.StartDate) {
            $scope.beData.StartDate = new Date(tran.StartDate);
            $scope.beData.StartDate_TMP = new Date(tran.StartDate);
        }

        if (tran.EndDate) {
            $scope.beData.EndDate = new Date(tran.EndDate);
            $scope.beData.EndDate_TMP = new Date(tran.EndDate);
        }
        
        $scope.beData.AgreementType = tran.AgreementType;

        if (tran.SigningDate) {
            $scope.beData.SigningDate = new Date(tran.SigningDate);
            $scope.beData.SigningDate_TMP = new Date(tran.SigningDate);
        }
        ...
        $scope.beData.PurchaseSaudaDetail = tran.PurchaseSaudaDetail;
        $scope.beData.PurchaseSaudaDetail = tran.PurchaseSaudaDetail;
        $scope.beData.ItemDetailsColl = [];
        $scope.beData.RebateColl = tran.RebateColl;

        if (!$scope.beData.RebateColl || $scope.beData.RebateColl.lengh == 0) {
            $scope.addRowInRB(0);
        }
        ...
        angular.forEach(tran.ItemAllocationColl, function (itemA) {
            ...
            itemA.RowTypeId = itemA.RowTypes;
            ...
            if (itemA.MultipleFixedProduct == true) {
                ...
                if (itemA.ItemDetailsColl.length > 1) {
                    ...
                    angular.forEach(itemA.ItemDetailsColl, function (itemAD) {
                        ...
                        itemAD.RowTypeId = itemA.RowTypeId;
                ...
                if ($scope.beData.CostClassId && $scope.beData.CostClassId > 0) {
                    ...
                    $http({
                        ...
                        if (res.data.IsSuccess && res.data.Data) {
                            ...
                            $scope.beData.DealNumber = vDet.AutoVoucherNo;
 ...
 <div class="input-group-prepend">
    ...
    $scope.getVoucherNo = function () {
        ...
        $scope.ClearItemDetails();
        $scope.beData.PurchaseSaudaDetail = {};
        $scope.beData.PartyLedgerId = null;
        $scope.beData.AditionalCostColl = [];
                        ...
                        $scope.$apply(function () {
                            if ($scope.SelectedVoucher) {
                                ...
                                if ($scope.SelectedVoucher.ActiveSummaryEntry == true) {
                                    ...
                                    if (!$scope.beData.PurchaseSaudaDetail)
                                        $scope.beData.PurchaseSaudaDetail = {};

                                    $scope.beData.PurchaseSaudaDetail.Buyes = 'CASH';
                                    $scope.beData.PurchaseSaudaDetail.Address = 'N/A';

                                    var findBT = mx($scope.PaymentTermColl).firstOrDefault(p1 => p1.Name == 'CASH');
                                    if (findBT) {
                                        $scope.beData.PurchaseSaudaDetail.PaymentTermsId = findBT.id;
    ...
    $scope.ShowBillImg = function () {
        ...
        var findI = $scope.PaymentTermColl_Qry.firstOrDefault(p1 => p1.id == $scope.beData.PurchaseSaudaDetail.PaymentTermsId);
        if (findI) {
            $scope.viewImg = {
    ...
    $scope.GetTransactionById = function (tran) {
        if (tran.TranId && tran.TranId > 0) {
            ...
            $timeout(function () {
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/GetPurchaseSaudaById",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    $timeout(function () {
    ...
    $scope.ShowSSFDetails = function () {
        if ($scope.beData.RefNo && $scope.beData.RefNo.length > 0 && $scope.beData.PurchaseSaudaDetail.SSFCode && $scope.beData.PurchaseSaudaDetail.SSFCode.length > 0) {
            if ($scope.beData.PurchaseSaudaDetail.SSF)
                $('#frmSSFDetails').modal('show');
        }
    };
    $scope.SSFCodeLostFocus = function () {
        if ($scope.beData.PurchaseSaudaDetail.SSFCode && $scope.beData.PurchaseSaudaDetail.SSFCode.length > 0) {

        }
        else {
            $scope.beData.PurchaseSaudaDetail.SSF = null;
        }
    }
    $scope.GetPatientById = function () {
        $scope.beData.PurchaseSaudaDetail.SSF = null;
        $scope.beData.PurchaseSaudaDetail.PatientId = null;
        $scope.beData.PurchaseSaudaDetail.PatientNo = null;
        if ($scope.beData.RefNo && $scope.beData.RefNo.length > 0) {
            ...
            $http({
                ...
                if (patient.IsSuccess == true) {
                    ...
                    $scope.beData.PurchaseSaudaDetail.Buyes = patient.PatientName;
                    $scope.beData.PurchaseSaudaDetail.Address = patient.Address;
                    $scope.beData.PurchaseSaudaDetail.PhoneNo = patient.MobileNo;
                    $scope.beData.PurchaseSaudaDetail.OtherRefereces = patient.ClaimCode;
                    $scope.beData.PurchaseSaudaDetail.SSFCode = patient.ClaimCode;
                    $scope.beData.PurchaseSaudaDetail.SSF = patient.SSF;
                    $scope.beData.PurchaseSaudaDetail.PatientId = patient.PatientId;
                    $scope.beData.PurchaseSaudaDetail.PatientNo = patient.PatientNo;

                    if ($scope.beData.PurchaseSaudaDetail.SSF) {
                        $scope.beData.PurchaseSaudaDetail.SSF.SchemePer = patient.SchemePer;
                        $scope.beData.PurchaseSaudaDetail.SSF.SchemeTypeId = patient.SchemeTypeId;
                    }
                    ...
                    $scope.beData.PurchaseSaudaDetail.SSF = null;
                    $scope.beData.PurchaseSaudaDetail.Buyes = '';
                    $scope.beData.PurchaseSaudaDetail.Address = '';
                    $scope.beData.PurchaseSaudaDetail.PhoneNo = '';
                    $scope.beData.PurchaseSaudaDetail.OtherRefereces = '';
                    $scope.beData.PurchaseSaudaDetail.SSFCode = '';
                    $scope.beData.PurchaseSaudaDetail.PatientNo = null;
                    $scope.beData.PurchaseSaudaDetail.PatientId = null;
        ...
        if ($scope.beData.RefVoucherType == 2) {
            ...
            $http({
                ...
                if (res.data.IsSuccess && res.data.Data) {
                    ...
                    //$('#newfrmPurchaseSaudaDetailsModel').modal('show');
                        ...
                        $timeout(function () {
                            ...
                            $http({
                                ...
                                if (res1.data.IsSuccess == true) {
                                    ...
                                    if ($scope.beData.PartyLedger) {
                                        $scope.beData.PurchaseSaudaDetail.Buyes = $scope.beData.PartyLedger.Name;
                                        $scope.beData.PurchaseSaudaDetail.Address = $scope.beData.PartyLedger.Address;
                                        $scope.beData.PurchaseSaudaDetail.SalesTaxNo = $scope.beData.PartyLedger.PanVat;
                                        $scope.beData.PurchaseSaudaDetail.PhoneNo = $scope.beData.PartyLedger.MobileNo1;
                                    ...
                                    $('#newfrmPurchaseSaudaDetailsModel').modal('hide');
                                }
    ...
    //                $scope.beData.PurchaseSaudaDetail.VinNo = dt.VinNo;
    //                $scope.beData.PurchaseSaudaDetail.ChassisNo = dt.ChassisNo;
    //                $scope.beData.PurchaseSaudaDetail.EngineNo = dt.EngineNo;
    //                $scope.beData.PurchaseSaudaDetail.RegdNo = dt.RegdNo;
    //                $scope.beData.PurchaseSaudaDetail.Buyes = dt.Party;
    //                $scope.beData.PurchaseSaudaDetail.JobCardTranId = dt.TranId;
    ...
    $scope.GetInvoicefromSAP = function () {
        ...
        if ($scope.beData.RefNo && $scope.beData.RefNo.length > 0 && $scope.SelectedVoucher.Series > 0) {
            ...
            $http({
                method: 'GET',
                url: base_url + "Inventory/Transaction/H_GetPurchaseSauda?DocNum=" + $scope.beData.RefNo + "&Series=" + $scope.SelectedVoucher.Series,
                dataType: "json"
            }).then(function (res) {
                $scope.loadingstatus = "stop";
                hidePleaseWait();
    ...
    $scope.ChangeTermsOfPayment = function () {
        if ($scope.beData && $scope.beData.PurchaseSaudaDetail) {
            if ($scope.beData.PurchaseSaudaDetail.TermsOfPayment) {
                var findPM = $scope.PaymentModeColl_Qry.firstOrDefault(p1 => p1.Name == $scope.beData.PurchaseSaudaDetail.TermsOfPayment);
                if (findPM) {
                    $scope.beData.PurchaseSaudaDetail.TypeOfPayment = findPM.TypeOfPayment;
                    $scope.beData.PurchaseSaudaDetail.PaymentTermsId = findPM.PaymentModeId;
                }
            }
        }
    }
    ...
    $scope.GenerateFonepayQR = function () {
        ...
        var reqData = {
            ...
            uniqueId: uniqueId,
            customerName: ($scope.beData.PurchaseSaudaDetail.Buyes ? $scope.beData.PurchaseSaudaDetail.Buyes : 'CASH'),
            mobileNo: ($scope.beData.PurchaseSaudaDetail.ContactNo ? $scope.beData.PurchaseSaudaDetail.ContactNo : 'CASH'),
        };
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SavePurchaseSauda);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
    ...
    $scope.addRowInRB = function (ind) {
        ...
        $scope.beData.RebateColl.splice(ind + 1, 0, newObj);
    };
    $scope.delRowInRB = function (ind) {
