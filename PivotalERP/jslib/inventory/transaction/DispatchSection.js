app.controller('DispatchSectionController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    ...
    function LoadData() {
        ...
        $scope.RefVoucherTypeColl = [{ id: 1, text: 'Dispatch Order' }];
 ...
 $scope.EPDet = {};
  ...
  itemDet.ProductName = itemDet.productDetail.Name;
    ...
    $scope.ChangePaymentTemrs = function () {
        if ($scope.beData.DispatchSectionDetail.TermsOfPayment == 'CASH') {
            if ($scope.RecVoucherTypeColl && $scope.RecVoucherTypeColl.length > 0)
                $scope.beData.DispatchSectionDetail.ReceiptVoucherId = $scope.RecVoucherTypeColl[0].VoucherId;
            else
                $scope.beData.DispatchSectionDetail.ReceiptVoucherId = null;
        };
    }
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            if (partyDet.PartyLedger) {
                ...
                if ($scope.lastPartyLedgerId != partyDet.PartyLedgerId && isModify == false) {
                    ...
                    partyDet.DispatchSectionDetail = {};


                    partyDet.DispatchSectionDetail.TermsOfPayment = partyDet.PartyLedger.PaymentType;

                    if (partyDet.PartyLedger.BillToColl && partyDet.PartyLedger.BillToColl.length > 0)
                        partyDet.DispatchSectionDetail.BillToId = partyDet.PartyLedger.BillToColl[0].Id;

                    if (partyDet.PartyLedger.ShipToColl && partyDet.PartyLedger.ShipToColl.length > 0)
                        partyDet.DispatchSectionDetail.ShipToId = partyDet.PartyLedger.ShipToColl[0].Id;

                    $scope.ChangeBillAddress();
                    $scope.ChangeShipAddress();

                    partyDet.DispatchSectionDetail.CreditDays = partyDet.PartyLedger.CreditLimitDays;
                    partyDet.DispatchSectionDetail.Buyes = partyDet.PartyLedger.Name;
                    partyDet.DispatchSectionDetail.Address = partyDet.PartyLedger.Address;
                    partyDet.DispatchSectionDetail.SalesTaxNo = partyDet.PartyLedger.PanVat;
                    partyDet.DispatchSectionDetail.PhoneNo = partyDet.PartyLedger.MobileNo1;
            ...
            if ($scope.SelectedVoucher.ActivePartyDetails == true) {
                ...
                $('#frmDispatchSectionDetailsModel').modal('show');
            }
            ...
            $scope.RecalculateAdditioncalCost();
            $scope.CalculateTotalAndSubTotal();
            $('#frmDispatchSectionDetailsModel').modal('hide');
        }
    ...
    $scope.ChangeBillAddress = function () {
        ...
        if ($scope.beData.PartyLedger.BillToColl) {
            var find = mx($scope.beData.PartyLedger.BillToColl).firstOrDefault(p1 => p1.Id == $scope.beData.DispatchSectionDetail.BillToId);
            if (find) {
                $scope.beData.DispatchSectionDetail.BillToAddress = find.ConcatAddress;
            }
        }
    }
    $scope.ChangeShipAddress = function () {
        if ($scope.beData.PartyLedger.ShipToColl) {
            var find = mx($scope.beData.PartyLedger.ShipToColl).firstOrDefault(p1 => p1.Id == $scope.beData.DispatchSectionDetail.ShipToId);
            if (find) {
                $scope.beData.DispatchSectionDetail.ShipToAddress = find.ConcatAddress;
            }
        }
    }
            ...
            $http({
                ...
                if (res1.data.IsSuccess && res1.data.Data) {
                    ...
                    $timeout(function () {
                        ...
                        $scope.beData.DispatchSectionDetail.TermsOfPayment = tranDet.TermsOfPayment;
                        $scope.beData.DispatchSectionDetail.OtherRefereces = tranDet.OtherRefereces;
                        $scope.beData.DispatchSectionDetail.TermsOfDelivery = tranDet.TermsOfDelivery;
                        $scope.beData.DispatchSectionDetail.DeliveryThrough = tranDet.DeliveryThrough;
                        $scope.beData.DispatchSectionDetail.DeliveryDocNo = tranDet.DeliveryDocNo;
                        $scope.beData.DispatchSectionDetail.Destination = tranDet.Destination;
                        $scope.beData.DispatchSectionDetail.SalesQuotationTermsOfPayment = tranDet.SalesQuotationTermsOfPayment;
                        $scope.beData.DispatchSectionDetail.SalesQuotationOtherRefereces = tranDet.SalesQuotationOtherRefereces;
                        $scope.beData.DispatchSectionDetail.SalesQuotationTermsOfDelivery = tranDet.SalesQuotationTermsOfDelivery;
                        $scope.beData.DispatchSectionDetail.Buyes = tranDet.Buyes;
                        $scope.beData.DispatchSectionDetail.Address = tranDet.Address;
                        $scope.beData.DispatchSectionDetail.SalesTaxNo = tranDet.SalesTaxNo;
                        $scope.beData.DispatchSectionDetail.CSTNumber = tranDet.CSTNumber;
                        $scope.beData.DispatchSectionDetail.Notes = tranDet.Notes;
                        $scope.beData.DispatchSectionDetail.PhoneNo = tranDet.PhoneNo;
                        $scope.beData.DispatchSectionDetail.Description = tranDet.Description;
                        $scope.beData.DispatchSectionDetail.OwnerName = tranDet.OwnerName;
                        $scope.beData.DispatchSectionDetail.OwnerContactNo = tranDet.OwnerContactNo;
                        $scope.beData.DispatchSectionDetail.DriverAddress = tranDet.DriverAddress;
                        $scope.beData.DispatchSectionDetail.DriverName = tranDet.DriverName;
                        $scope.beData.DispatchSectionDetail.DriverContactNo = tranDet.DriverContactNo;
                        $scope.beData.DispatchSectionDetail.LicenseNo = tranDet.LicenseNo;
                        $scope.beData.DispatchSectionDetail.Goods = tranDet.Goods;
                        $scope.beData.DispatchSectionDetail.Quantity = tranDet.Quantity;
                        $scope.beData.DispatchSectionDetail.FreightRate = tranDet.FreightRate;
                        $scope.beData.DispatchSectionDetail.AdvancePayment = tranDet.AdvancePayment;
                        $scope.beData.DispatchSectionDetail.TotalWT = tranDet.TotalWT;
                        $scope.beData.DispatchSectionDetail.ContactNo = tranDet.ContactNo;
                        $scope.beData.DispatchSectionDetail.RegdNo = tranDet.RegdNo;
                        $scope.beData.DispatchSectionDetail.EngineNo = tranDet.EngineNo;
                        $scope.beData.DispatchSectionDetail.ChassisNo = tranDet.ChassisNo;
                        $scope.beData.DispatchSectionDetail.Model = tranDet.Model;
                        $scope.beData.DispatchSectionDetail.VinNo = tranDet.VinNo;
                        $scope.beData.DispatchSectionDetail.CreditDays = tranDet.CreditDays;
                        $scope.beData.DispatchSectionDetail.ExportCountry = tranDet.ExportCountry;
                        $scope.beData.DispatchSectionDetail.PPNo = tranDet.PPNo;
                        $scope.beData.DispatchSectionDetail.PPDate = tranDet.PPDate;
                        $scope.beData.DispatchSectionDetail.OtherRefereces1 = tranDet.OtherRefereces1;
                        $scope.beData.DispatchSectionDetail.PaymentTermsId = tranDet.PaymentTermsId;
                        $scope.beData.DispatchSectionDetail.FreightTypeId = tranDet.FreightTypeId;
                        $scope.beData.Narration = tranData.Narration;
                        $scope.beData.No = tranData.RefNo;
                        $scope.beData.RefNo = tranData.RefNo;
                        $scope.beData.DispatchSectionDetail.BillToId = tranDet.BillToId;
                        $scope.beData.DispatchSectionDetail.ShipToId = tranDet.ShipToId;
                        $scope.beData.DispatchSectionDetail.BillToAddress = tranDet.BillToAddress;
                        $scope.beData.DispatchSectionDetail.ShipToAddress = tranDet.ShipToAddress;
        ...
        if (filterData.length > 0) {
            ...
            angular.forEach(filterData, function (fd) {
                var refItem = {
                    ...
                    RefQty: fd.ActualQty,
                    DispatchOrderItemAllocationId: refVType == 1 ? fd.ItemAllocationId : null,
                    RefVType: refVType,
            ...
            $timeout(function () {
                ...
                //refVType == 2 ? 'GetSalesOrderDetailsByItemAllocationId' : 'GetDispatchSectionDetailsByItemAllocationId';
                $http({
                    method: 'POST',
    ...
    $scope.RefVoucherChange = function (refVType) {
        ...
        var funName = "getPendingDispatchOrder"
    ...
    $scope.SaveDispatchSection = function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $timeout(function () {
                            ...
                            $http({
                                method: 'POST',
                                url: base_url + "Inventory/Transaction/SaveUpdateDispatchSection",
    ...
    $scope.GetTransactionById = function (tran) {
        if (tran.TranId && tran.TranId > 0) {
            ...
            $timeout(function () {
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/GetDispatchSectionById",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    $timeout(function () {
    ...
    $scope.IsValidData = function () {
        ...
        if ($scope.SelectedVoucher.AllowCashPurchaseSale == true && $scope.beData.DispatchSectionDetail.TermsOfPayment == 'CASH') {
    ...
    $scope.GetData = function () {
        ...
        var tmpSales = {
            ...
            AditionalCostColl: [],
            DispatchSectionDetail: $scope.beData.DispatchSectionDetail,
            GodownId: $scope.beData.GodownId,
            DocumentColl: $scope.beData.DocumentColl,
            PaymentTermColl: $scope.beData.PaymentTermColl,
            ReceiptTranId: $scope.beData.ReceiptTranId,
        };



        if (tmpSales.DispatchSectionDetail) {
            if (tmpSales.DispatchSectionDetail.FreightRate == undefined || tmpSales.DispatchSectionDetail.FreightRate == null || tmpSales.DispatchSectionDetail.FreightRate === undefined)
                tmpSales.DispatchSectionDetail.FreightRate = 0;

            if (tmpSales.DispatchSectionDetail.AdvancePayment == undefined || tmpSales.DispatchSectionDetail.AdvancePayment == null || tmpSales.DispatchSectionDetail.AdvancePayment === undefined)
                tmpSales.DispatchSectionDetail.AdvancePayment = 0;

            if (tmpSales.DispatchSectionDetail.CreditDays == undefined || tmpSales.DispatchSectionDetail.CreditDays == null || tmpSales.DispatchSectionDetail.CreditDays === undefined)
                tmpSales.DispatchSectionDetail.CreditDays = 0;
        }

        if (tmpSales.PaymentTermColl) {
            var vid = $scope.beData.DispatchSectionDetail.ReceiptVoucherId;
            if (!vid) {
                if ($scope.RecVoucherTypeColl && $scope.RecVoucherTypeColl.length > 0)
        ...
        angular.forEach($scope.beData.ItemDetailsColl, function (itemDet) {
            if (itemDet.ProductId && itemDet.ProductId > 0 && itemDet.RowType == 'P') {
                ...
                var itemAllocation = {
                    ...
                    DispatchOrderItemAllocationId: itemDet.DispatchOrderItemAllocationId ? itemDet.DispatchOrderItemAllocationId : 0,                    
   ...
   //     });
        ...
        $scope.beData.DispatchSectionDetail = tran.DispatchSectionDetail;
 ...
 $timeout(function () {                      
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SaveDispatchSection);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
