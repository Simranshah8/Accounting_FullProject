app.controller('CannibalizeOut', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    $scope.Title = 'CannibalizeOut';
    ...
    function LoadData() {
        ...
        $scope.SearchOptions = [{ text: 'VoucherNo', value: 'ADS.VoucherNo', searchType: 'Number' }, { text: 'EngineerNo', value: 'TS.ServiceEngineer', searchType: 'text' }, { text: 'ChassisNo', value: 'TS.ChassisNo', searchType: 'text' }];
 ...
 $scope.EPDet = {};
  ...
  itemDet.ProductName = itemDet.productDetail.Name;
        ...
        if (filterData.length > 0) {
            ...
            angular.forEach(filterData, function (fd) {
                var refItem = {
                    ...
                    RefQty: fd.ActualQty, 
                    DeliveryNoteItemAllocationId:fd.ItemAllocationId,
                };
                $scope.beData.ItemDetailsColl.push(refItem);
            });

            var para = {
                tranId:filterData[0].TranId,
            };
            $http({
                method: 'POST',
                ...
                $timeout(function () {
                    if (res.data.IsSuccess && res.data.Data) {
                        ...
                        $scope.beData.RefNo = tran.AutoManualNo;
    ...
    $scope.RefVoucherChange = function (refVType) {
        ...
        if ($scope.beData.SourceGodownId > 0) {
            var funName = "getPendingCannibalizeIn";

            var para = {
                godownId: $scope.beData.SourceGodownId,
                dateFrom: $scope.comDet.StartDate,
                dateTo:new Date(),
            }; 
    ...
    $scope.SaveCannibalizeOut = function () {
        ...
        if ($scope.IsValidData() == true) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/SaveUpdateCannibalizeOut",
   ...
   //     });
        ...
        $scope.beData.SourceGodownId = tran.GodownId;
 ...
 $timeout(function () {                      
        ...
        $timeout(function () {
            ...
            if (tran.TranId && tran.TranId > 0) {
                ...
                $timeout(function () {
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/GetCannibalizeOutById",
                        dataType: "json",
                        data: JSON.stringify(para)
                    }).then(function (res) {
                        $timeout(function () {
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SaveCannibalizeOut);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
