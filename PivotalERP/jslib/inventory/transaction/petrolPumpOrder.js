app.controller("PetrolPumpOrder", function ($scope, $http, $timeout, $filter, $compile,GlobalServices, $document) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            if (partyDet.PartyLedger) {
                if (partyDet.SalesOrderDetail) {
                    ...
                    partyDet.SalesOrderDetail.BuyesId = null;
                }
            }
    ...
    $scope.ChangeBuyerSelection = function () {
        ...
        if ($scope.beData.SalesOrderDetail.Buyes && $scope.beData.SalesOrderDetail.Buyes.length > 2) {
            var findP = $scope.CashPartyColl_Qry.firstOrDefault(p1 => p1.Buyes == $scope.beData.SalesOrderDetail.Buyes);
            if (findP) {
                $scope.beData.SalesOrderDetail.SalesTaxNo = findP.SalesTaxNo;
                $scope.beData.SalesOrderDetail.Address = findP.Address;
                $scope.beData.SalesOrderDetail.PhoneNo = findP.PhoneNo;
            }
        }
    }
    ...
    $scope.GetData = function () {
        ...
        if (tmpSales.SalesOrderDetail) {
            ...
            tmpSales.SalesOrderDetail.TermsOfPayment = $scope.beData.BillingAs == 1 ? 'CASH' : 'CREDIT';            
            if (tmpSales.SalesOrderDetail.Buyes && $scope.beData.BillingAs==1) {
                tmpSales.SalesOrderDetail.Buyes = 'CASH( ' + tmpSales.SalesOrderDetail.Buyes + ')';
            }
        }
