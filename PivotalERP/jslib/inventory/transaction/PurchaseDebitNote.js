app.controller('PurchaseDebitNoteController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
 ...
 $scope.EPDet = {};
  ...
  itemDet.ProductName = itemDet.productDetail.Name;
    ...
    $scope.PartySelectionChange = function (partyDet) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            ...
            if ($scope.SelectedVoucher.ActivePartyDetails == true)
                $('#frmPurchaseDebitNoteDetailsModel').modal('show');
            ...
            $scope.RecalculateAdditioncalCost();
            $scope.CalculateTotalAndSubTotal();
            $('#frmPurchaseDebitNoteDetailsModel').modal('hide');
        }
    ...
    $scope.SavePurchaseDebitNote = function () {
        ...
        if ($scope.IsValidData() == true) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/SaveUpdatePurchaseDebitNote",
                        headers: { 'Content-Type': undefined },
    ...
    $scope.GetTransactionById = function (tran) {
        $timeout(function () {
            ...
            if (tran.TranId && tran.TranId > 0) {
                ...
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/GetPurchaseDebitNoteById",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    $timeout(function () {
 ...
 $timeout(function () {                      
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SavePurchaseDebitNote);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
