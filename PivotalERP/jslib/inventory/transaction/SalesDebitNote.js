app.controller('SalesDebitNoteController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
 ...
 $scope.EPDet = {};
  ...
  itemDet.ProductName = itemDet.productDetail.Name;
    ...
    $scope.PartySelectionChange = function (partyDet) {
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            ...
            if ($scope.SelectedVoucher.ActivePartyDetails == true)
            {
                ...
                $('#frmSalesDebitNoteDetailsModel').modal('show');
            }
            ...
            $scope.RecalculateAdditioncalCost();
            $scope.CalculateTotalAndSubTotal();
            $('#frmSalesDebitNoteDetailsModel').modal('hide');
        }
    ...
    $scope.SaveSalesDebitNote = function () {
        ...
        if ($scope.IsValidData() == true) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/SaveUpdateSalesDebitNote",
                        headers: { 'Content-Type': undefined },
    ...
    $scope.GetTransactionById = function (tran) {
        if (tran.TranId && tran.TranId > 0) {
            ...
            $timeout(function () {
                $http({
                    method: 'POST',
                    url: base_url + "Inventory/Transaction/GetSalesDebitNoteById",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    $timeout(function () {
 ...
 $timeout(function () {                      
    ...
    $document.bind('keydown', function (event) {
        if (event.ctrlKey && event.key === 's') { // Ctrl + S
            ...
            $scope.$apply($scope.SaveSalesDebitNote);
        }
        if (event.key === 'F3') { // F3
            event.preventDefault(); // Prevent browser save action
