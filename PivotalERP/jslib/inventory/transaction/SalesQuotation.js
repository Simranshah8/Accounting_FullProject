app.controller('SalesQuotationController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
    ...
    function LoadData() {
        ...
        $scope.RateAsPerColl = GlobalServices.getRateAsPer();
        $scope.ProjectColl = [];
        ...
        $scope.VoucherSearchOptions = [{ text: 'PartyName', value: 'ADS.Buyes', searchType: 'text' }, { text: 'PanVat', value: 'ADS.SalesTaxNo', searchType: 'text' }, { text: 'PartyLedger', value: 'Led.Name', searchType: 'text' }, { text: 'RefNo', value: 'TS.[QuotationNo]', searchType: 'text' }, { text: 'Invoice No.', value: 'TS.AutoManualNo', searchType: 'text' }, { text: 'Voucher', value: 'V.VoucherName', searchType: 'text' }, { text: 'CostClass', value: 'CC.Name', searchType: 'text' }, { text: 'VoucherDate', value: 'TS.VoucherDate', searchType: 'date' }, { text: 'Amount', value: 'TS.TotalAmount', searchType: 'number' },
        ...
        $scope.beData =
        {
            ...
            SaveClear: false,
            DocumentColl: [],
            VehicleSearchOption: $scope.VehicleSearchOptions[2].value,
            PaymentTermColl: [],
            Attributes: '', 
        };
 ...
 $scope.EPDet = {};
        ...
        if (partyDet.PartyLedgerId && partyDet.PartyLedgerId > 0) {
            if (partyDet.PartyLedger) {
                ...
                if ($scope.lastPartyLedgerId != partyDet.PartyLedgerId && isModify == false) {
                    ...
                    if (isModify == false)
                        partyDet.SalesQuotationDetail.TermsOfPayment = partyDet.PartyLedger.PaymentType;

                    if (!partyDet.SalesQuotationDetail.Buyes)
                        partyDet.SalesQuotationDetail.Buyes = partyDet.PartyLedger.Name;
                    ...
                    if (!partyDet.SalesQuotationDetail.SalesTaxNo)
                        partyDet.SalesQuotationDetail.SalesTaxNo = partyDet.PartyLedger.PanVat;

                    if (!partyDet.SalesQuotationDetail.ContactNo) {
                        partyDet.SalesQuotationDetail.ContactNo = partyDet.PartyLedger.MobileNo1;
                        partyDet.SalesQuotationDetail.PhoneNo = partyDet.PartyLedger.MobileNo1;
                    }
            ...
            $http({
                ...
                if (res1.data.IsSuccess && res1.data.Data) {
                    ...
                    if (refVType == 1) {
                        ...
                        $scope.beData.SalesQuotationDetail.ContactNo = tmpdata.DeliveryNoteDetail.PhoneNo;
                        $scope.beData.SalesQuotationDetail.PhoneNo = tmpdata.DeliveryNoteDetail.PhoneNo;
                        $scope.beData.SalesQuotationDetailSalesTaxNo = tmpdata.DeliveryNoteDetail.SalesTaxNo;
                        ...
                        $scope.beData.SalesQuotationDetail.ContactNo = tmpdata.PhoneNo;
        ...
        $timeout(function () {
            ...
            if (tran.TranId && tran.TranId > 0) {
                ...
                $timeout(function () {
                    $http({
                        method: 'POST',
                        url: base_url + "Inventory/Transaction/GetSalesQuotationById",
                        dataType: "json",
                        data: JSON.stringify(para)
                    }).then(function (res) {
                        $timeout(function () {
    ...
    $scope.IsValidData = function () {
        ...
        if ($scope.beData.UDFFeildsColl) {
            if ($scope.beData.UDFFeildsColl.length > 0) {
                
                angular.forEach($scope.beData.UDFFeildsColl, function (uf) {
                    if (result == true)
                    {
                        if (uf.IsMandatory == true) {

                            if (uf.FieldType == 1) {
                                if (isEmptyObj(uf.UDFValue) == true) {
                                    result = false;
                                    Swal.fire('Please ! Enter ' + uf.Name);

                                }
                            }
                            else if (uf.FieldType == 2) {
                                if (!uf.UDFValueDet) {
                                    result = false;
                                    Swal.fire('Please ! Enter ' + uf.Name);
                                }
                            }
                            else if (uf.FieldType == 3) {
                                if (isEmptyObj(uf.UDFValue) == true) {
                                    result = false;
                                    Swal.fire('Please ! Select ' + uf.Name);

                                }
                            }
                            else if (uf.FieldType == 4) {

                            }
                            else if (uf.FieldType == 5) {
    ...
    $scope.GetData = function () {
        ...
        if (tmpSales.SalesQuotationDetail) {
            if (tmpSales.SalesQuotationDetail.FreightRate == undefined || tmpSales.SalesQuotationDetail.FreightRate == null || tmpSales.SalesQuotationDetail.FreightRate === undefined)
                tmpSales.SalesQuotationDetail.FreightRate = 0;

            if (tmpSales.SalesQuotationDetail.AdvancePayment == undefined || tmpSales.SalesQuotationDetail.AdvancePayment == null || tmpSales.SalesQuotationDetail.AdvancePayment === undefined)
                tmpSales.SalesQuotationDetail.AdvancePayment = 0;

            if (tmpSales.SalesQuotationDetail.CreditDays == undefined || tmpSales.SalesQuotationDetail.CreditDays == null || tmpSales.SalesQuotationDetail.CreditDays === undefined)
                tmpSales.SalesQuotationDetail.CreditDays = 0;
        }
...
var aditionColl = angular.copy($scope.SelectedVoucher.AditionalChargeColl);
        ...
        if ($scope.SelectedVoucher.VoucherWiseGodownColl && $scope.SelectedVoucher.VoucherWiseGodownColl.length>0) {
            $scope.beData.GodownId = $scope.SelectedVoucher.VoucherWiseGodownColl[0].GodownId;
        }else if ($scope.GodownColl.length > 0 && (!$scope.beData.GodownId || $scope.beData.GodownId == 0)) {
            $scope.beData.GodownId = $scope.GodownColl[0].GodownId;
        }
        ...
        $scope.beData.SalesQuotationDetail = tran.SalesQuotationDetail;
            ...
            angular.forEach(itemA.ItemDetailsColl, function (itemAD) {
                ...
                angular.forEach(udfColl, function (uc) {
                    itemAD.UDFFeildsColl.push({                         
                        ...
                        Source: uc.Source,
                        Formula: uc.Formula,
                        RefTable: uc.RefTable,
                        RefColumn: uc.RefColumn,
                        TextColumn: uc.TextColumn,
                        Label: uc.Label,
                    });
                });
 ...
 $timeout(function () {                      
    ...
    $scope.ClearData = function () {
        ...
        var lastVId = null, lastCId = null;
        lastVId = $scope.beData.VoucherId;
        lastCId = $scope.beData.CostClassId;
        ...
        if ($scope.VoucherTypeColl.length > 0) {
            ...
            if (lastVId > 0) {
                $scope.SelectedVoucher = mx($scope.VoucherTypeColl).firstOrDefault(p1=>p1.VoucherId==lastVId);
                $scope.beData.VoucherId = $scope.SelectedVoucher.VoucherId;
            } else {
                $scope.SelectedVoucher = $scope.VoucherTypeColl[0];
                $scope.beData.VoucherId = $scope.SelectedVoucher.VoucherId;
        ...
        if ($scope.CostClassColl.length > 0) {
            ...
            if (lastCId > 0) {
                $scope.SelectedCostClass = mx($scope.CostClassColl).firstOrDefault(p1=>p1.CostClassId==lastCId);
                $scope.beData.CostClassId = $scope.SelectedCostClass.CostClassId;
    ...
    $scope.VehicleSelectionChange = function (data, index) {
        $scope.SelectedVehicle = null;
        if (data.vehicleDetail && data.vehicleDetail.TranId>0) {
            var det = data.vehicleDetail;            
            $scope.SelectedVehicle = det;
            $scope.beData.SalesQuotationDetail.RegdNo = det.RegdNo;
            $scope.beData.SalesQuotationDetail.EngineNo = det.EngineNo;
            $scope.beData.SalesQuotationDetail.ChassisNo = det.ChassisNo;
            $scope.beData.SalesQuotationDetail.VinNo = det.VinNo;
            $scope.beData.SalesQuotationDetail.Model = det.VehicleModelName;
            $scope.beData.SalesQuotationDetail.Color = det.Color;
            $scope.beData.SalesQuotationDetail.Buyes = det.Party;
            $scope.beData.PartyLedgerId = det.PartyId;

               
        } else {
            $scope.beData.SalesQuotationDetail.RegdNo = '';
            $scope.beData.SalesQuotationDetail.EngineNo = '';
            $scope.beData.SalesQuotationDetail.ChassisNo = '';
            $scope.beData.SalesQuotationDetail.VinNo = '';
            $scope.beData.SalesQuotationDetail.Model = '';
            $scope.beData.SalesQuotationDetail.Color = '';
            $scope.beData.SalesQuotationDetail.Buyes = '';
        }
    }
