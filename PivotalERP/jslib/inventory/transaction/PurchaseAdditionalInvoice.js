...
app.controller('AdditionalInvoiceController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
 ...
 $timeout(function () {                      
    ...
    $scope.GetData = function () {
        ...
        if ($scope.beData.LCDetail) {
            ...
            if ($scope.beData.LCDetail.GateInWardDateDet) {
            ...
            if ($scope.beData.LCDetail.BillDateDet) {
            ...
            if ($scope.beData.LCDetail.PPDateDet) {
            ...
            if ($scope.beData.LCDetail.LCDateDet) {
            ...
            if ($scope.beData.LCDetail.BuiltyDateDet) {
    ...
    $scope.ChangeCostAs = function (ind,curRow, curLedger) {
        if (ind == 0 && curRow.CostAs == 2) {
            ...
            if (totalProduct > 0) {
                for (var s = 0; s < totalProduct; s++) {
                    curRow.CostAsId = $scope.CostProductColl[s].id;
    ...
    $scope.ShowCostCenterImport = function () {
        $scope.ImportCostCenterDataColl = [];
        $scope.ImportCostCenterDataColl.push({});
        $('#frmImportCostCenter').modal('show');
    }
    $scope.PasteCostCenterData = function (colName, ind) {
        var clipText = event.clipboardData.getData('text/plain');
        $scope.ImportCostCenterAmount = 0;
        ...
        if (clipText) {
            ...
            clipText.split("\n").forEach(function (line) {
                if (line && line.length > 0) {
                    ...
                    if ($scope.ImportCostCenterDataColl.length < (startInd + 1)) {
                        var newObj = {
                            CostCenter: '',
                            CostAs: '',
                            CostFor: '',
                        ...
                        $scope.ImportCostCenterDataColl.push(newObj);
                    }

                    $scope.ImportCostCenterDataColl[startInd][colName] = line.trim();
                    $scope.ImportCostCenterAmount = $scope.ImportCostCenterAmount + isEmptyAmt($scope.ImportCostCenterDataColl[startInd].Amount);
                    startInd++;
                }
            });
    ...
    $scope.GetCostCenterImportData = function () {
        if ($scope.ImportCostCenterDataColl) {
            $scope.loadingstatus = "running";
            showPleaseWait();
            ...
            $http({
                ...
                data: { jsonData: $scope.ImportCostCenterDataColl }
            }).then(function (res1) {
                ...
                if (res1.data.IsSuccess == true && res1.data.Data) {
                    $('#frmImportCostCenter').modal('hide');
                    ...
                    if (dataColl.length > 0)
                    {
                        ...
                        dataColl.forEach(function (dc) {
                            $scope.CurLedgerAllocation.CostCenterColl.push(dc);

                            $scope.ChangeDrCrAmount($scope.beData.LedgerAllocationColl[$scope.beData.LedgerAllocationColl.length - 1]);
                        });

                        if ($scope.CurLedgerAllocation.CostCenterColl.length == 0)
                            $scope.CurLedgerAllocation.CostCenterColl.push({});
