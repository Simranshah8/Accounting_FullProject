app.controller("PetrolPump", function ($scope, $http, $timeout, $filter, $compile,GlobalServices, $document) {
    ...
    $scope.CashPartySelectionChange = function (partyDet) {

        $scope.sideBarData = partyDet.partySideBarData;

        if (partyDet.CashPartyLedger && partyDet.SalesInvoiceDetail.BuyesId > 0) {
            if (partyDet.CashPartyLedger) {
                if (partyDet.CashPartyLedger) {
                    partyDet.SalesInvoiceDetail.Buyes = partyDet.CashPartyLedger.Name;
                    partyDet.SalesInvoiceDetail.Address = partyDet.CashPartyLedger.Address;
                    partyDet.SalesInvoiceDetail.SalesTaxNo = partyDet.CashPartyLedger.PanVat;
                    partyDet.SalesInvoiceDetail.PhoneNo = partyDet.CashPartyLedger.MobileNo1;              
                }
            }
        } 
    ...
    $scope.ClickOnProduct = function (pro)
    {
        if (pro) {
            ...
            $scope.beData.ItemDetailsColl[0].Rate = ($filter('number')(pro.SalesRate, $scope.beData.ItemDetailsColl[0].RateDecimal)).parseDBL();
            ...
            $scope.beData.ItemDetailsColl[0].Amount = 0;
        }
        $scope.ChangeQtAmt('qty');
    }
    $scope.ChangeQtAmt = function (col) {
        var idet = $scope.beData.ItemDetailsColl[0];
        if (col == 'qty') {
            var amt = ($filter('number')((idet.ActualQty * idet.Rate), idet.AmountDecimal)).parseDBL();
            idet.Amount = amt;            
        } else if (col == 'amt') {
            var qty = ($filter('number')((idet.Amount / idet.Rate), idet.QtyDecimal)).parseDBL();
            idet.ActualQty = qty;
        }

        var totalAmt = 0;
        $scope.beData.ItemDetailsColl.forEach(function (itemDet) {
            totalAmt += isEmptyNum(itemDet.Amount);
        });
