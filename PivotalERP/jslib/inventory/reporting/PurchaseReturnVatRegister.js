...
app.controller("PurchaseReturnVatRegister", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
  ...
  var PrintPreviewAs = 1;
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'PurchaseReturnVatRegister.csv',
            sheetName: 'PurchaseReturnVatRegister'
        };
    ...
    function LoadData() {
        ...
        $scope.PurchaseReturnVatRegister = {
        ...
        var columnDefs = [
            ...
            {
                headerName: "Date(B.S.)", width: 140, field: "EntryDateStr", cellRenderer: 'agGroupCellRenderer',
            ...
            { headerName: "EntryDate", width: 120, field: "EntryDateStr", cellStyle: { 'text-align': 'center' }  },

            { headerName: "RefNo", width: 180, field: "RefNo", cellStyle: { 'text-align': 'center' }  },
            { headerName: "VoucherNo", width: 180, field: "VoucherNo", cellStyle: { 'text-align': 'center' }  },
            { headerName: "PartyName", width: 180, field: "PartyName", cellStyle: { 'text-align': 'left' }  },
            { headerName: "PartyCostCenter", width: 180, field: "PartyCostCenter", cellStyle: { 'text-align': 'left' }  },
            { headerName: "Address", width: 180, field: "Address", cellStyle: { 'text-align': 'left' }  },
            { headerName: "PanVatNo", width: 180, field: "PanVatNo", cellStyle: { 'text-align': 'center' }  },
            { headerName: "PartyGroup", width: 180, field: "SalesLedger", cellStyle: { 'text-align': 'left' }  },
            { headerName: "TotalPurchase", width: 180, field: "TotalSales", cellStyle: { 'text-align': 'right' }  },
            { headerName: "Discount", width: 180, field: "Discount", cellStyle: { 'text-align': 'right' }  },
            { headerName: "TaxableValue", width: 180, field: "TaxableValue",cellStyle: { 'text-align': 'right' }  },
            { headerName: "AdditionalCharge", width: 180, field: "AdditionalCharge", cellStyle: { 'text-align': 'right' }  },
            { headerName: "Tax", width: 180, field: "Tax", cellStyle: { 'text-align': 'right' }  },
            { headerName: "ImportValue", width: 180, field: "ImportValue", cellStyle: { 'text-align': 'right' }  },
            { headerName: "ImportTax", width: 180, field: "ImportTax", cellStyle: { 'text-align': 'right' }  },
            { headerName: "TotalValue", width: 180, field: "TotalValue", cellStyle: { 'text-align': 'right' }  },
 ...
 $timeout(function () {
    ...
    $scope.GetPurchaseReturnVatRegister = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.PurchaseReturnVatRegister.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.PurchaseReturnVatRegister.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.PurchaseReturnVatRegister.DateToDet)
            dateTo = new Date(($filter('date')($scope.PurchaseReturnVatRegister.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            VoucherType: $scope.PurchaseReturnVatRegister.VoucherId,
            isPost: $scope.PurchaseReturnVatRegister.IsPost,
            branchId: $scope.PurchaseReturnVatRegister.BranchId
        };
        ...
        $http({
            method: "post",
            url: base_url + "Inventory/Reporting/GetPurchaseReturnVatRegister",
    ...
    $scope.GetDataForPrint = function () {
        ...
        $scope.gridOptions.api.forEachNodeAfterFilterAndSort(function (node) {
            ...
            beData.VoucherName = PurchaseReturnVatRegister.VoucherName;
            beData.VoucherType = PurchaseReturnVatRegister.VoucherType;
            beData.AutoManualNo = PurchaseReturnVatRegister.AutoManualNo;
            beData.AutoVoucherNo = PurchaseReturnVatRegister.AutoVoucherNo;
            beData.CanUpdateDocument = PurchaseReturnVatRegister.CanUpdateDocument;
            beData.CostClassName = PurchaseReturnVatRegister.CostClassName;
            beData.IsInventory = PurchaseReturnVatRegister.IsInventory;
            beData.IsParent = true;
            beData.Narration = PurchaseReturnVatRegister.Narration;
            beData.ND = PurchaseReturnVatRegister.ND;
            beData.NM = PurchaseReturnVatRegister.NM;
            beData.NY = PurchaseReturnVatRegister.NY;
            beData.RefNo = PurchaseReturnVatRegister.RefNo;
            beData.VoucherDate = PurchaseReturnVatRegister.VoucherDate;
            ...
            beData.CreatedByName = PurchaseReturnVatRegister.CreatedByName;

            if (beData.IsInventory == true) {
                beData.Particulars = PurchaseReturnVatRegister.PartyLedger;
                beData.DrAmount = PurchaseReturnVatRegister.DrAmount;
                beData.CrAmount = PurchaseReturnVatRegister.CrAmount;
                filterData.push(beData);

                var ledData = {};
                ledData.Particulars = "  " + PurchaseVatRegister.Particulars;

                if (GatePassDetails.DrAmount != 0)
                    ledData.CrAmount = PurchaseReturnVatRegister.DrAmount - mx(PurchaseReturnVatRegister.AditionalCostColl).Sum(p1 => p1.Amount);
                else
                    ledData.DrAmount = PurchaseReturnVatRegister.CrAmount - mx(PurchaseReturnVatRegister.AditionalCostColl).Sum(p1 => p1.Amount);

                filterData.push(ledData);

                angular.forEach(PurchaseReturnVatRegister.AditionalCostColl, function (add) {

                    var addData = {};
                    addData.Particulars = "  " + add.LedgerName;
                    if (PurchaseReturnVatRegister.DrAmount != 0) {
                ...
                angular.forEach(PurchaseReturnVatRegister.ItemAllocationColl, function (item) {
                ...
                var firstTime = true;
                angular.forEach(PurchaseReturnVatRegister.LedgerAllocationColl, function (ledAll) {
                    if (firstTime) {
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "PurchaseReturnVatRegister.xlsx");
            }
