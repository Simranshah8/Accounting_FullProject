...
app.controller("FixedProductOpeningList", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
  ...
  var PrintPreviewAs = 1;
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'FixedProductOpeningList.csv',
            sheetName: 'FixedProductOpeningList'
        };
    ...
    function LoadData() {
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.FixedProductOpeningList.DateFrom_TMP = new Date(comDet.StartDate);
                }
            }, function (errormessage) {
                alert('Unable to Delete data. pls try again.' + errormessage.responseText);
            });
        });
        $scope.FixedProductOpeningList = {
        ...
        var columnDefs = [
            ...
            { headerName: "ProductName", width: 180, field: "Name", cellStyle: { 'text-align': 'left' } },

            { headerName: "ProductGroup", width: 180, field: "ProductGroup", cellStyle: { 'text-align': 'left' } },
            { headerName: "Godown", width: 180, field: "GodownName", cellStyle: { 'text-align': 'left' } },
            ...
            { headerName: "Color", width: 180, field: "Color", cellStyle: { 'text-align': 'left' }},
            { headerName: "MFGYear", width: 180, field: "MFGYear", cellStyle: { 'text-align': 'right' }},
            { headerName: "Rate", width: 180, field: "Rate", cellStyle: { 'text-align': 'right' } },
            { headerName: "Amount", width: 180, field: "Amount", cellStyle: { 'text-align': 'right' } },
            { headerName: "Qty", width: 180, field: "Qty", cellStyle: { 'text-align': 'right' } },
            { headerName: "Per", width: 180, field: "Per", cellStyle: { 'text-align': 'right' } },
    ...
    $scope.GetFixedProductOpeningList = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.FixedProductOpeningList.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.FixedProductOpeningList.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.FixedProductOpeningList.DateToDet)
            dateTo = new Date(($filter('date')($scope.FixedProductOpeningList.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            VoucherType: $scope.FixedProductOpeningList.VoucherId,
            isPost: $scope.FixedProductOpeningList.IsPost,
            branchId: $scope.FixedProductOpeningList.BranchId
        };
        ...
        $http({
            method: "post",
            url: base_url + "Inventory/Reporting/GetFixedProductOpeningList",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "FixedProductOpeningList.xlsx");
            }
