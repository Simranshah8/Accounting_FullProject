...
app.controller("PendingPartsDemandAdditionalReport", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
  ...
  var PrintPreviewAs = 1;
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'PendingPartsDemandAdditionalReport.csv',
            sheetName: 'PendingPartsDemandAdditionalReport'
        };
    ...
    function LoadData() {
        ...
        $scope.PendingPartsDemandAdditional = {
            PendingType: 0,
        ...
        var columnDefs = [
            ...
            { headerName: "Particulars", width: 120, field: "Name", cellStyle: { 'text-align': 'left' }  },

            { headerName: "In No", width: 180, field: "AutoVoucherNo", cellStyle: { 'text-align': 'right' }  },
            { headerName: "In Qty", width: 180, field: "DemandQty", cellStyle: { 'text-align': 'right' }  },
            { headerName: "OutQty", width: 180, field: "OutQty", cellStyle: { 'text-align': 'right' }  },
            { headerName: "PendingQty", width: 180, field: "DemandQty", cellStyle: { 'text-align': 'right' }  },
            { headerName: "Godown", width: 180, field: "FromGodown", cellStyle: { 'text-align': 'left' }  },
            { headerName: "IssuesDetails", width: 180, field: "IssuesDetails", cellStyle: { 'text-align': 'left' }  },
            { headerName: "ServiceEnginer", width: 180, field: "ServiceEnginer", cellStyle: { 'text-align': 'left' }  },
            ...
            { headerName: "ChassisNo", width: 180, field: "ChassisNo", cellStyle: { 'text-align': 'right' }  },
            { headerName: "VinNo", width: 180, field: "VinNo", cellStyle: { 'text-align': 'right' }  },
        ...
        $scope.gridOptions = {
            ...
            onFilterChanged: function () {
                ...
                $scope.gridOptions.api.forEachNodeAfterFilterAndSort(function (node) {
                    if (node.data.DemandQty == 1 || node.data.DemandQty == 'DR')
                        Opening += node.data.DemandQty;
                    else
                        Opening -= node.data.DemandQty;
                });
                ...
                $scope.dataForBottomGrid[0].DemandQty = Opening;
    ...
    $scope.GetPendingPartsDemandAdditional = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.PendingPartsDemandAdditional.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.PendingPartsDemandAdditional.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.PendingPartsDemandAdditional.DateToDet)
            dateTo = new Date(($filter('date')($scope.PendingPartsDemandAdditional.DateToDet.dateAD, 'yyyy-MM-dd')));

        $scope.DataColl = []; //declare an empty array
        $scope.gridOptions.api.setRowData($scope.DataColl);

        var beData = {
            PendingType: $scope.PendingPartsDemandAdditional.PendingType,
            DateFrom: dateFrom,
            DateTo: dateTo,
        ...
        $http({
            method: "post",
            url: base_url + "Inventory/Reporting/GetPendingPartsDemandAdditional",
            ...
            if (res.data.IsSuccess && res.data.Data) {
                ...
                angular.forEach($scope.DataColl, function (dc) {
                    if (dc.DemandQty == 1 || dc.DemandQty == 'DR')
                        Opening += dc.DemandQty;
                    else
                        Opening -= dc.DemandQty;
                });
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "PendingPartsDemandAdditional.xlsx");
            }
