...
app.controller("FixedProductVoucher", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
  ...
  var PrintPreviewAs = 1;
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'FixedProductVoucher.csv',
            sheetName: 'FixedProductVoucher'
        };
    ...
    function LoadData() {
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.FixedProductVoucher.DateFrom_TMP = new Date(comDet.StartDate);
                }
            }, function (errormessage) {
                alert('Unable to Delete data. pls try again.' + errormessage.responseText);
            });
        });
        $scope.FixedProductVoucher = {
            ProductId: 0,
        ...
        var columnDefs = [
            ...
            { headerName: "Particular", width: 120, field: "Particular", cellStyle: { 'text-align': 'left' } },
            ...
            { headerName: "OutAmt", width: 180, field: "OutAmt", cellStyle: { 'text-align': 'right' } },
            ...
            { headerName: "UserName", width: 180, field: "UserName", cellStyle: { 'text-align': 'left' } },
        ...
        $scope.gridOptions = {
            ...
            onFilterChanged: function () {
                ...
                $scope.gridOptions.api.forEachNodeAfterFilterAndSort(function (node) {
                    if (node.data.OutAmt == 1 || node.data.OutAmt == 'DR')
                        Opening += node.data.OutAmt;
                    else
                        Opening -= node.data.OutAmt;
                });
                ...
                $scope.dataForBottomGrid[0].OutAmt = Opening;
 ...
 $timeout(function () {
    ...
    $scope.GetFixedProductVoucher = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.FixedProductVoucher.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.FixedProductVoucher.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.FixedProductVoucher.DateToDet)
            dateTo = new Date(($filter('date')($scope.FixedProductVoucher.DateToDet.dateAD, 'yyyy-MM-dd')));

        $scope.DataColl = []; //declare an empty array
        $scope.gridOptions.api.setRowData($scope.DataColl);

        var beData = {
            ProductId: $scope.FixedProductVoucher.ProductId,
            DateFrom: dateFrom,
            DateTo: dateTo,
        ...
        $http({
            method: "post",
            url: base_url + "Inventory/Reporting/GetFixedProductVoucher",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "FixedProductVoucher.xlsx");
            }
