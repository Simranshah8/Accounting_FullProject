...
app.controller("SalesReturnVatRegister", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
  ...
  var PrintPreviewAs = 1;
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'SalesReturnVatRegister.csv',
            sheetName: 'SalesReturnVatRegister'
        };
    ...
    function LoadData() {
        ...
        { text: 'ProductGroupName', value: 'ProductGroupName', dataType: 'text' },
        { text: 'ProductCategories', value: 'ProductCategories', dataType: 'text' },
        ...
        var columnDefs = [
            ...
            { headerName: "Party CostCenter", width: 250, field: "PartyCostCenter", cellStyle: { 'text-align': 'left' }  },
            { headerName: "Ref No.", width: 250, field: "RefNo", cellStyle: { 'text-align': 'right' }  },
            { headerName: "Ref.V.No.", width: 250, field: "OtherReferences", cellStyle: { 'text-align': 'right' }  },
 ...
 $timeout(function () {
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    ...
                    if (res.data.IsSuccess) {
                        $scope.GetSalesReturnVatRegister();
                    } else {
                        Swal.fire(res.data.ResponseMSG);
                    }
    ...
    $scope.GetSalesReturnVatRegister = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.SalesReturnVatRegister.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.SalesReturnVatRegister.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.SalesReturnVatRegister.DateToDet)
            dateTo = new Date(($filter('date')($scope.SalesReturnVatRegister.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            VoucherType: $scope.SalesReturnVatRegister.VoucherId,
            isPost: $scope.SalesReturnVatRegister.IsPost,
            branchId: $scope.SalesReturnVatRegister.BranchId
        };
        ...
        $http({
            method: "POST",
            url: base_url + "Inventory/Reporting/GetSalesReturnVatRegister",
