  ...
  var PrintPreviewAs = 1;
    ...
    function LoadData() {
        ...
        var columnDefs = [
            ...
            { headerName: "TDS", width: 180, field: "TDSAmount", cellStyle: { 'text-align': 'right' }  },
 ...
 $timeout(function () {
    ...
    $scope.GetPurchaseVatRegister = function () {
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            VoucherType: $scope.PurchaseVatRegister.VoucherId,
            isPost: $scope.PurchaseVatRegister.IsPost,
            branchId: $scope.PurchaseVatRegister.BranchId
        };
        ...
        $http({
            method: "post",
            url: base_url + "Inventory/Reporting/GetPurchaseVatRegister",
    ...
    $scope.GetDataForPrint = function () {
        ...
        $scope.gridOptions.api.forEachNodeAfterFilterAndSort(function (node) {
            ...
            beData.VoucherName = PurchaseVatRegister.VoucherName;
            beData.VoucherType = PurchaseVatRegister.VoucherType;
            beData.AutoManualNo = PurchaseVatRegister.AutoManualNo;
            beData.AutoVoucherNo = PurchaseVatRegister.AutoVoucherNo;
            beData.CanUpdateDocument = PurchaseVatRegister.CanUpdateDocument;
            beData.CostClassName = PurchaseVatRegister.CostClassName;
            beData.IsInventory = PurchaseVatRegister.IsInventory;
            beData.IsParent = true;
            beData.Narration = PurchaseVatRegister.Narration;
            beData.ND = PurchaseVatRegister.ND;
            beData.NM = PurchaseVatRegister.NM;
            beData.NY = PurchaseVatRegister.NY;
            beData.RefNo = PurchaseVatRegister.RefNo;
            beData.VoucherDate = PurchaseVatRegister.VoucherDate;
            ...
            beData.CreatedByName = PurchaseVatRegister.CreatedByName;

            if (beData.IsInventory == true) {
                beData.Particulars = PurchaseVatRegister.PartyLedger;
                beData.DrAmount = PurchaseVatRegister.DrAmount;
                beData.CrAmount = PurchaseVatRegister.CrAmount;
                ...
                if (GatePassDetails.DrAmount != 0)
                    ledData.CrAmount = PurchaseVatRegister.DrAmount - mx(PurchaseVatRegister.AditionalCostColl).Sum(p1 => p1.Amount);
                else
                    ledData.DrAmount = PurchaseVatRegister.CrAmount - mx(PurchaseVatRegister.AditionalCostColl).Sum(p1 => p1.Amount);

                filterData.push(ledData);

                angular.forEach(PurchaseVatRegister.AditionalCostColl, function (add) {

                    var addData = {};
                    addData.Particulars = "  " + add.LedgerName;
                    if (PurchaseVatRegister.DrAmount != 0) {
                ...
                angular.forEach(PurchaseVatRegister.ItemAllocationColl, function (item) {
                ...
                var firstTime = true;
                angular.forEach(PendingDeliveryNote.LedgerAllocationColl, function (ledAll) {
                    if (firstTime) {
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "PurchaseVatRegister.xlsx");
            }
