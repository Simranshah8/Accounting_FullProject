...
app.controller("FixedProductSalesSummary", function ($scope, $http, $filter, $timeout, GlobalServices, $compile) {
  ...
  var PrintPreviewAs = 1;
    ...
    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'FixedProductSalesSummary.csv',
            sheetName: 'FixedProductSalesSummary'
        };
    ...
    function LoadData() {
        ...
        $timeout(function () {
            $http({
                ...
                if (comDet) {
                    $scope.FixedProductSalesSummary.DateFrom_TMP = new Date(comDet.StartDate);
                }
            }, function (errormessage) {
                alert('Unable to Delete data. pls try again.' + errormessage.responseText);
            });
        });
        $scope.FixedProductSalesSummary = {
        ...
        var columnDefs = [
            ...
            {
                headerName: "Date(B.S.)", width: 140, field: "VoucherDateBS", cellRenderer: 'agGroupCellRenderer', cellStyle: { 'text-align': 'center' },
            ...
            { headerName: "MFGDate", width: 120, field: "MFGDate", cellStyle: { 'text-align': 'center' } },

            { headerName: "EXPDate", width: 180, field: "EXPDate", cellStyle: { 'text-align': 'center' } },
            { headerName: "EXPDays", width: 180, field: "ExpiredAfterDays", cellStyle: { 'text-align': 'center' } },
            { headerName: "Batch", width: 180, field: "Batch", cellStyle: { 'text-align': 'left' } },
            ...
            { headerName: "Product", width: 180, field: "Name", cellStyle: { 'text-align': 'left' } },
            { headerName: "Alias", width: 180, field: "Alias", cellStyle: { 'text-align': 'left' } },
            { headerName: "Code", width: 180, field: "Code", cellStyle: { 'text-align': 'left' } },
            { headerName: "RegdNo", width: 180, field: "RegdNo", cellStyle: { 'text-align': 'center' } },
            { headerName: "EngineNo", width: 180, field: "EngineNo", cellStyle: { 'text-align': 'center' } },
            { headerName: "ChassisNo", width: 180, field: "ChassisNo", cellStyle: { 'text-align': 'center' } },
            { headerName: "CostRate", width: 180, field: "Rate", cellStyle: { 'text-align': 'right' } },
        ...
        $scope.gridOptions = {
            ...
            onFilterChanged: function () {
                ...
                $scope.gridOptions.api.forEachNodeAfterFilterAndSort(function (node) {
                    if (node.data.Rate == 1 || node.data.Rate == 'DR')
                        Opening += node.data.Rate;
                    else
                        Opening -= node.data.Rate;
                });
                ...
                $scope.dataForBottomGrid[0].Rate = Opening;
                $scope.dataForBottomGrid[0].Code = drcr;
 ...
 $timeout(function () {
    ...
    $scope.GetFixedProductSalesSummary = function () {

        var dateFrom = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));
        var dateTo = new Date(($filter('date')(new Date(), 'yyyy-MM-dd')));

        if ($scope.FixedProductSalesSummary.DateFromDet)
            dateFrom = new Date(($filter('date')($scope.FixedProductSalesSummary.DateFromDet.dateAD, 'yyyy-MM-dd')));

        if ($scope.FixedProductSalesSummary.DateToDet)
            dateTo = new Date(($filter('date')($scope.FixedProductSalesSummary.DateToDet.dateAD, 'yyyy-MM-dd')));
        ...
        var beData = {
            DateFrom: dateFrom,
            DateTo: dateTo,
            VoucherType: $scope.FixedProductSalesSummary.VoucherId,
            isPost: $scope.FixedProductSalesSummary.IsPost,
            branchId: $scope.FixedProductSalesSummary.BranchId
        };
        ...
        $http({
            method: "post",
            url: base_url + "Inventory/Reporting/GetFixedProductSalesSummary",
	...
	$scope.DownloadAsXls = function () {
        ...
        $http({
            ...
            hidePleaseWait();
            if (res.data.IsSuccess && res.data.Data) {
                down_file(base_url + "//" + res.data.Data.ResponseId, "FixedProductSalesSummary.xlsx");
            }
