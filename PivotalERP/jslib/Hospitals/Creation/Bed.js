app.controller('Bed', function ($scope, $filter, $http, $timeout, GlobalServices) {
    $scope.Title = 'Bed';
    ...
    $scope.LoadData = function () {
        ...
        $scope.perPage = {
            Bed: GlobalServices.getPerPageRow(),
        };
    ...
    $scope.GetAllBed = function () {
        /* $scope.BedList = [];*/ //declare an empty array
        $scope.loadingstatus = 'running';
        showPleaseWait();
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetAllBed",
            dataType: "json"
    ...
    $scope.IsValidBed = function () {
        return true;
    }

    $scope.SaveUpdateBed = function () {
        if ($scope.IsValidBed() == true) {
    ...
    $scope.CallSaveUpdateBed = function () {
        $scope.loadingstatus = 'running';
        showPleaseWait();
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/SaveBed",
    ...
    $scope.getBedById = function (beData) {
        $scope.loadingstatus = "running";
        var para = {
            BedId: beData.BedId
        };
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetBedById",
            dataType: "json",
            data: JSON.stringify(para)
        }).then(function (res) {
            $scope.loadingstatus = "stop";
    ...
    $scope.deleteBed = function (refData, ind) {
        Swal.fire({
            title: 'Are you sure to delete selected Bed :-' + refData.Name,
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    method: 'POST',
                    url: base_url + "Hospital/Creation/DelBed",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    hidePleaseWait();
