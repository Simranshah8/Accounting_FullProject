app.controller('Service', function ($scope, $filter, $http, $timeout, GlobalServices) {
    $scope.Title = 'Service';
    ...
    $scope.LoadData = function () {
        ...
        $scope.perPage = {
            Service: GlobalServices.getPerPageRow(),
        };
    ...
    $scope.GetAllService = function () {
        ...
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetAllService",
            dataType: "json"
    ...
    $scope.SaveUpdateService = function () {
        if ($scope.IsValidService() == true) {
    ...
    $scope.CallSaveUpdateService = function () {
        $scope.loadingstatus = 'running';
        showPleaseWait();

        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/SaveService",
    ...
    $scope.GetServiceById = function (beData) {

        $scope.loadingstatus = "running";
        var para = {
            ServiceId: beData.ServiceId
        };
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetServiceById",
            dataType: "json",
            data: JSON.stringify(para)
        }).then(function (res) {
            $scope.loadingstatus = "stop";
    ...
    $scope.DelService = function (refData, ind) {

        Swal.fire({
            title: 'Are you sure to delete selected Service:-' + refData.Name,
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                var para = {
                    ServiceId: refData.ServiceId
                };

                $http({
                    method: 'POST',
                    url: base_url + "Hospital/Creation/DelService",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    hidePleaseWait();
