app.controller('Designation', function ($scope, $filter, $http, $timeout, GlobalServices) {
...

            {
                ...
                cellRenderer: function (params) {
                    ...
                    eDiv.querySelector(".edit-btn").addEventListener("click", function () {
                        scope.GetDesignationById(params.data);
                        scope.$apply();
                    });
                    // DELETE CLICK
                    eDiv.querySelector(".delete-btn").addEventListener("click", function () {
                        scope.DelDesignation(params.data);
                        scope.$apply();
                    });
                    return eDiv;
                }
    ...
    $scope.GetAllDesignation = function () {
        ...
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetAllDesignation",
            dataType: "json"
    ...
    $scope.CallSaveUpdateDesignation = function () {
        ...
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/SaveDesignation",
    ...
    $scope.GetDesignationById = function (beData) {

        $scope.loadingstatus = "running";
        var para = {
            DesignationId: beData.DesignationId
        };
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetDesignationById",
            dataType: "json",
            data: JSON.stringify(para)
        }).then(function (res) {
            $scope.loadingstatus = "stop";
    ...
    $scope.DelDesignation = function (refData, ind) {

        Swal.fire({
            title: 'Are you sure to delete selected Designation:-' + refData.Name,
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    method: 'POST',
                    url: base_url + "Hospital/Creation/DelDesignation",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    hidePleaseWait();
