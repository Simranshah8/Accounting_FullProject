app.controller('Vital', function ($scope, $filter, $http, $timeout, GlobalServices) {
    $scope.Title = 'Vital';
    ...
    $scope.LoadData = function () {
        ...
        $scope.perPage = {
            Vital: GlobalServices.getPerPageRow(),
        };
        $scope.beData =
        {
            Name: '',
    ...
    $scope.GetAllVital = function () {
        /*  $scope.MethodsColl = []; *///declare an empty array
        $scope.loadingstatus = 'running';
        showPleaseWait();
                $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetAllVital",
            dataType: "json"
    ...
    $scope.SaveUpdateVital = function () {
        if ($scope.IsValidVital() == true) {
    ...
    $scope.CallSaveUpdateVital = function () {
        $scope.loadingstatus = 'running';
        showPleaseWait();
                $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/SaveVital",
    ...
    $scope.GetVitalById = function (beData) {
        $scope.loadingstatus = "running";
        var para = {
            VitalId: beData.VitalId
        };
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetVitalById",
            dataType: "json",
            data: JSON.stringify(para)
        }).then(function (res) {
            $scope.loadingstatus = "stop";
    ...
    $scope.DelVital = function (refData, ind) {
        Swal.fire({
            title: 'Are you sure to delete selected Vital:-' + refData.Name,
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                showPleaseWait();
                var para = {
                    VitalId: refData.VitalId
                };
                $http({
                    method: 'POST',
                    url: base_url + "Hospital/Creation/DelVital",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    hidePleaseWait();
