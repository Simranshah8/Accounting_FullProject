app.controller('Diagnosis', function ($scope, $filter, $http, $timeout, GlobalServices) {
    $scope.Title = 'Diagnosis';
    ...
    $scope.LoadData = function () {
        ...
        $scope.perPage = {
            Diagnosis: GlobalServices.getPerPageRow(),
        };
            ...
            { field: "Description", headerName: "Description", filter: 'agTextColumnFilter', flex: 2, cellStyle: { 'textAlign': 'left' } },
            {
                ...
                cellRenderer: function (params) {
                    ...
                    eDiv.querySelector(".edit-btn").addEventListener("click", function () {
                        scope.GetDiagnosisById(params.data);
                        scope.$apply();
                    });
                    // DELETE CLICK
                    eDiv.querySelector(".delete-btn").addEventListener("click", function () {
                        scope.DelDiagnosis(params.data);
                        scope.$apply();
                    });
                    return eDiv;
                }
    ...
    $scope.GetAllDiagnosis = function () {
        $scope.loadingstatus = 'running';
        showPleaseWait();
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetAllDiagnosis",
            dataType: "json"
    ...
    $scope.IsValidDiagnosis = function () {
    ...
    $scope.SaveUpdateDiagnosis = function () {
        if ($scope.IsValidDiagnosis() == true) {
    ...
    $scope.CallSaveUpdateDiagnosis = function () {
        $scope.loadingstatus = 'running';
        showPleaseWait();
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/SaveDiagnosis",
    ...
    $scope.GetDiagnosisById = function (beData) {
        $scope.loadingstatus = "running";
        var para = {
            DiagnosisId: beData.DiagnosisId
        };
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetDiagnosisById",
            dataType: "json",
            data: JSON.stringify(para)
        }).then(function (res) {
            $scope.loadingstatus = "stop";
    ...
    $scope.DelDiagnosis = function (refData, ind) {
        Swal.fire({
            title: 'Are you sure to delete selected Diagnosis:-' + refData.Name,
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                showPleaseWait();
                var para = {
                    DiagnosisId: refData.DiagnosisId
                };
                $http({
                    method: 'POST',
                    url: base_url + "Hospital/Creation/DelDiagnosis",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    hidePleaseWait();
