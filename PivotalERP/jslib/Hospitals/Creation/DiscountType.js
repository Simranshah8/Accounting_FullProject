app.controller('DiscountType', function ($scope, $filter, $http, $timeout, GlobalServices) {
    $scope.Title = 'DiscountType';
    var glSrv = GlobalServices;

    $scope.onBtExportCSV = function () {
        var params = {
            fileName: 'DiscountType.csv',
            sheetName: 'DiscountType'
        };
            ...
            { field: "Description", headerName: "Description", filter: 'agTextColumnFilter', flex: 2, cellStyle: { 'textAlign': 'left' } },
            {
                ...
                cellRenderer: function (params) {
                    ...
                    eDiv.querySelector(".edit-btn").addEventListener("click", function () {
                        scope.GetDiscountTypeById(params.data);
                        scope.$apply();
                    });
                    // DELETE CLICK
                    eDiv.querySelector(".delete-btn").addEventListener("click", function () {
                        scope.DelDiscountType(params.data);
                        scope.$apply();
                    });
                    return eDiv;
                }
    ...
    $scope.GetAllDiscountType = function () {
        ...
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetAllDiscountType",
            dataType: "json"
    ...
    $scope.CallSaveUpdateDiscountType = function () {
        ...
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/SaveDiscountType",
    ...
    $scope.GetDiscountTypeById = function (beData) {

        $scope.loadingstatus = "running";
        var para = {
            DiscountTypeId: beData.DiscountTypeId
        };
        $http({
            method: 'POST',
            url: base_url + "Hospital/Creation/GetDiscountTypeById",
            dataType: "json",
            data: JSON.stringify(para)
        }).then(function (res) {
            $scope.loadingstatus = "stop";
    ...
    $scope.DelDiscountType = function (refData, ind) {

        Swal.fire({
            title: 'Are you sure to delete selected Discount Type:-' + refData.Name,
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                $http({
                    method: 'POST',
                    url: base_url + "Hospital/Creation/DelDiscountType",
                    dataType: "json",
                    data: JSON.stringify(para)
                }).then(function (res) {
                    hidePleaseWait();
