...
//var app = angular.module("myApp", ['agGrid', 'angularUtils.directives.dirPagination', 'angular-cron-gen']);
...
//app.run(function ($rootScope, GlobalServices) {
//    GlobalServices.getGenConfig().then(function (res) {
//        if (res.data.IsSuccess && res.data.Data) {
//            $rootScope.GenConfig = res.data.Data;
//        }
//    }, function (reason) {
...
app.controller('mainController', ['$scope',  '$http', '$timeout', 'companyDet', 'generalConfig', function ($scope, $http, $timeout, companyDet, generalConfig) {
            ...
            if (res.data.IsSuccess && res.data.Data) {
                ...
                $http({
                    ...
                    if (res.data.IsSuccess==true && res.data.Data) {
                        $scope.SelectedBranch = res.data.Data;
                        $scope.LastSelectedBranch = res.data.Data;
                    }
                }, function (reason) {
                    alert('Failed' + reason);
                });
                ...
                if (res.data.IsSuccess && res.data.Data) {
                    angular.forEach(res.data.Data, function (ec) {
                        if (ec.PageURL.length > 0) {
                            if (ec.FilterPara && ec.FilterPara.length > 0) {
                                ec.PageURL = ec.PageURL + "?" + ec.FilterPara;
                                //ec.PageURL = ec.PageURL + "?" + encodeURIComponent(ec.FilterPara);
                            }                            
                            $scope.CustomEntityColl.push(ec);
                        }
                        else if (ec.UrlPath.length > 0) {
                            ec.PageURL = ec.UrlPath;
                            if (ec.FilterPara && ec.FilterPara.length > 0) {
                                ec.PageURL = ec.PageURL + "?" + ec.FilterPara;                               
                            }
                            $scope.CustomEntityColl.push(ec);
    ...
    $scope.ChangeDefaultBranch = function () {
        ...
        var lastBranch = angular.copy($scope.LastSelectedBranch);

        Swal.fire({
            title: 'Password',
            input: 'password',
            inputLabel: 'Enter Company Admin Password',
            inputPlaceholder: 'Type your password here...',
            showCancelButton: true
        }).then((result1) => {
            if (result1.isConfirmed) {
                if (result1.value && result1.value.length > 0) {
                    var pwd = result1.value;

                    var para = {
                        userName: $scope.SelectedBranch.Code,
                        pwd: pwd,
                    };

                    $http({
                        method: 'POST',
                        url: base_url + "Home/ChangeCompany",
                        dataType: "json",
                        ...
                        if (res.data.IsSuccess == true) {
                            ...
                            $http({
                                method: 'POST',
                                url: base_url + "Home/ChangeDefaultBranch",
                                dataType: "json",
                            ...
                            $scope.SelectedBranch = lastBranch;
                            Swal.fire(res.data.ResponseMSG);
                        }
                    }, function (reason)
                    {
