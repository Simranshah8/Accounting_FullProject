app.controller('ExpenseClaimController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.LoadData = function () {
		...
		$scope.ExpenseTypeColl = [{ id: 1, text: 'Travelling Expense(s)' }, { id: 2, text: 'Lodging Expense(s)' }, { id: 2, text: 'Fooding Expense(s)' }, { id: 2, text: 'Meeting Expense(s)' }, { id: 2, text: 'Office Expense(s)' }, { id: 2, text: 'Other Expense(s)' }]
		...
		$scope.newExpenseClaim = {
			...
			Reason: '',
			ExpAttachment: '',
			TranExpenseClaimDetailsColl: [],
			DocumentColl: [],
			AttechFiles: [],
			SelectEmployee: $scope.EmployeeSearchOptions[0].value,
			Mode: 'Save',
		};
		$scope.newExpenseClaim.TranExpenseClaimDetailsColl.push({});
	};
	...
	$scope.AddExpenseEntryColl = function (ind) {
		if ($scope.newExpenseClaim.TranExpenseClaimDetailsColl) {
			if ($scope.newExpenseClaim.TranExpenseClaimDetailsColl.length > ind + 1) {
				$scope.newExpenseClaim.TranExpenseClaimDetailsColl.splice(ind + 1, 0, {
					Description: ''
				})
			} else {
				$scope.newExpenseClaim.TranExpenseClaimDetailsColl.push({
	...
	$scope.DelExpenseEntryColl = function (ind) {
		if ($scope.newExpenseClaim.TranExpenseClaimDetailsColl) {
			if ($scope.newExpenseClaim.TranExpenseClaimDetailsColl.length > 1) {
				$scope.newExpenseClaim.TranExpenseClaimDetailsColl.splice(ind, 1);
			}
		}
	};
    ...
    $scope.CallSaveUpdateExpenseClaim = function () {
		...
		var RcPhoto = $scope.newExpenseClaim.TranExpenseClaimDetailsColl;
        ...
        $scope.newExpenseClaim.ExpenseClaimForId = $scope.newExpenseClaim.EmployeeDetails.UserId,
            angular.forEach($scope.newExpenseClaim.TranExpenseClaimDetailsColl, function (D) {
                if (D.ExpenseDateDet) {
        ...
        $http({
            method: 'POST',
            url: base_url + "Expense/Transaction/SaveExpenseClaim",
            ...
            transformRequest: function (data) {
				...
				angular.forEach($scope.newExpenseClaim.TranExpenseClaimDetailsColl, function (rg) {
