...
function DateTimeFormatAD(date) {
    ...
    if (date) {
        ...
        if (mtDate) {
            if (mtDate.format('YYYY') == '0001')
                ...
                return mtDate.format('YYYY-MM-DD hh:mm A');
        } else
            return '';
...
function Numberformat(val, noOfDecPlace) {
    ...
    if (noOfDecPlace == null || noOfDecPlace == undefined)
        nval = numeral(val).format('0,0.00');
    else if (noOfDecPlace == 0)
        nval = numeral(val).format('0,0');
    else if (noOfDecPlace == 1)
        nval = numeral(val).format('0,0.0');
    else if (noOfDecPlace == 2)
        nval = numeral(val).format('0,0.00');
    else if (noOfDecPlace == 3)
        nval = numeral(val).format('0,0.000');
    else if (noOfDecPlace == 4)
        nval = numeral(val).format('0,0.0000');
    else if (noOfDecPlace == 5)
        nval = numeral(val).format('0,0.00000');
    else if (noOfDecPlace == 6)
        nval = numeral(val).format('0,0.000000');
    else if (noOfDecPlace == 7)
        nval = numeral(val).format('0,0.0000000');
    else if (noOfDecPlace == 8)
        nval = numeral(val).format('0,0.00000000');
    else
        nval = numeral(val).format('0,0.00');
...
function NumberformatAC(val) {
    ...
    if (val > 0) {
        var nval = numeral(val).format('0,0.00');
        if (nval == 'NaN')
            return '';
        else
            return nval + ' DR';
...
app.filter('sumOfValue', function () {
    return function (data, key) {
        if (angular.isUndefined(data) || angular.isUndefined(key) || data == null)
            return 0;
        var sum = 0;
...
app.directive('generatePdf', function () {
    return {
        restrict: 'A',
        scope: {
            pdfTarget: '@',
        ...
        link: function (scope, element) {
            element.on('click', function () {
                const el = document.getElementById(scope.pdfTarget);
                if (!el) {
                ...
                // Remove scroll temporarily for full rendering
                el.style.overflow = 'visible';
                el.style.maxHeight = 'none';

                // Wait for browser to reflow (optional)
                setTimeout(() => {
                    html2canvas(el, { scale: 2 }).then(canvas => {
                        ...
                        const filename = scope.pdfFilename || 'document.pdf';
                        pdf.save(filename);
        ...
        element.bind('click', function () {
            ...
            $http({
                ...
                if (res.data.IsSuccess && res.data.Data) {
                    ...
                    if (scope.tableId.includes('#') == false)
...
app.directive("htmlEditor", ['$timeout', function ($timeout) {
    return {
        ...
        link: function (scope, el, attr, ngModel) {
            ...
            function variableButton(context) {
                var ui = $.summernote.ui;

                // 1. Create Dropdown UI
                var $dropdown = $('<div class="note-dropdown-menu" style="padding:10px; width:250px; border: 1px solid #ccc; background: #fff; z-index: 1050;"></div>');
                var $search = $('<input type="text" class="form-control" placeholder="Search variables..." style="margin-bottom:8px; width: 100%;">');
                var $list = $('<div class="variable-list" style="max-height:200px; overflow-y:auto;"></div>');
                ...
                function renderList(filter) {
                    ...
                    vars.forEach(function (v) {
                        if (!filter || v.datatype.toLowerCase().indexOf(filter.toLowerCase()) !== -1) {
                            var $item = $('<a href="javascript:void(0)" class="note-dropdown-item" style="display:block; padding:5px 10px; color:#333; text-decoration:none; border-bottom:1px solid #eee;">' + v.datatype + '</a>');
                            ...
                            $item.on('click', function (e) {
                                ...
                                var vVal = '##' + v.datatype.toLowerCase() + '##';

                                // CRITICAL: Use the element to invoke commands to ensure we hit the active instance
                                $(el).summernote('editor.restoreRange');
                                $(el).summernote('editor.focus');
                                $(el).summernote('editor.insertText', vVal);

                                // Manually trigger change for AngularJS
                                scope.$applyAsync(function () {
                                    ngModel.$setViewValue($(el).summernote('code'));
                                });
                            });
                ...
                // 2. Prevent dropdown clicks from stealing focus permanently
                $dropdown.on('mousedown', function (e) {
                    // Save range right before the focus shifts to the search box/dropdown
                    $(el).summernote('editor.saveRange');
                });
            ...
            var toolbar = $.extend(true, [], $.summernote.options.toolbar);
            toolbar.push(['custom', ['variables']]);
            ...
            var initEditor = function () {
                $(el).summernote({
                    ...
                    callbacks: {
                        ...
                        onBlur: function () { onFocus = false; },
                        onChange: function (contents) {
                            scope.$applyAsync(() => ngModel.$setViewValue(contents));
                        },
                        // Keep range updated as user types
                        onKeyup: function () { $(el).summernote('editor.saveRange'); },
                        onMouseup: function () { $(el).summernote('editor.saveRange'); }
                    }
                });
            };
            ...
            scope.$watch(() => ngModel.$modelValue, function (newValue) {
                if (!onFocus) {
                    var currentCode = $(el).summernote('code');
                    if (newValue !== currentCode) {
                        $(el).summernote('code', newValue || '');
                    }
                }
            });

            // Rebuild if variables list changes
    ...
    return {
        ...
        link: function (scope, elem, attrs, ngModelCtrl) {
            ...
            elem.bind('blur', function () {
                ...
                var lval = isEmptyAmt(ngModelCtrl.$modelValue);
                var mVal = isEmptyAmt(scope.minValue);
                if (mVal > 0) {
                    if (lval < mVal) {
                        setDisplayNumber(0);
                        Swal.fire('Please ! Enter Greater Than ' + mVal);
                    }
                }

                var maxVal = isEmptyAmt(scope.maxValue);
                if (maxVal > 0 && lval != 0) {
                    if (lval > maxVal) {
                        setDisplayNumber(0);
                        Swal.fire('Please ! Enter Less Than ' + maxVal);
                    }
                }
            });
            ...
            elem.on('keypress', function (event) {
                // ASCII codes: 46 = dot (.), 110 = numpad dot
                if (scope.noofdecimal === 0) {
            ...
            elem.on('input', function () {
                if (scope.noofdecimal === 0) {
                    elem.val(element.val().replace(/\./g, ''));
                }
            });
            ...
            function setDisplayNumber(val, formatter) {
                ...
                if (valStr.length === 1 && valStr[0] === '-') {
                    ...
                    if (!attrs.formatnumber || attrs.formatnumber == true) {
                        ...
                        if (hasValue(scope.noofdecimal) == true && scope.noofdecimal == 0)
                            displayValue = $filter('number')(displayValue, scope.noofdecimal);
                        else if (hasValue(scope.noofdecimal) == true && dvStr.indexOf('.') > 0) {
                ...
                if (attrs.positive == true && displayValue[0] === '-') {
...
app.directive("allLedger", ['$http', '$timeout', '$filter', function ($http, $timeout, $filter) {
    ...
    function link(scope, element, attrs, ngModel) {
        ...
        // --- NEW FLEXIBLE DROPDOWNPARENT LOGIC ---
        if (attrs.dropdownParentSelector) {
            var parentElement = $(attrs.dropdownParentSelector);
            if (parentElement.length > 0) {
                select2Options.dropdownParent = parentElement;
            } else {
                console.warn('Select2 Directive: The selector "' + attrs.dropdownParentSelector + '" did not find any elements to attach the dropdown to.');
            }
        }
        else if (element.hasClass('i-append-in-sec')) { // Fallback for old method
            var parentWrapper = element.closest('.select2-wrapper');
            if (parentWrapper.length > 0) {
                select2Options.dropdownParent = parentWrapper;
            } else {
                console.error("Select2 Directive: Element has 'i-append-in-sec' but no parent '.select2-wrapper' was found.");
            }
        }
        // --- END OF NEW LOGIC ---
        ...
        var isLedgerFocused = false;

        $(element).on("select2:open", function (e) {
            isLedgerFocused = true;
            if (showColumns != false) {
                ...
                if (!headerAdded) {
                    $(".select2-results").prepend(
                        `<div class="custom-result-product">
                                    <table class="custom-table-product sales-account-select2-w-fix" style="font-size: 12px;">
                                        <thead>
                                            <tr>
                    ...
                    $(".select2-dropdown").append(
                        `<div class="select2-footer d-flex justify-content-center cstm-select2-footer">
                        <button type="button" class="select2-add-new-btn" >
                            Add New
                        </button>
                    </div>`
                    );

                    var parentDiv = $('.select2-dropdown').closest('.select2-container');
                    parentDiv.addClass('has-add-new-button');
                    headerAdded = true;

                }

                $(".select2-footer").on("click", ".add-new-particular-btn", function () {
                    console.log("Add New Particular button clicked");
                });
        ...
        $(element).on("select2:close", function (e) {
            isLedgerFocused = false;
            //var element = document.getElementById("dvCustomCol");
            //if (element) {
            //    element.remove();
            //}
        ...
        $(document).on("keydown", function (e) {
            ...
            if (e.ctrlKey && e.key.toLowerCase() === "c") {
                ...
                if (selectedData && selectedData.length > 0) {
                    var dt = selectedData[0].data;
                    if (dt && dt.Name) {
                        var textToCopy = dt.Name;
                    ...
                    //navigator.clipboard.writeText(dt.Name);
                }
            }
...
app.directive("allProduct", ['$http', '$timeout', '$filter', function ($http, $timeout, $filter, $compile) {



    var linkFun = function (scope, element, attrs, ngModel, compile) {
        ...
        $(element).on("select2:open", function (e) {
            isLedgerFocused = true;
            //$('.select2-dropdown').css('min-width', '700px');
            $('.select2-container--open .select2-dropdown').css('min-width', '700px');
            if (!headerAdded) {
                $(".select2-results").prepend(
                    `<div class="custom-result-product">
                                    <table class="custom-table-product  sales-invoice-particular-select2" style="font-size: 12px;">
                                        <thead>
                                            <tr>
                                                <th style="width: 15%;">Code</th>
                                                <th style="width: 45%;">Name</th>
        ...
        $(element).on("select2:close", function (e) {
            ...
            isLedgerFocused = false;
            var selectedData = $(element).select2('data');
            if (selectedData && scope.productDetail && (!scope.productDetail.CustomDataColl || scope.productDetail.CustomDataColl == undefined)) {
        ...
        $(element).on("change", function (e) {
            ...
            if (selectedData && selectedData.length > 0) {
                ...
                if (showDetails && selectedProduct.ProductId) {
                    ...
                    var gid = (scope.godownId ? scope.godownId : 0);
                    var rtId = scope.rateType > 0 ? scope.rateType : 0;
                    var pid = scope.patientId > 0 ? scope.patientId : 0;
                    var eid = scope.entityId > 0 ? scope.entityId : 0;
                    showPleaseWait();
                    $http({
                        method: 'GET',
                        url: base_url + "Global/GetProductDetail?ProductId=" + selectedProduct.ProductId + "&LedgerId=" + lid + "&VoucherType=" + VoucherType + "&VoucherId=" + vid + "&GodownId=" + gid + "&RateTypeId=" + rtId + "&PatientId=" + pid + "&EntityId=" + eid,
                        dataType: "json"
                    }).then(function (res) {
                        hidePleaseWait();
                        if (res.data.IsSuccess && res.data.Data) {
            ...
            $timeout(function () {
                if (scope.productDetail) {
                    ...
                    if (selectedProduct.ProductId > 0) {
                        ...
                        $http({
                            method: 'GET',
                            url: base_url + "Global/GetProductDetail?ProductId=" + selectedProduct.ProductId + "&LedgerId=" + lid + "&VoucherType=" + VoucherType + "&VoucherId=" + vid + "&GodownId=" + gid + "&RateTypeId=" + rtId + "&PatientId=" + pid,
                            dataType: "json"
                        }).then(function (res) {
                            hidePleaseWait();
                            if (res.data.IsSuccess && res.data.Data) {
                    ...
                    return {
                        results: $.map(res.Data, function (item) {
                            ...
                            return {
                                ...
                                text: item.ChSrlNo + ' - ' + item.EngineNo + ' - ' + item.RegdNo,
                                id: item.TranId,
                                data: item
                            }
                        })
...
app.directive("multiSelectOption", ['$http', '$timeout', '$filter', function ($http, $timeout, $filter) {
    ...
    function link(scope, element, attrs, ngModel) {
        ...
        if (scope.udfentity && scope.udfentity.Source && scope.udfentity.Source.length > 0) {
            scope.MSData = {
                SelectedCount: 0,
                DataColl: [],
                searchData: '',
            ...
            var pPara = {
                source: scope.udfentity.Source,
            };

            $http({
                method: 'POST',
                url: base_url + "Setup/ReportWriter/GetParaValues",
                dataType: "json",
                data: JSON.stringify(pPara)
            }).then(function (res1) {

                if (res1.data.IsSuccess && res1.data.Data) {
                    var dtColl = res1.data.Data.DataColl;
                    if (dtColl) {
                        scope.MSData.DataColl = dtColl;
                    }
                }
            }, function (reason) {
                ...
                if (onSelectEvent == false) {
                    if (ngModel.$modelValue == null) {
                        ...
                        if (ngModel.$modelValue && ngModel.$modelValue.length > 0) {
                            var mxQry = mx(ngModel.$modelValue.toString().split(','));
        ...
        $(element).select2({
            ...
            ajax: {
                ...
                processResults: function (res) {
                    ...
                    if (res.Data.length > 0) {
                        scope.costcenterDetail = res.Data[0];
                        scope.costcenterDetail.Init = false;
                        ngModel.$setViewValue(scope.costcenterDetail.CostCenterId);
                    } else {
                        scope.costcenterDetail = null;
                        ngModel.$setViewValue(null);
                    }
        ...
        if (ngModelController) {
            ...
            scope.watcher = scope.$watch(function () {
                ...
                if (ngModelController.$modelValue) {
                    var dt = ngModelController.$modelValue;

                    if (!scope.dateDetail && scope.modelName)
                        dt = scope.modelName;

                    var m = moment(dt, momentFormat, true);
                    if (m.isValid()) {
                        var dt = m.toDate();
                        var bsDate = DateFormatAD(dt);
                        scope.dateDetail = {
                        ...
                        element.datepicker('update', ngModelController.$modelValue);
                    };
                }
            });
...
app.directive("udfComboBox", ['$http', '$timeout', '$filter', function ($http, $timeout, $filter) {
    ...
    var linkFun = function (scope, element, attrs, ngModel) {
        ...
        var qrySource = scope.qry;
        var varColl = extractVariables(qrySource);
        if (varColl && varColl.length > 0) {
            var curRowObjColl = (scope.curRow ? mx(Object.keys(scope.curRow)) : []);
            var tranObjColl = (scope.curTran ? mx(Object.keys(scope.curTran)) : []);
            varColl.forEach(function (pr) {
                var pName = pr.replaceAll("@", "");
                if (curRowObjColl.contains(pName) == true) {
                    var findPVal = scope.curRow[pName];
                    if (findPVal) {
                        qrySource = qrySource.replaceAll(pr, findPVal);
                    } else
                        qrySource = qrySource.replaceAll(pr, 'null');
                }
                else if (tranObjColl.contains(pName) == true) {
                    var findPVal = scope.curTran[pName];
                    if (findPVal) {
                        qrySource = qrySource.replaceAll(pr, findPVal);
                    } else
                        qrySource = qrySource.replaceAll(pr, 'null');
        ...
        // 🔹 Load ALL data ONCE
        $.ajax(
            base_url + "Global/GetCustomData?" + param({
                procName: '',
                qry: qry,
                asParentChild: false,
                tblNames: '',
            ...
            // 🔹 Initialize Select2 with ALL data
            $(element).select2({
                placeholder: placeholder,
                allowClear: true,
                width: '100%',
            ...
            // 🔹 Set model value if already exists
            if (ngModel.$modelValue) {
            ...
            ajax: {
                ...
                data: function (params) {
                    ...
                    if (params.term && params.term.length > 0)
                        qry = qrySource + (scope.qry.includes('where ') == true ? ' and ' : ' where ') + ' ' + colName + " like N'" + params.term + "%'";
                    else
                        qry = qrySource;
                    ...
                    $timeout(function () {
                        if (ngModel.$modelValue) {
                            ...
                            if (id != null && id !== "") {
                                ...
                                {
                                    procName: '',
                                    qry: qrySource + (scope.qry.includes('where ') == true ? ' and ' : ' where ') + colName + '=' + (isNumber(id) == true ? id : "'" + id + "'"),
                                    asParentChild: false,
                                    tblNames: '',
                                    colRelations: ''
                                };
                    ...
                    $timeout(function () {
                        if (ngModel.$modelValue) {
                            ...
                            if ((id != null && id !== "" && oldValue != newValue) || scope.valDetail == undefined) {
                                ...
                                var searchCol = (scope.udfentity ? (scope.udfentity.RefColumn && scope.udfentity.RefColumn.length > 0 ? scope.udfentity.RefColumn : scope.udfentity.Name) : '[UTranId]');
    ...
    var getTemplate = function (uf) {
        ...
        switch (ftype) {
            ...
            case 2:
                ...
                break;
            case 22:
                template = '<input type="text" class="form-control form-control-sm serial" date-picker date-style="1" ng-model="udf.UDFValue_TMP" onfocus="this.select()" date-detail="udf.UDFValueDet">';
                break;
            case 23:
                template = '<input type="text" class="form-control form-control-sm serial" date-picker date-style="2" ng-model="udf.UDFValue_TMP" onfocus="this.select()" date-detail="udf.UDFValueDet">';
                break;
            case 3: {

                if (uf.Source && uf.Source.length > 0) {
                    template = '<select cur-tran="curTran" cur-row="curRow" class="form-control form-control-sm serial" udf-select2 udf-det="udf" label="udf.DisplayName" colname="udf.NameId" qry="udf.Source" ng-model="udf.UDFValue" val-detail="udf.UDFValueDet"> </select >';
            ...
            case 21:
                {
                    if (uf.Source && uf.Source.length > 0) {
                        template = '<multi-select-option udfentity="udf" ng-model="udf.UDFValue" source="' + uf.Source + '" label="text" value="id" confirm-action="clickEvent()" ></multi-select-option>';
                    }
            ...
            case 19: {
                ...
                if (uf.Source && uf.Source.length > 0) {
                    template = '<select class="form-control form-control-sm" udf-select2 udf-det="udf" label="udf.Label"  colname="udf.Name"  qry="udf.Source" ng-model="udf.UDFValue" val-detail="udf.UDFValueDet">   </select >';

                } else {
                    template = '<select class="form-control form-control-sm" active-select2 ng-model="udf.UDFValue" > <option ng-repeat="ss in udf.DropDownList | commaBreak" ng-value="ss" ng-selected="ss==udf.' + uf.Name + '" > {{ ss }}</option> </select >';
                }
            ...
            case 24: {
                ...
                if (uf.Source && uf.Source.length > 0) {
                    template = '<select class="form-control form-control-sm" udf-combo-box udf-det="udf" label="udf.Label"  colname="udf.Name"  qry="udf.Source" ng-model="udf.UDFValue" val-detail="udf.UDFValueDet">   </select >';

                } else {
                    template = '<select class="form-control form-control-sm" ng-model="udf.UDFValue" > <option ng-repeat="ss in udf.DropDownList | commaBreak" ng-value="ss" ng-selected="ss==udf.' + uf.Name + '" > {{ ss }}</option> </select >';
                }
            }
            ...
            case 8:
            case 18:
                template = '<input type="number" class="form-control form-control-sm" ng-model="udf.UDFValue" onfocus="this.select()" ng-change="changeEvent()" ' + hasFormula + '>';
                break;
            case 9:
                ...
                break;
            case 20:
                template = '<input type="time" class="form-control form-control-sm" ng-model="udf.UDFValue" onfocus="this.select()" style="text-align:right" ng-change="changeEvent()">';
                break;
            case 5:
                template = '<input type="text" number-input class="form-control form-control-sm" ng-model="udf.UDFValue" onfocus="this.select()" style="text-align:right" ng-change="changeEvent()" noofdecimal="3" ' + hasFormula + '>';
                break;
            //case 6:
            ...
            //    break;
            case 7:
                template = '<textarea rows="5"  class="form-control form-control-sm" ng-model="udf.UDFValue" onfocus="this.select()"   ng-change="changeEvent()">';
                break;
            case 12:
                ...
                template = '<div  id="htm' + uf.Name + '" html-editor class="summernote" ng-model="udf.UDFValue" onfocus="this.select()"  ng-change="changeEvent()">';
                break;
            case 13:
                template = '<div class="person-photo-attachment-wrap text-center"> <div class="img-wrap mx-auto" style = "width:100%;"> <img ng-src="{{udf.' + uf.Name + 'Data || udf.' + uf.Name + '}}" class="img-fluid" alt="" id="img' + uf.Name + '"></div>';
                ...
                template = template + '<input id="choose-file' + uf.Name + uid + '" class="btn btn-outline-secondary btn-sm" style="display: none;" type="file" accept=".jpg,.jpeg,.png" file-model ng-model="udf.' + uf.Name + '_TMP" fileread="udf.' + uf.Name + 'Data"/>';
            ...
            case 15:
                ...
                template = template + ' <input type="file" class="custom-file-input" id="docSF' + uf.Name + uid + '"  file-model="udf.' + uf.Name + 'Files" ng-model="beData.UDFValueFiles">';
                template = template + ' <label class="custom-file-label" for="docSF' + uf.Name + uid + '">Choose file ({{ udf.' + uf.Name + 'Files.length }} )</label>';
                template = template + '</div >';
                break;
            case 16:
                template = '<div class="custom-file">';
                template = template + ' <input type="file" class="custom-file-input" id="docMF' + uf.Name + uid + '" multiple  file-model="udf.' + uf.Name + 'Files" ng-model="beData.UDFValueFiles">';
                template = template + ' <label class="custom-file-label" for="docMF' + uf.Name + uid + '">Choose file ({{ udf.' + uf.Name + 'Files.length }} )</label>';
                template = template + '</div >';
                break;
            case 17:
                ...
                template = template + '<input id="choose-file' + uf.Name + uid + '" class="btn btn-outline-secondary btn-sm" style="display: none;" type="file" accept="image/*, video/*" img-video-model ng-model="udf.UDFValue_TMP" fileread="udf.' + uf.Name + 'Data" fieldname="' + uf.Name + '" />';
                template = template + '<a ng-click="clickEvent()" href="#" class="text-danger d-block mt-3">remove current image</a></div >';
    ...
    var getTemplate = function (uf, beData) {
        ...
        switch (ftype) {
            ...
            case 21:
                {
                    if (uf.Source && uf.Source.length > 0) {
                        template = '<multi-select-option udfentity="udf" ng-model="beData.' + uf.Name + '" source="' + uf.Source + '" label="text" value="id" confirm-action="clickEvent()" ></multi-select-option>';
                    }
...
app.directive("allEmployee", ['$http', '$timeout', function ($http, $timeout) {

    function link(scope, element, attrs, ngModel) {

        var placeholder = attrs.placeholder || "** Select Employee **";
        ...
        $(element).select2({
            ...
            ajax: {
                ...
                processResults: function (res) {
                    ...
                    if (!res || res.IsSuccess === false) {
                        Swal.fire(res.ResponseMSG || "Failed to load employee");
                        return { results: [] };
                    }

                    if (res.Data.length > 0) {
                        scope.employeeDetail = res.Data[0];
                        scope.employeeDetail.Init = false;
                        ngModel.$setViewValue(scope.employeeDetail.UserId);
                    } else {
            ...
            initSelection: function (element, callback) {
                scope.$watch("model", function (newVal, oldValue) {
                    $timeout(function () {
                        if (ngModel.$modelValue || ngModel.$modelValue > 0) {
                            var id = ngModel.$modelValue;

                            if (id != null && id !== "" && id > 0 && (!scope.employeeDetail || scope.employeeDetail.UserId != id)) {
                                ...
                                return $.ajax(
                                    base_url + "Global/GetAllEmployee?" + param(params),
                                    {
                                        dataType: "json",
                                        type: "GET",
                                        // data: queryParameters
                                        ...
                                        else if (res.IsSuccess == true && res.Data.length > 0) {

                                            var item = res.Data[0];
                                            scope.employeeDetail = item;

                                            scope.$apply(function () {
                                                scope.employeeDetail.Init = true;
                                                scope.employeeDetail = item;
                                            });
        ...
        $(element).on("change", function (e) {
            ...
            if (selectedData && selectedData.length > 0) {
                var selectedEmp = selectedData[0].data;
                scope.employeeDetail = selectedEmp;
                scope.$apply(function () {
                    scope.employeeDetail = selectedEmp;
                });
                $timeout(function () {
                    scope.$apply(function () {
                        ngModel.$setViewValue(selectedEmp.UserId);
                        scope.confirmAction();
                    });
                });
