app.controller('AssetRequestController', function ($scope, $http, $timeout, $filter, $compile, GlobalServices, $document) {
	$scope.Title = 'AssetRequest';
	$scope.LoadData = function () {
		...
		$scope.perPage = {
			AssetRequest: GlobalServices.getPerPageRow(),
		};
		...
		$scope.newDetails.AssetsReqDetailsColl.push({
			ReqFrom_TMP: new Date(),
			ReqTO_TMP: new Date()
		});
	...
	$scope.reloadVoucherDate = function () {
		...
		$timeout(function () {
			...
			if (dateFormat == 2) { // BS only
				...
				dtPicker =
					'<input type="text" class="form-control form-control-sm" date-picker ' +
					'ng-model="newDetails.VoucherDate_TMP" date-detail="newDetails.VoucherDateDet" disabled' +
					'confirm-action="getVoucherNoOnly(1)" ' +
					'title="{{newDetails.VoucherDateDet.dateBS}}" ' +
					'date-style="1" id="dtVoucherDateAD">';
			}
	...
	$scope.IsValidAssetRequest = function () {
		if (!$scope.newDetails.RequestById || $scope.newDetails.RequestById === 0) {
			Swal.fire("Warning", "Please select Request By.", "warning");
			return false;
		}
		if ($scope.newDetails.RequestById === 2) {
		...
		var filledRows = $scope.newDetails.AssetsReqDetailsColl.filter(function (row) {
			return row.ParticularId && row.ParticularId !== 0;
		});
		// 1️⃣ at least one valid child row required
		if (filledRows.length === 0) {
			Swal.fire("Warning", "Please add at least one asset request detail.", "warning");
			return false;
		}
		...
		for (var i = 0; i < filledRows.length; i++) {
			...
			if (!p.ReqFrom_TMP || !p.ReqFromDet.dateAD) {
				Swal.fire("Warning", "Please select Request From Date at row " + row, "warning");
				return false;
			}
			if (!p.ReqTO_TMP || !p.ReqTODet.dateAD) {
				Swal.fire("Warning", "Please select Request To Date at row " + row, "warning");
				return false;
			}
		}
		return true;
	...
	$scope.SaveUpdateAssetRequest = function () {
		if ($scope.IsValidAssetRequest() == true) {
			if ($scope.confirmMSG.Accept == true) {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						$scope.CallSaveUpdateAssetRequest();
					}
				});
			} else
				$scope.CallSaveUpdateAssetRequest();
	...
	$scope.CallSaveUpdateAssetRequest = function () {
		...
		if ($scope.newDetails.VoucherDateDet) {
			...
			$scope.newDetails.VoucherDate = new Date();

		if ($scope.newDetails.AssetsReqDetailsColl) {
			$scope.newDetails.AssetsReqDetailsColl.forEach((S) => {
		...
		$scope.newDetails.AssetsReqDetailsColl =
			$scope.newDetails.AssetsReqDetailsColl.filter(function (row) {
				return row.ParticularId != null;
			});

		$http({
			method: 'Post',
			url: base_url + "AssetsManagement/Creation/SaveAssetRequest",
	...
	$scope.GetAllAssetRequest = function () {
		$scope.TableData = [];
		$scope.loadingstatus = "running";
		showPleaseWait();
		$http({
	...
	$scope.GetAssetRequestById = function (refData) {
		$scope.loadingstatus = "running";
		showPleaseWait();
		var para = {
		...
		$http({
			method: 'POST',
			url: base_url + "AssetsManagement/Creation/GetAssetRequestById",
			dataType: "json",
			...
			if (res.data.IsSuccess && res.data.Data) {
				...
				if (!$scope.newDetails.AssetsReqDetailsColl || $scope.newDetails.AssetsReqDetailsColl.length == 0) {
					$scope.newDetails.AssetsReqDetailsColl = [];
					$scope.newDetails.AssetsReqDetailsColl.push({});
				}
	...
	$scope.DelAssetRequestById = function (refData, ind) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				$http({
					method: 'POST',
					url: base_url + "AssetsManagement/Creation/DelAssetRequest",
					dataType: "json",
					data: JSON.stringify(para)
				}).then(function (res) {
					hidePleaseWait();
	...
	$scope.AddPHDDetails = function (ind) {
		if (!$scope.newDetails.AssetsReqDetailsColl[ind].QTY &&
			!$scope.newDetails.AssetsReqDetailsColl[ind].ReqFrom_TMP &&
			!$scope.newDetails.AssetsReqDetailsColl[ind].ReqTO_TMP) {
			Swal.fire({
				title: 'Warning!',
		...
		if ($scope.newDetails.AssetsReqDetailsColl) {
			if ($scope.newDetails.AssetsReqDetailsColl.length > ind + 1) {
				$scope.newDetails.AssetsReqDetailsColl.splice(ind + 1, 0, {
					ReqFrom_TMP: new Date(),
					ReqTO_TMP: new Date(),
				})
			} else {
	...
	$scope.delPHDDetails = function (ind) {
		if ($scope.newDetails.AssetsReqDetailsColl) {
			if ($scope.newDetails.AssetsReqDetailsColl.length > 1) {
				$scope.newDetails.AssetsReqDetailsColl.splice(ind, 1);
			}
		}
	};
	...
	$scope.onParticularChange = function (index, R) {
		var row = $scope.newDetails.AssetsReqDetailsColl[index];
		...
		if (index === $scope.newDetails.AssetsReqDetailsColl.length - 1) {
			$scope.newDetails.AssetsReqDetailsColl.push({
				ParticularId: null,
				CategoryId: null,
				PurposeId: null,
	...
	$scope.validateDateRange = function (pDet) {
		if (pDet.ReqFromDet && pDet.ReqTODet && pDet.ReqTODet.dateAD < pDet.ReqFromDet.dateAD) {
			pDet.ReqTO_TMP = null;
			Swal.fire('Invalid Date', 'Date To cannot be earlier then Date From', 'warning');
		}
	};
	...
	$scope.GetRequestAttById = function (refData) {
		$scope.loadingstatus = "running";
		showPleaseWait();
		var para = {
			TranId: refData.TranId
