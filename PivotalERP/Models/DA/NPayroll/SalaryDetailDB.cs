...
{
    internal class SalaryDetailDB : Dynamic.DataAccess.Common.CommonDB
    {
        ...
        public ResponeValues UpdateSalaryDetail(int UserId, List<Dynamic.BE.NPayroll.SalaryDetail> DataColl)
        {
        ...
        public Dynamic.BE.NPayroll.EmployeeForSalaryDetailCollections getAllSalaryDetail(int UserId, int EntityId, int? BranchId, int? DepartmentId, int? CategoryId, int YearId,int MonthId, int? CompanyRelationshipId)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    Dynamic.BE.NPayroll.EmployeeForSalaryDetail beData = new Dynamic.BE.NPayroll.EmployeeForSalaryDetail();
                    ...
                    if (!(reader[24] is DBNull)) beData.LevelId = Convert.ToInt32(reader[24]);
                    if (!(reader[25] is DBNull)) beData.LevelName = Convert.ToString(reader[25]);
                    dataColl.Add(beData);
                }
                reader.Close();
