...
namespace Dynamic.DA.NPayroll
{
	...
	internal class UnitsOfWorkDB
	{
		...
		public ResponeValues SaveUpdate(Dynamic.BE.NPayroll.UnitsOfWork beData, bool isModify)
		{
		...
		public Dynamic.BE.NPayroll.UnitsOfWorkCollections getAllUnitsOfWork(int UserId, int EntityId)
		{
			...
			try
			{
				...
				while (reader.Read())
				{
					Dynamic.BE.NPayroll.UnitsOfWork beData = new Dynamic.BE.NPayroll.UnitsOfWork();
		...
		public Dynamic.BE.NPayroll.UnitsOfWork getUnitsOfWorkById(int UserId, int EntityId, int UnitsOfWorkId)
		{
			Dynamic.BE.NPayroll.UnitsOfWork beData = new Dynamic.BE.NPayroll.UnitsOfWork();
			dal.OpenConnection();
			System.Data.SqlClient.SqlCommand cmd = dal.GetCommand();
			...
			try
			{
				...
				if (reader.Read())
				{
					beData = new Dynamic.BE.NPayroll.UnitsOfWork();
					if (!(reader[0] is DBNull)) beData.UnitsOfWorkId = reader.GetInt32(0);
					if (!(reader[1] is DBNull)) beData.Name = reader.GetString(1);
					if (!(reader[2] is DBNull)) beData.Alias = reader.GetString(2);
					if (!(reader[3] is DBNull)) beData.Description = reader.GetString(3);
