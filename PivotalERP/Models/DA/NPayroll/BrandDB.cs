...
namespace Dynamic.DA.NPayroll
{
    internal class BrandDB
    {
        ...
        public ResponeValues SaveUpdate(Dynamic.BE.NPayroll.Brand beData, bool isModify)
        {
        ...
        public Dynamic.BE.NPayroll.BrandCollections getAllBrand(int UserId, int EntityId)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    Dynamic.BE.NPayroll.Brand beData = new Dynamic.BE.NPayroll.Brand();
        ...
        public Dynamic.BE.NPayroll.Brand getBrandById(int UserId, int EntityId, int TranId)
        {
            Dynamic.BE.NPayroll.Brand beData = new Dynamic.BE.NPayroll.Brand();
            ...
            try
            {
                ...
                if (reader.Read())
                {
                    beData = new Dynamic.BE.NPayroll.Brand();
                    beData.TranId = reader.GetInt32(0);
                    if (!(reader[1] is DBNull)) beData.Name = reader.GetString(1);
                    if (!(reader[2] is DBNull)) beData.Code = reader.GetString(2);
                    if (!(reader[3] is DBNull)) beData.SNo = reader.GetInt32(3);
