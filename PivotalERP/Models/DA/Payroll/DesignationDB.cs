...
namespace Dynamic.DA.Payroll
{
	...
	internal class DesignationDB
	{
		...
		public ResponeValues SaveUpdate(Dynamic.BE.Payroll.Designation beData, bool isModify)
		{
		...
		public Dynamic.BE.Payroll.DesignationCollections getAllDesignation(int UserId, int EntityId)
		{
			...
			try
			{
				...
				while (reader.Read())
				{
					Dynamic.BE.Payroll.Designation beData = new Dynamic.BE.Payroll.Designation();
		...
		public Dynamic.BE.Payroll.Designation getDesignationById(int UserId, int EntityId, int DesignationId)
		{
			Dynamic.BE.Payroll.Designation beData = new Dynamic.BE.Payroll.Designation();
			dal.OpenConnection();
			System.Data.SqlClient.SqlCommand cmd = dal.GetCommand();
			...
			try
			{
				...
				if (reader.Read())
				{
					beData = new Dynamic.BE.Payroll.Designation();
					if (!(reader[0] is DBNull)) beData.DesignationId = reader.GetInt32(0);
					if (!(reader[1] is DBNull)) beData.Name = reader.GetString(1);
					if (!(reader[2] is DBNull)) beData.Code = reader.GetString(2);
					if (!(reader[3] is DBNull)) beData.OrderNo = reader.GetInt32(3);
