...
namespace Dynamic.DA.Payroll
{
	...
	internal class ServiceTypeDB
	{
		DataAccessLayer1 dal = null;
		public ServiceTypeDB(string hostName, string dbName)
		{
			dal = new DataAccessLayer1(hostName, dbName);
		}

		public ResponeValues SaveUpdate(Dynamic.BE.Payroll.ServiceType beData, bool isModify)
		{
			...
			if (isModify)
			{
				cmd.CommandText = "usp_UpdateServiceType";
			}
			else
			{
				cmd.Parameters[5].Direction = System.Data.ParameterDirection.Output;
				cmd.CommandText = "usp_AddServiceType";
			}
		...
		public ResponeValues DeleteById(int UserId, int EntityId, int ServiceTypeId)
		{
			...
			cmd.CommandText = "usp_DelServiceTypeById";
			cmd.Parameters.Add("@ResponseMSG", System.Data.SqlDbType.NVarChar, 254);
			cmd.Parameters.Add("@IsSuccess", System.Data.SqlDbType.Bit);
			cmd.Parameters.Add("@ErrorNumber", System.Data.SqlDbType.Int);
		...
		public Dynamic.BE.Payroll.ServiceTypeCollections getAllServiceType(int UserId, int EntityId)
		{
			...
			cmd.Parameters.AddWithValue("@UserId", UserId);
			cmd.Parameters.AddWithValue("@EntityId", EntityId);
			cmd.CommandText = "usp_GetAllServiceType";
			try
			{
				System.Data.SqlClient.SqlDataReader reader = cmd.ExecuteReader();
				while (reader.Read())
				{
					Dynamic.BE.Payroll.ServiceType beData = new Dynamic.BE.Payroll.ServiceType();
		...
		public Dynamic.BE.Payroll.ServiceType getServiceTypeById(int UserId, int EntityId, int ServiceTypeId)
		{
			Dynamic.BE.Payroll.ServiceType beData = new Dynamic.BE.Payroll.ServiceType();
			dal.OpenConnection();
			System.Data.SqlClient.SqlCommand cmd = dal.GetCommand();
			...
			cmd.Parameters.AddWithValue("@UserId", UserId);
			cmd.Parameters.AddWithValue("@EntityId", EntityId);
			cmd.CommandText = "usp_GetServiceTypeById";
			try
			{
				System.Data.SqlClient.SqlDataReader reader = cmd.ExecuteReader();
				if (reader.Read())
				{
					beData = new Dynamic.BE.Payroll.ServiceType();
					if (!(reader[0] is DBNull)) beData.ServiceTypeId = reader.GetInt32(0);
					if (!(reader[1] is DBNull)) beData.Name = reader.GetString(1);
					if (!(reader[2] is DBNull)) beData.Code = reader.GetString(2);
					if (!(reader[3] is DBNull)) beData.OrderNo = reader.GetInt32(3);
