...
namespace Dynamic.DA.Attendance
{
    internal class LeaveOpeningDB
    {
        ...
        public ResponeValues SaveUpdate(Dynamic.BE.Attendance.LeaveOpening beData, bool isModify)
        {
        ...
        private void SaveLeaveDetail(int TranId, Dynamic.BE.Attendance.LeaveQuotaDetailsCollections dataColl)
        {
            System.Data.SqlClient.SqlCommand cmd = dal.GetCommand();
            cmd.CommandType = System.Data.CommandType.StoredProcedure;
        ...
        public Dynamic.BE.Attendance.LeaveOpeningCollections getAllLeaveOpening(int UserId)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    Dynamic.BE.Attendance.LeaveOpening beData = new BE.Attendance.LeaveOpening();
        ...
        public Dynamic.BE.Attendance.LeaveOpening getLeaveOpeningById(int UserId,int UsersId, int PeriodId)
        {
            Dynamic.BE.Attendance.LeaveOpening beData = new BE.Attendance.LeaveOpening();
        ...
        public Dynamic.BE.Attendance.LeaveOpeningCollections getAllEmpLeaveOpening(int UserId, int? BranchId, int? DepartmentId, int? CategoryId, int PeriodId,int EmployeeOrSalesman)
        {
            Dynamic.BE.Attendance.LeaveOpeningCollections dataColl = new Dynamic.BE.Attendance.LeaveOpeningCollections();
            dal.OpenConnection();
            System.Data.SqlClient.SqlCommand cmd = dal.GetCommand();
            ...
            cmd.CommandText = "usp_GetEmployeForLeaveOpening";
            try
            {
                System.Data.SqlClient.SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    Dynamic.BE.Attendance.LeaveOpening beData = new Dynamic.BE.Attendance.LeaveOpening();
                    ...
                    if (!(reader[14] is DBNull)) beData.NoOfLeave = Convert.ToDouble(reader[14]);
        ...
        public ResponeValues SaveUpdateLeaveOpeningColl(int UserId, Dynamic.BE.Attendance.LeaveOpeningCollections dataColl)
        {
            ResponeValues resVal = new ResponeValues();
            dal.OpenConnection();
            System.Data.SqlClient.SqlCommand cmd = dal.GetCommand();
            ...
            try
            {
                foreach (var beData in dataColl)
                {
                    ...
                    cmd.CommandText = "usp_SaveLeaveOpeningColl";
                    cmd.ExecuteNonQuery();
                    ...
                    if (resVal.IsSuccess && beData.LeaveQuotaDetail != null)
                    {
                        SaveLeaveDetail(resVal.RId, beData.LeaveQuotaDetail);
                    }
                }
                ...
                resVal.ResponseMSG = "Leave Opening Saved";
            }
            catch (System.Data.SqlClient.SqlException ee)
            {
                resVal.IsSuccess = false;
